...
\begin{itemize}
 ...
 \item \emph{Constant in time} (\texttt{constant})
  ...
  \item \emph{Reading atmosphere boundary conditions from a file} (\texttt{given})
    ...
    This atmosphere model is similar to \texttt{constant} in that it uses \texttt{artm} and \texttt{precip} fields given by the user by providing them directly to the surface model. The name of the file PISM will read \texttt{artm} and \texttt{precip} from is specified using the \fileopt{atmosphere_file} option.
 ...
 The \texttt{lapse_rate} modifier allows correcting air temperature and precipitation using elevation lapse rates. It uses the following options.
  \begin{itemize}
  \item \fileopt{atmosphere_file} specifies the file containing the reference surface elevation field (standard name: \texttt{surface_altitude}). This file can contain several surface elevation records to use lapse rate corrections relative to time-dependent surface. If one record is provided, the reference surface elevation is assumed to be time-independent.
  \item \intextoption{atmosphere_period} gives the period, in model years, to use when interpreting data in the file given with \texttt{-atmosphere_file},
  \item \intextoption{atmosphere_reference_year} takes the time $T$ in model years. The record for $t$ years in \texttt{-atmosphere_file} is interpreted as corresponding to $t$ years since $T$.
  \item \intextoption{atmosphere_time_average} enables interpolating (and in some cases, averaging) B.C. data in time.
...
\begin{itemize}
  ...
  \item \emph{Reading top-surface boundary conditions from a file} (\texttt{given})
    ...
    This surface model is similar to \texttt{constant} in that it uses \texttt{artm} and \texttt{acab} fields given by the user by providing them directly to the ice dynamics model. The name of the file PISM will read \texttt{artm} and \texttt{acab} from is specified using the \fileopt{surface_file} option.
...
Option \txtopt{surface_period}{years} makes PISM interpret data in \texttt{-surface_file} as periodic. In this case the time in the NetCDF file is understood as the time \emph{from the beginning of a period}, i.e. from the beginning of a year with \texttt{-surface_period 1}, from the beginning of a decade with \texttt{-surface_period 10}, etc.
...
\begin{verbatim}
$ pimsr -surface given -surface_file foo.nc -surface_period 1
\end{verbatim}%$

To force PISM using monthly records with longer periods, just add more records to the \texttt{-surface_file}  and change the \texttt{-surface_period} value.
...
In this case \texttt{foo.nc} contains fields \texttt{acab} and \texttt{artm}, as normal for \texttt{-surface constant}, and \texttt{bar.nc} contains field \texttt{thk} which will serve as the target thickness.  Option \intextoption{force_to_thk_alpha} adjusts the value of $\alpha_0$, which has a default value specified in the \emph{Source Code Browser} \url{\PISMBROWSERURL}.
  ...
  \begin{itemize}
  \item \fileopt{surface_file} specifies the file containing the reference surface elevation field (standard name: \texttt{surface_altitude}). This file can contain several surface elevation records to use lapse rate corrections relative to time-dependent surface. If one record is provided, the reference surface elevation is assumed to be time-independent.
  \item \intextoption{surface_period} gives the period, in model years, to use when interpreting data in the file given with \texttt{-surface_file},
  \item \intextoption{surface_reference_year} takes the time $T$ in model years. The record for $t$ years in \texttt{-surface_file} is interpreted as corresponding to $t$ years since $T$.
  \item \intextoption{surface_time_average} enables interpolating (and in some cases, averaging) B.C. data in time.
  \item \intextoption{temp_lapse_rate} gives the temperature lapse rate, in $K/km$. Note that we use the following definition of the temperature lapse rate:
    \begin{displaymath}
      \gamma = -\frac{dT}{dz}.
    \end{displaymath}
