...
\end{verbatim}%$
chooses a combination of SeaRISE-Greenland atmosphere model (using an air temperature parameterization and constant-in-time precipitation),  ice-core derived temperature forcing, a PDD local surface mass balance model, and a default (constant) ocean model.
...
\item \textbf{PISM atmosphere models.}
  \begin{itemize}
  \item \emph{SeaRISE-Greenland} (\verb|searise_greenland|)
  ...
  \item \emph{Constant in time} (\verb|constant|)
    ...
    This atmosphere model reads the snow precipitation (variable \verb|snowprecip|) and the mean annual near-surface air temperature (variable \verb|temp_ma|) and provides them to a selected surface model.
  ...
  \item \intextoption{anomaly\und precip} specifies a file containing spatially-variable ice-equivalent precipitation anomalies (in thickness per time) (variable \verb|snowprecip_anomaly|).
  \end{itemize}
  ...
  Note that only one temperature forcing mechanism can be used at any time.

\item \textbf{PISM surface (snow) process models}
  \begin{itemize}
  \item \emph{The ``invisible'' model} (\verb|simple|)

    This is the simplest ``surface model'', re-interpreting precipitation as mass flux into the ice and mean annual near-surface air temperature as the temperature of the ice at the ice surface (below firn). (This implies that there is no melt as long as precipitation is non-negative.)

    However primitive it may be, this model can be useful: to model Antarctica, for example.
  \item \emph{Constant in time} (\verb|constant|)

    This surface model reads and provides constant in time top surface boundary conditions (see table \ref{tab:ice-dynamics-bc}). When PISM is used with this model, variables \verb|artm| (ice temperature at the ice surface) and \verb|acab| are expected to be present in an input file.
  ...
  \item \emph{Local mass balance (PDD)} (\verb|pdd|)
    ...
    This surface model can be used to 
  \end{itemize}
