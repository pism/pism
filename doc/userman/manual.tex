...
\normalspacing
PISM is a joint project between developers in the ice sheet modeling group at the University of Alaska (UAF) and in Anders Levermann's research group at the Potsdam Institute for Climate Impact Research (PIK).  The number of source code and script contributors, documentation authors, and source code authors is now large enough so that perhaps the best description is an alphabetical list, below.  Users will reach the \underline{underlined} developers at UAF by using the email address \texttt{help\@@pism-docs.org}.
...
\textbf{\underline{Andreas Aschwanden}} & scripts, visualization, SeaRISE-Greenland, Storglaci\"aren \\
