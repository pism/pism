...
This \emph{User's Manual} describes how to run PISM using certain publicly-available data for the Greenland ice sheet and the Ross ice shelf.  It illustrates how PISM's numerical approximations are verified.  It documents all the PISM options.

But it does not explain the design of the code, the numerical models, nor how to extend the functionality of PISM.  Users who want to understand how PISM works, extend it, or generally advance the science of ice sheets will need to go beyond what is described here.  For such users there is a \emph{Source Code Browser}\index{PISM!\emph{Source Code Browser (HTML)}} which can also be found online.  It gives a complete view of the class/object structure of the PISM source code.
...
\noindent We do not claim to follow these principles perfectly, but principle 1 is illustrated by our existence and principle 2 by examples in sections \ref{sec:start}, and \ref{sec:ross}.  Principle 3 is addressed in section \ref{sec:verif}.  Principles 4, 5, 6, and 7 are addressed next.
...
\emph{PISMOceanModel} therein.  Also, section \ref{sec:start} describes a
