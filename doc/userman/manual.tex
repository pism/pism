...
\begin{titlepage}
  \begin{center}
    ...
    \small \today. Support by email: \texttt{help\@@pism-docs.org}. Based
    on PISM \PISMREV\,and PETSC release \PETSCREL. \PISMDOWNLOADMSG
  \end{center}
\end{titlepage}
...
By default, PISM does not numerically solve the SSA at all.  The user must add the option \texttt{-ssa} (FIXME).  An exception is the use of the executable \texttt{pismv} for verifying the SSA balance of momentum equations, as addressed in section \ref{sect:verif}.
...
\small
\begin{tabular}{p{0.18\linewidth}p{0.4\linewidth}p{0.3\linewidth}}\hline
