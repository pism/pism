...
\vspace{1.0in}
\begin{quote}
\textsl{Copyright (C) 2004--2010 Ed Bueler and Constantine Khroulev and Jed Brown and Nathan Shemonski}
\medskip
...
The Arctic Region Supercomputing Center\index{Arctic Region Supercomputing Center (ARSC)} has provided significant computational resources and technical help in the development of PISM, including the vital support of postdoc Andy Aschwanden.
...
\subsection{EISMINT II}\label{subsect:EISMINTII}  There are seven experiments described in the published EISMINT II writeup \cite{EISMINT00}.\index{EISMINT}  They are named A, B, C, D, F, G, and H.  They have these common features:\begin{itemize}
...
See table \ref{tab:eisII} for how to run all EISMINT II experiments in PISM.  Note that the vertical grid is not specified in EISMINT II, and it seems that good simulation of the thermomechanically-coupled conditions near the base of the ice requires relatively-fine resolution there.  (It is difficult to be quantitative because of a lack of theory.)  Our suggested default unequally-spaced grid with 61 levels gives a grid spacing of 21 m.  An equally-spaced grid is an option in PISM, and in this case we suggest using 201 vertical levels to give 25 m spacing.  Compare these:

\small
\begin{verbatim}
pisms -eisII A -Mx 61 -My 61 -Mz 201 -z_spacing equal -skip 5 -y 2e5 -o eisIIA_equal.nc
pisms -eisII A -Mx 61 -My 61 -Mz 61 -skip 5 -y 2e5 -o eisIIA.nc
\end{verbatim}
\normalsize
\noindent The latter run is about a factor of two faster because of reduced cache memory use for the three-dimensional temperature field.

These SIA-only simulations parallelize well, and we see speedups up to at least 30 processors for the standard $61\times 61$ horizontal grid.
...
\begin{table}[ht]
\caption{Running the EISMINT II experiments in PISM.\index{PISM!running the EISMINT II experiments in}  Use \t{-skip 5}, on the $61\times 61$ default grid, for significant speedup.  Replace ``\t{-Mz 61}'' with \t{-Mz 201 -z\und spacing equal} for equal vertical spacing.}\label{tab:eisII}
\small
\begin{tabular}{@{}llll}\hline
...
The script \verb|ssh2.sh| uses options \intextoption{ts\und file} and \intextoption{save\und file} to do the run with ``snapshots'' saved every 10000 model years, and the volume saved every 100 model years.  So out discussion will now assume that the user \emph{actually} did this to completion:
...
%\includegraphics[height=3.7in,width=2.9in]{greenTpa_SSL2}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=5in]{artm_timeseries}
...
\begin{verbatim}
$ pcctest -i green20km_y1.nc -o bar.nc -ys -125000.0 -ye -0.0 -dt 1000.0  -dTforcing grip_dT.nc -dSLforcing specmap_dSL.nc -sma
\end{verbatim}
...
\subsubsection*{Additional visualization}  The visualization abilities of PISM's runtime viewers, and of \verb|ncview| for NetCDF files, are limited.  On the other hand, we can produce pretty pictures using Python.  The script \verb|rossplot.py| uses Python packages \verb|netcdf4-python|, \verb|matplotlib|, and \t{scikits.delaunay} to do this.  Assuming that \verb|rossComputed.nc| was produced as described previously, then we can do:

\verb|$ ./rossplot.py --pism-output=rossComputed.nc --riggs=riggs_clean.dat|
...
Note that each of the requested variables must have missing values specified
according to CF Metadata conventions. Namely, each of the listed variables must
have one of the following: \verb|valid_range| or both of \verb|valid_min| and
\verb|valid_max| defined if the values are in a specific range; one of
\verb|valid_min| and \verb|valid_max| if values are greater (or less,
respectively) than some value, or \verb|_FillValue|. Also note that
\verb|_FillValue| is interpreted as \verb|valid_max| if it is positive and as
\verb|valid_min| otherwise; see
\href{http://www.unidata.ucar.edu/software/netcdf/guide_10.html#SEC76}{NetCDF
  User's Guide: Attributes} for details. The \verb|missing_value| is deprecated
by the NetCDF User's Guide, but is supported for backward compatibility

Please run \texttt{fill\und missing.py --help} for the list of available
command-line options.


\scripthead{test/vfnow.py}\index{vfnow.py}  This Python script in directory \verb|test/| organizes the process of verifying PISM.  It specifies standard refinement paths for each of the tests described in section \ref{sect:verif}.  It runs the tests, times them, and summarizes the numerical errors reported at the end.

In a standard PISM installation, runs as \verb|test/vfnow.py|, or \texttt{cd test/} and do \texttt{./vfnow.py}. Example usage:
\begin{itemize}
\item ``\verb|vfnow.py|'' without options will use one processor and do three levels of refinement; this command is equivalent to \verb|vfnow.py -n 1 -l 3 -t CGIJ|,
\item ``\verb|vfnow.py -n 8 -l 5 -t J --prefix=bin/ --mpido='aprun -n'|'' will use \verb|aprun -n 8 bin/pismv| as the command and do five levels (the maximum) of refinement only on test J,
\item ``\verb|vfnow.py -n 2 -l 3 -t CEIJGKL -u 1|'' uses two processers (cores) and runs in about two hours; the vertical spacing is not equal (\verb|-z_spacing quadratic|),
\item ``\verb|vfnow.py -n 40 -l 5 -t ABCDEFGIJKLM|'' will use forty processors to do all possible verification as managed by \verb|vfnow.py|; don't run this unless you have a big computer and you are prepared to wait.
\end{itemize}

For a list of options do \verb|test/vfnow.py --help|.

Timing information is given in the \verb|vfnow.py| output so performance, including parallel performance, can be assessed along with accuracy.  The exact meaning of the numerical errors reported by \verb|pismv|, which appear in a \verb|vfnow.py| output as well, is currently only documented in the source files \verb|pism/src/verif/iceCompModel.cc|, \verb|pism/src/verif/iCMthermo.cc|, and \verb|pism/src/verif/iceExactSSAModel.cc|.  


\scripthead{test/vnreport.py}\index{vnreport.py}  This Python script processes
a NetCDF report file created by \texttt{vfnow.py} and creates plots similar
to figures \ref{fig:thickerrsB}--~\ref{fig:temperrsK}.

The basic use is this:
\begin{verbatim}
$  vfnow.py -n 2 -l 5 -t K -r K.nc
$  vnreport.py -i K.nc -t K
\end{verbatim}

The \verb|vfnow.py| run will take a while.  The \verb|vnreport.py| run will
(quickly) look at \verb|K.nc| and extract the geometry (thickness) numerical
errors and refinement path and create and image like that in Figure
\ref{fig:temperrsK}. This script does not save the image created, you can
do that using the ``save'' button of the displayed dialog.
...
Please run \texttt{vnreport.py --help} for the list of all the options accepted.
