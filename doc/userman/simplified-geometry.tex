...
There are many runs for a complete MISMIP intercomparison submission.  Specifically, there are $62$ runs for each model and grid choice, three (suggested) grid choices, and two models, and thus a full suite is $6 \times 62 = 372$ runs!
...
\noindent first generate a bash script, then use it to do a run (which takes about 20 minutes), and then generate an image in \texttt{.png} format.  It is shown in Figure \ref{fig:MISMIPmodel1exper1aA7} (left).
...
The effect when we do this at higher resolution (grid mode 3 with 6 km spacing, instead of 12 km in mode), is an improvement; see the right parts of Figure \ref{fig:MISMIPmodel1exper1aA7} and \ref{fig:cflx1aA1}.
