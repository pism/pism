...
\renewcommand{\optindexsection}{Output}
...
bigger (or smaller) using the \intextoption{o_size} command-line option.

Possible choices are ``\texttt{-o_size small}'' (no diagnostic variables),
``\texttt{medium}'' (the default, adds variables listed in the \texttt{output_medium} configuration variable to model state fields) and ``\texttt{big}'',
saving \emph{every} variable listed in Table \ref{tab:extra-vars}.

\subsection{Saving time series of scalar diagnostic quantities}\ind{time-series}\ind{PISM!saving time-series}
\label{sec:saving-time-series}
\renewcommand{\optindexsection}{Saving scalar time-series}
 It is also possible to save time-series of certain scalar diagnostic quantities using a combination of the options \intextoption{ts_file}, \intextoption{ts_times}, and \intextoption{ts_vars}.  For example,
\begin{verbatim}
...
Omitting the \intextoption{ts_vars} option makes PISM save \emph{all} the variables listed in table \ref{tab:time-series}.  Because scalar time-series take very little storage space, compared to spatially-varying data, this is usually a reasonable choice.

If the file \texttt{foo.nc}, specified by \texttt{-ts_file foo.nc}, already exists then by default the existing file will be moved to \texttt{foo.nc~} and the new time series will go into \texttt{foo.nc}.  To append the time series onto the end of the existing file, use option \intextoption{ts_append}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\toprule
    ...
    \midrule
    \intextoption{ts_file} [file name] & Specifies the file to save to.\\
    \intextoption{ts_times} [range or list] & Specifies times to save at either as a MATLAB-style range $a:\Delta t:b$ or a comma-separated list. \\
    \intextoption{ts_vars} [list of variables] & Comma-separated list of variables, see table~\ref{tab:time-series}. Omitting this option is equivalent to listing all the possible variables.\\
...
\subsection{Saving time series of spatially-varying diagnostic quantities}\ind{PISM!saving diagnostic quantities regularly}
\label{sec:saving-spat-vari}
\renewcommand{\optindexsection}{Saving 2D and 3D time-series}
...
Use options \intextoption{extra_file}, \intextoption{extra_times}, and \intextoption{extra_vars} for this.  For example,
\begin{verbatim}
...
If the file \texttt{foo.nc}, specified by \texttt{-extra_file foo.nc}, already exists then by default the existing file will be moved to \texttt{foo.nc~} and the new time series will go into \texttt{foo.nc}.  To append the time series onto the end of the existing file, use option \intextoption{extra_append}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\toprule
    \textbf{Option} & \textbf{Description}\\
    \midrule
    \intextoption{extra_file} [file name] & Specifies the file to save to; should be different from the output \intextoption{o} file.\\
    \intextoption{extra_times} [range or list] & Specifies times to save at either as a MATLAB-style range $a:\Delta t:b$ or a comma-separated list.\\
    \intextoption{extra_vars} [list of variables]& Comma-separated list of variables, see table~\ref{tab:extra-vars}\\
    \intextoption{extra_split} & Save to separate files, similar to \intextoption{split_snapshots}\\
...
\subsection{Saving re-startable snapshots of the model state}\ind{snapshots of the model state}\ind{PISM!saving snapshots of the model state}
\label{sec:snapshots}
\renewcommand{\optindexsection}{Saving snapshots}
...
If saving exactly at specified times is not critical, then use the \intextoption{save_file} and \intextoption{save_times} options.  For example,
\begin{verbatim}
...
\end{verbatim}
will initialize using the last record in the file, at about $5000$ years.  By contrast, to restart from $1500$ years (for example) it is necessary to extract the corresponding record using \texttt{ncks}\ind{NCO (NetCDF Operators)!ncks}
\begin{verbatim}
...
\intextoption{split_snapshots} option.  For example, the run above can be changed to
\begin{verbatim}
...
large file at each snapshot when \texttt{-split_snapshots} is not used.  Note
that tools like NCO\ind{NCO (NetCDF Operators)!wildcards} and
\texttt{ncview}\ind{NetCDF!tools!work with wildcards} usually behave as desired with wildcards like ``\texttt{snapshots-*.nc}''.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\toprule
    \textbf{Option} & \textbf{Description} \\
    \midrule
    \intextoption{save_file} [file name] & Specifies the file to save to.\\
    \intextoption{save_times} [range or list] & Specifies times at which to save snapshots, by either a MATLAB-style range $a:\Delta t:b$ or a comma-separated list. \\
    \intextoption{split_snapshots} & Separate the snapshot output into files named \texttt{snapshots-}year\texttt{.nc}.  Faster if you are saving more than a dozen or so snapshots. \\
    \bottomrule
  \end{tabular}
...
\renewcommand{\optindexsection}{Run-time diagnostic viewers}
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.4\linewidth}p{0.5\linewidth}}\toprule
   ...
   \textbf{Option} & \textbf{Description}\\
    \midrule
    \intextoption{view_map} [list of variables] & Turns on map-plane views of one or several variables, see tables \ref{tab:extra-vars}  \\
    \intextoption{view_slice} [list of variables] & Turns on viewers showing a slice of a 3D field at a certain level above the base of ice\\
    \intextoption{slice_level} number& Specifies the level for the previous option\\
    \intextoption{viewer_size} number & desired viewer size, in pixels\\
    \intextoption{view_sounding} [list of variables] &Turns on sounding viewers showing values in a column\\
...
\begin{center}
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\toprule
    ...
    \texttt{log_nuH} & log base ten of the vertically-averaged effective viscosity times thickness on the regular grid. Only meaningful in ice streams and shelves.\\
    \intextoption{ksp_monitor_draw} & Iteration monitor for the Krylov subspace routines (KSP) in PETSc. Shows norm of residual versus iteration number.\\
    \normalsize
  \end{tabular}
\end{center}
...
\renewcommand{\optindexsection}{PISM configuration file}
...
\label{subsect:pcctest}
\renewcommand{\optindexsection}{Visualizing climate inputs}
...
\texttt{pcctest}\ind{PISM!pcctest} allows one to visualize climate inputs without
...
\texttt{pcctest} takes options listed in table \ref{tab:pcctest}.

\begin{table}[ht]
  \caption{\texttt{pcctest} command-line options}
  \centering
  \begin{tabular}{p{0.1\linewidth}p{0.8\linewidth}}\toprule
    \textbf{Option} & \textbf{Description}\\
    \midrule
    \intextoption{i} & specifies an input file, which has been produced by PISM;\\
    \intextoption{o} & sets the output file name;\\
    \intextoption{ys} & sets the start time, in years;\\
    \intextoption{ye} & sets the final time, in years;\\
    \intextoption{dt} & sets the interval between saved climate snapshots.\\
    \bottomrule
 \end{tabular}
 \label{tab:pcctest}
\end{table}
...
\bigskip
As an example, set up an ice sheet state file and run \texttt{pcctest} on it:
\begin{verbatim}
...
EISMINT II choices \cite{EISMINT00}.  \texttt{pcctest} extracts the 
...
is used.  Viewing \texttt{camovie.nc} we see these same values as from \texttt{state.nc},
...
The excuse for the executable \texttt{pcctest} becomes clearer if we use a positive degree-day
model (subsection \ref{sec:boundary-models}).  The positive degree-day
model uses a variable called \texttt{snowaccum}, and a calculation of melting, to get the
surface mass balance \texttt{acab}.  As a demonstration here we use an NCO\ind{NCO (NetCDF Operators)!ncrename} (subsection 
\ref{subsect:nctoolsintro}) to change a variable name in \texttt{state.nc}.  Then we run \texttt{pcctest}
with option \texttt{-pdd} to invoke the \texttt{PISMPDDCoupler}
...
\end{verbatim}
In particular, a time-dependent view of the variable \texttt{artmpdd} shows the time- and space-
...
computation the net surface mass balance is computed, namely \texttt{acab} in \texttt{pddmovie.nc}.

See section \ref{sec:eismint-greenland} for another \texttt{pcctest} example.

\subsection{Regridding}
\renewcommand{\optindexsection}{Regridding}
...
Such ``regridding'' is done using the \intextoption{regrid_file} and \intextoption{regrid_vars} commands as in this example:\ind{pisms}
...
\intextoption{regrid_file}.  Only model state variables are regriddable, while climate and boundary data generally are not explicitly regriddable.  (Bootstrapping, however, allows the same general interpolation as this explicit regrid.)

\begin{table}[ht]
\centering
\caption{Regriddable variables.\ind{regrid}\ind{regrid_vars}  Use \texttt{-regrid_vars} with these names.}\label{tab:regridvar}
...
Here is another example: suppose you have an output of a PISM run on a fairly coarse grid (stored in \texttt{foo.nc}) and you want to continue this run on a finer grid. This can be done using \intextoption{regrid_file} along with \intextoption{boot_from}:
\begin{verbatim}
...
Here is an example.  Suppose we start a long verification run in the background, with standard out redirected into a file:\ind{pismv}

\verb|pismv -test G -Mz 101 -y 1e6 -o testGmillion.nc >> log.txt &|

\noindent This run gets a Unix process id,\ind{signals!the pid}, which we assume is ``8920''.  (Get it using \texttt{ps} or \texttt{pgrep}.)  If we want to observe the run without stopping it we send the \texttt{USR1} signal:\ind{signals!usr1}
...
\end{verbatim}
\noindent appears around that time step.  Suppose, on the other hand, that the run needs to be stopped.  Then a graceful way is\ind{signals!term}
...
\ind{PISM!adaptive time stepping scheme}
\renewcommand{\optindexsection}{Adaptive time-stepping scheme}
...
\begin{tabular}{p{0.05\linewidth}p{0.85\linewidth}}\toprule
...
\begin{table}[ht]
  ...
  \begin{tabular}{lp{0.7\linewidth}}
    ...
    \intextoption{max_dt} & The maximum time-step in years.  The adaptive
    time-stepping scheme will make the time-step shorter than this as needed
    for stability, but not longer.\\
    \intextoption{dt_force} & The time step (in years) to take, overriding the
    adaptive scheme. \emph{Not recommended.}\\
    \intextoption{skip} & Number of mass-balance steps, including SIA
    diffusivity updates, to perform before a the temperature, age, and SSA
...
\subsection{Utility and test scripts} \label{subsect:scripts}\ind{python scripts} Some nice scripts.
 
FIXME: the python scripts are all documented at the \textsl{Packages} tab on the browser

FIXME: the python scripts all take \texttt{fill_missing.py --help}
...
\begin{table}[h]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.65\linewidth}}\\
    ...
    \scripthead{util/check_polarstereo.py} & \\
    ...
    \scripthead{util/pism_config_editor.py} & \\
    \scripthead{util/pism_matlab.m} & \\
   \bottomrule
  \end{tabular}
