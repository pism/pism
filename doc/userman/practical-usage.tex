...
bigger (or smaller) using the \intextoption{o\_size} command-line option.

Possible choices are ``\texttt{-o\_size small}'' (no diagnostic variables),
``\texttt{medium}'' (the default, adds variables listed in the \texttt{output\_medium} configuration variable to model state fields) and ``\texttt{big}'',
...
\renewcommand{\optindexsection}{Saving scalar time-series}
 It is also possible to save time-series of certain scalar diagnostic quantities using a combination of the options \intextoption{ts\_file}, \intextoption{ts\_times}, and \intextoption{ts\_vars}.  For example,
\begin{verbatim}
...
Omitting the \intextoption{ts\_vars} option makes PISM save \emph{all} the variables listed in table \ref{tab:time-series}.  Because scalar time-series take very little storage space, compared to spatially-varying data, this is usually a reasonable choice.

If the file \verb|foo.nc|, specified by \verb|-ts_file foo.nc|, already exists then by default the existing file will be moved to \verb|foo.nc~| and the new time series will go into \verb|foo.nc|.  To append the time series onto the end of the existing file, use option \intextoption{ts\_append}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\hline
    ...
    \hline
    \intextoption{ts\_file} [file name] & Specifies the file to save to.\\
    \intextoption{ts\_times} [range or list] & Specifies times to save at either as a MATLAB-style range $a:\Delta t:b$ or a comma-separated list. \\
    \intextoption{ts\_vars} [list of variables] & Comma-separated list of variables, see table~\ref{tab:time-series}. Omitting this option is equivalent to listing all the possible variables.\\
    \intextoption{ts\_append} & Append time series to file if it already exists.  No effect if file does not yet exist. \\
    \hline
  \end{tabular}
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.4\linewidth}p{0.1\linewidth}p{0.4\linewidth}}\hline
   ...
   \hline
    ...
    \texttt{total\_surface\_ice \_flux} & kg$^{3}\,$s$^{-1}$ & total surface ice flux \\
    \texttt{total\_basal\_ice\_flux} & kg$^{3}$\,s$^{-1}$ & total basal ice flux \\
    \texttt{total\_sub\_shelf\_ice\_flux} & kg$^{3}\,$s$^{-1}$  & total sub-ice-shelf ice flux \\
...
Use options \intextoption{extra\_file}, \intextoption{extra\_times}, and \intextoption{extra\_vars} for this.  For example,
\begin{verbatim}
...
If the file \verb|foo.nc|, specified by \verb|-extra_file foo.nc|, already exists then by default the existing file will be moved to \verb|foo.nc~| and the new time series will go into \verb|foo.nc|.  To append the time series onto the end of the existing file, use option \intextoption{extra\_append}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\hline
    \textbf{Option} & \textbf{Description}\\
    \hline
    \intextoption{extra\_file} [file name] & Specifies the file to save to; should be different from the output \intextoption{o} file.\\
    \intextoption{extra\_times} [range or list] & Specifies times to save at either as a MATLAB-style range $a:\Delta t:b$ or a comma-separated list.\\
    \intextoption{extra\_vars} [list of variables]& Comma-separated list of variables, see table~\ref{tab:extra-vars}\\
    \intextoption{extra\_split} & Save to separate files, similar to \intextoption{split\_snapshots}\\
    \intextoption{extra\_append} & Append variables to file if it already exists.  No effect if file does not yet exist, and no effect if \intextoption{extra\_split} is set. \\
    \hline
  \end{tabular}
 \label{tab:extras}
\end{table}
...
\small \hspace{-2mm}
  \begin{longtable}{p{0.15\linewidth}p{0.15\linewidth}p{0.6\linewidth}}\hline
    ...
    \texttt{litho\_temp} & K & lithosphere (bedrock) temperature\\
    ...
    \texttt{temp\_pa} & $^{\circ}$C & pressure-adjusted ice temperature \\
...
If saving exactly at specified times is not critical, then use the \intextoption{save\_file} and \intextoption{save\_times} options.  For example,
\begin{verbatim}
...
\end{verbatim}
will initialize using the last record in the file, at about $5000$ years.  By contrast, to restart from $1500$ years (for example) it is necessary to extract the corresponding record using \verb|ncks|\ind{NCO (NetCDF Operators)!ncks}
\begin{verbatim}
...
\intextoption{split\_snapshots} option.  For example, the run above can be changed to
\begin{verbatim}
$ pismr -i foo.nc -y 10000 -o output.nc -save_file snapshots \
        -save_times 1000,1500,2000,5000 -split_snapshots
\end{verbatim}
for this purpose.  This will produce files called \verb|snapshots-|year\verb|.nc|.  This option is generally faster if many snapshots are needed, apparently because of the time necessary to reopen a large file at each snapshot when \verb|-split_snapshots| is not used.  Note that tools like NCO\ind{NCO (NetCDF Operators)!wildcards} and \verb|ncview|\ind{ncview!wildcards} usually behave as desired with wildcards like ``\verb|snapshots-*.nc|''.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\hline
    \textbf{Option} & \textbf{Description} \\
    \hline
    \intextoption{save\_file} [file name] & Specifies the file to save to.\\
    \intextoption{save\_times} [range or list] & Specifies times at which to save snapshots, by either a MATLAB-style range $a:\Delta t:b$ or a comma-separated list. \\
    \intextoption{split\_snapshots} & Separate the snapshot output into files named \texttt{snapshots-}year\texttt{.nc}.  Faster if you are saving more than a dozen or so snapshots. \\
    \hline
  \end{tabular}
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.4\linewidth}p{0.5\linewidth}}\hline
   ...
   \textbf{Option} & \textbf{Description}\\
    \hline
    \intextoption{view\_map} [list of variables] & Turns on map-plane views of one or several variables, see tables \ref{tab:extra-vars}  \\
    \intextoption{view\_slice} [list of variables] & Turns on viewers showing a slice of a 3D field at a certain level above the base of ice\\
    \intextoption{slice\_level} number& Specifies the level for the previous option\\
    \intextoption{viewer\_size} number & desired viewer size, in pixels\\
    \intextoption{view\_sounding} [list of variables] &Turns on sounding viewers showing values in a column\\
...
\begin{center}
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\hline
    ...
    \texttt{log\_nuH} & log base ten of the vertically-averaged effective viscosity times thickness on the regular grid. Only meaningful in ice streams and shelves.\\
    \intextoption{ksp\_monitor\_draw} & Iteration monitor for the Krylov subspace routines (KSP) in PETSc. Shows norm of residual versus iteration number.\\
    \normalsize
  \end{tabular}
\end{center}
...
\verb|pcctest|\ind{PISM!pcctest} allows one to visualize climate inputs without
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.1\linewidth}p{0.8\linewidth}}\hline
    ...
    \intextoption{ca} &chooses the constant atmosphere coupler (\verb|PISMConstAtmosCoupler|);\\
    \intextoption{sma} & chooses the PDD atmosphere coupler (\verb|PISMPDDCoupler|);\\
    \intextoption{co} & chooses the constant ocean coupler (\verb|PISMConstOceanCoupler|);\\
    ...
    \hline
 \end{tabular}
 \label{tab:pcctest}\ind{\texttt{pcctest} command-line options}
\end{table}
...
surface mass balance \verb|acab|.  As a demonstration here we use an NCO\ind{NCO (NetCDF Operators)!ncrename} (subsection 
...
Such ``regridding'' is done using the \intextoption{regrid\_file} and \intextoption{regrid\_vars} commands as in this example:\ind{pisms}
...
\intextoption{regrid\_file}.  Only model state variables are regriddable, while climate and boundary data generally are not explicitly regriddable.  (Bootstrapping, however, allows the same general interpolation as this explicit regrid.)

\begin{table}[ht]
\centering
\caption{Regriddable variables.\ind{regrid}\ind{regrid\_vars}  Use \texttt{-regrid\_vars} with these names.}\label{tab:regridvar}
\begin{tabular}{ll}\hline
\textbf{Name} & \textbf{Description}\\ \hline
\texttt{age} & age of ice\\
\texttt{bwat} & effective thickness of subglacial melt water \\
\texttt{dbdt} & bedrock uplift rate \\
\texttt{litho\_temp} & lithosphere (bedrock) temperature \\
...
Here is another example: suppose you have an output of a PISM run on a fairly coarse grid (stored in \verb|foo.nc|) and you want to continue this run on a finer grid. This can be done using \intextoption{regrid\_file} along with \intextoption{boot\_from}:
\begin{verbatim}
...
\noindent This run gets a Unix process id,\ind{signals!the pid}, which we assume is ``8920''.  (Get it using \verb|ps| or \verb|pgrep|.)  If we want to observe the run without stopping it we send the \verb|USR1| signal:\ind{signals!usr1}
...
FIXME: the python scripts all take \texttt{fill\_missing.py --help}
...
\begin{table}[h]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.65\linewidth}}\\
    ...
    \scripthead{util/pism\_config\_editor.py} & \\
    \scripthead{util/pism\_matlab.m} & \\
   \hline
  \end{tabular}
