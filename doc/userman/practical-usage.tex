...
You can choose one of the three supported output orders using the \intextoption{o_order} option with one of \texttt{xyz}, \texttt{yxz}, and \texttt{zyx} as the argument.
...
Note also that for each variable named \dots\texttt{_flux}, positive values mean ice
sheet mass gain.

Omitting the \texttt{-ts_vars} option makes PISM save all available variables, as listed in tables \ref{tab:time-series-1}~and~\ref{tab:time-series-2}.  Because scalar time-series take very little storage space, compared to spatially-varying data, this is usually a reasonable choice.
...
This allows us to compute areas and volumes (see tables \ref{tab:time-series-1}~and~\ref{tab:time-series-2})
more accurately.
...
\begin{table}[ht]
 \centering
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\toprule
    ...
    \listopt{ts_vars} & Comma-separated list of variables, see tables~\ref{tab:time-series-1}~and~\ref{tab:time-series-2}. Omitting this option is equivalent to listing the default variables.\\
...
\begin{table}[ht]
 ...
 \begin{tabular}{p{0.4\linewidth}p{0.1\linewidth}p{0.4\linewidth}}
    ...
    \midrule
    \texttt{basal_ice_flux} & \textsl{kg $s^{-1}$} &  total basal ice flux \\
    ...
    \texttt{dimassdt} & \textsl{kg $s^{-1}$} &  total ice mass rate of change \\
    \texttt{discharge_flux} & \textsl{kg $s^{-1}$} &  discharge (calving and icebergs) flux \\
    \texttt{divoldt} & \textsl{$m^{3}$ $s^{-1}$} &  total ice volume rate of change \\
    \texttt{dt} & \textsl{second} &  mass continuity time step \\
    \texttt{float_kill_flux} & \textsl{kg $s^{-1}$} &  \texttt{-float_kill} flux \\
...
\begin{table}[ht]
 ...
 \begin{tabular}{p{0.4\linewidth}p{0.1\linewidth}p{0.4\linewidth}}
    ...
    \texttt{max_diffusivity} & \textsl{$m^{2}$ $s^{-1}$} &  maximum diffusivity \\
    \texttt{nonneg_rule_flux} & \textsl{kg $s^{-1}$} &  \emph{numerical} ice flux resulting from enforcing the $\mathrm{thk} \ge 0$ rule \\
    \texttt{ocean_kill_flux} & \textsl{kg $s^{-1}$} &  \texttt{-ocean_kill} flux \\
    \texttt{slvol} & \textsl{m} &  total sea-level relevant ice \textbf{in sea-level equivalent} \\
    \texttt{sub_shelf_ice_flux} & \textsl{kg $s^{-1}$} &  total sub-shelf ice flux \\
    \texttt{surface_ice_flux} & \textsl{kg $s^{-1}$} &  total over ice domain of top surface ice mass flux \\
    \bottomrule
  \end{tabular}
...
\end{verbatim} %$
will save the rate of change of the ice thickness every month for 100 years. PISM uses the Gregorian calendar to compute lengths of months and years.

Times given using \texttt{-extra_times} are interpreted as endpoints of reporting intervals. This implies that \texttt{0:1:10} will produce 10 records and not 11.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.15\linewidth}p{0.15\linewidth}p{0.6\linewidth}}
    ...
    \texttt{uvel} & \textsl{m $s^{-1}$} &  horizontal velocity of ice in the X direction \\
    \texttt{vvel} & \textsl{m $s^{-1}$} &  horizontal velocity of ice in the Y direction \\
    \texttt{wvel} & \textsl{m $s^{-1}$} &  vertical velocity of ice, relative to geoid \\
    \texttt{wvel_rel} & \textsl{m $s^{-1}$} &  vertical velocity of ice, relative to base of ice directly below \\
    \bottomrule
  \end{tabular}
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.15\linewidth}p{0.15\linewidth}p{0.6\linewidth}}
    ...
    \textbf{Name} & \textbf{Units} & \textbf{Description} \\
    \midrule
    \texttt{climatic_mass_balance} & \textsl{m $s^{-1}$} & ice-equivalent surface mass balance (accumulation/ablation) rate \\
    ...
    \texttt{bmelt} & \textsl{m $s^{-1}$} & ice basal melt rate in ice thickness per time \\
    \texttt{bwat} & \textsl{m} & effective thickness of subglacial melt water \\
    \texttt{cbar} & \textsl{m $s^{-1}$} &  magnitude of vertically-integrated horizontal velocity of ice \\
    \texttt{cbase} & \textsl{m $s^{-1}$} &  magnitude of horizontal velocity of ice at base of ice \\
    \texttt{cell_area} & \textsl{$m^{2}$} & cell areas \\
    \texttt{cflx} & \textsl{$m^{2}$ $s^{-1}$} &  magnitude of vertically-integrated horizontal flux of ice \\
    \texttt{csurf} & \textsl{m $s^{-1}$} &  magnitude of horizontal velocity of ice at ice surface \\
    \texttt{dHdt} & \textsl{m $s^{-1}$} &  ice thickness rate of change \\
    \texttt{dbdt} & \textsl{m $s^{-1}$} & bedrock uplift rate \\
    \texttt{diffusivity} & \textsl{$m^{2}$ $s^{-1}$} &  diffusivity of SIA mass continuity equation \\
    \texttt{enthalpybase} & \textsl{J $kg^{-1}$} &  ice enthalpy at the base of ice \\
    \texttt{enthalpysurf} & \textsl{J $kg^{-1}$} &  ice enthalpy at 1m below the ice surface \\
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.15\linewidth}p{0.15\linewidth}p{0.6\linewidth}}
    ...
    \texttt{shelfbmassflux} & \textsl{m $s^{-1}$} & ice mass flux from ice shelf base (positive flux is loss from ice shelf) \\
    ...
    \texttt{velbar} & \textsl{m $s^{-1}$} &  vertical mean of horizontal ice velocity in the X and Y directions \\
    \texttt{velbase} & \textsl{m $s^{-1}$} &  horizontal velocity of ice at the base of ice in the X and Y directions\\
    \texttt{velsurf} & \textsl{m $s^{-1}$} &  horizontal velocity of ice at ice surface in the X and Y directions\\
    \texttt{wvelbase} & \textsl{m $s^{-1}$} &  vertical velocity of ice at the base of ice, relative to the geoid \\
    \texttt{wvelsurf} & \textsl{m $s^{-1}$} &  vertical velocity of ice at ice surface, relative to the geoid \\
  \bottomrule
  \end{tabular}
