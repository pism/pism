...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.15\linewidth}p{0.15\linewidth}p{0.6\linewidth}}
    ...
    \texttt{flux_divergence} & \textsl{1 / year} &  flux divergence \\
...
\end{verbatim}
\normalsize
That is, we have the total ice volume, total ice area, maximum diffusivity of the SIA mass conservation, and maximum horizontal velocity ($\max(\max(|u|), \max(|v|))$).  A ``\texttt{U}'' line near the beginning of the standard output indicates the units.

The characters ``\texttt{v\$Eh}'' at the beginning of the flag line give a very terse description of which physical processes were modeled in that time step.  Both the SIA and SSA stress balances were solved, and the latter, which is the most computationally-expensive, describes its nonlinear iterations.
