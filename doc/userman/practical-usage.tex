...
\label{sec:stdout}
\optsection{Input and output!standard out}
...
\texttt{pclimate}\index{PISM!pclimate} allows one to visualize climate inputs without
...
FIXME: this section needs an overhaul!

\texttt{pclimate} takes options listed in table \ref{tab:pclimate}.
...
$ pclimate -i state.nc -ca -ys 0.0 -ye 2.5 -dt 0.1 -o camovie.nc
\end{verbatim}
...
surface mass balance \texttt{acab}.  As a demonstration here we use an NCO\index{NCO (NetCDF Operators)!ncrename} (subsection 
\ref{subsect:nctoolsintro}) to change a variable name in \texttt{state.nc}.  Then we run \texttt{pclimate}
...
$ pclimate -i restate.nc -pdd -ys 0.0 -ye 2.5 -dt 0.1 -o pddmovie.nc
\end{verbatim}
...
\listopt{regrid_vars} commands as in this example:\index{pisms}
...
\begin{table}[ht]
\centering
\caption{Regriddable variables.\index{regrid}\index{regrid_vars}  Use \texttt{-regrid_vars} with these names.}\label{tab:regridvar}
...
\noindent This run gets a Unix process id,\index{signals!the pid}, which we assume is ``8920''.  (Get it using \texttt{ps} or \texttt{pgrep}.)  If we want to observe the run without stopping it we send the \texttt{USR1} signal:\index{signals!usr1}
