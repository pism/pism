...
\renewcommand{\optindexsection}{Input and output}
\optseealso{Bootstrapping}
\optseealso{Regridding}

At first glance PISM is a program that reads one or more NetCDF file and
then produces at least one output file. Table \ref{tab:input-output-options} summarizes
command-line options controlling input and output.
\begin{table}
  \centering
  \begin{tabular}{lp{0.6\linewidth}}
    ...
    \\\midrule
    \fileopt{i} & Chooses a PISM output file to restart from.\\
    ...
    \\\bottomrule
 \end{tabular}
  \caption{Basic input and output options}
  \label{tab:input-output-options}
\end{table}

Please see configuration variables \texttt{output_medium} and
\texttt{output_big} for lists of variables.

The following sections describe more input and output options.

\subsubsection{PISM's Standard output}
\label{sec:stdout}
\renewcommand{\optindexsection}{Input and output!standard out}

Please see table \ref{tab:stdout} for a summary of PISM's options controlling
...
\begin{table}
  ...
  \begin{tabular}{lp{0.7\linewidth}}
    ...
    \textbf{Option} & \textbf{Description}
    \\\midrule
    \intextoption{verbose} & Increased verbosity of standard output.  Usually given with an integer level; 0,1,2,3,4,5 are allowed.  At the extremes, \texttt{-verbose 0} produces no stdout at all, \texttt{-verbose 1} only prints only warnings and a few high priority messages, and \texttt{-verbose 5} spews a lot of usually-undesirable stuff.  If given without argument (``\texttt{-verbose}'') then sets level 3, while ``\texttt{-verbose 2}'' is the default.\\
   \intextoption{v} &   Show version number of PETSc.\\
   ...
   \intextoption{display} & The option \texttt{-display :0} seems to frequently be needed to let PETSc use Xwindows when running multiple processes.  \emph{It must be given as a \emph{final} option, after all the others.}\\
  \intextoption{help} & Gives PISM help message and then a brief description of many PETSc options.\\
 \intextoption{info} & Gives excessive information about PETSc operations during run.  Option \texttt{-verbose} for PISM (above) is generally more useful, except possibly for debugging.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}\toprule
    ...
    \intextoption{extra_split} & Save to separate files, similar to \texttt{-split_snapshots}\\
...
\texttt{-split_snapshots} option.  For example, the run above can be changed to
\begin{verbatim}
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.4\linewidth}p{0.5\linewidth}}\toprule
   ...
   \textbf{Option} & \textbf{Description}\\
    ...
    \txtopt{slice_level}{(meters)}& Specifies the level for the previous option\\
...
\listopt{regrid_vars} commands as in this example:\ind{pisms}
...
Here is another example: suppose you have an output of a PISM run on a fairly coarse grid (stored in \texttt{foo.nc}) and you want to continue this run on a finer grid. This can be done using \texttt{-regrid_file} along with \texttt{-boot_from}:
\begin{verbatim}
...
\renewcommand{\optindexsection}{Adaptive time-stepping}
...
% LocalWords:  NetCDF Regridding emacs
