...
\section{Example: Validating PISM as a flow model for the Ross ice shelf}\label{sect:ross} \ind{PISM!running the EISMINT Ross ice shelf intercomparison in}\ind{Antarctic ice sheet!Ross ice shelf} \ind{EISMINT!intercomparison of Ross ice shelf models} \ind{PISM!validation of ice shelf model} \ind{Ross ice shelf}
\renewcommand{\optindexsection}{EISMINT-Ross}
...
There are many variations on this basic ``\verb|pross|''\pismoptionindex{ross} run above.  First of all one can get more information during the run by adding diagnostic viewers and a more complete (verbose) report to standard out:\ind{pross} \pismoptionindex{ssaBC}\pismoptionindex{pause} \pismoptionindex{ssa}
...
\subsubsection*{Comparison to RIGGS data}  The file \verb|riggs_clean.dat| is a cleaned-up version of the original RIGGS\ind{RIGGS} data \cite{RIGGS1, RIGGS2}.  (See \texttt{README} for more explanation on this RIGGS data.)  To convert this data to a NetCDF file, as needed next, do
