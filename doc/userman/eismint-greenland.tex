...
\section{Example: Modeling the Greenland ice sheet}\label{sec:eismint-greenland} \ind{PISM!running the EISMINT Greenland intercomparison}\ind{Greenland ice sheet}\ind{EISMINT!intercomparison of Greenland models} 
\renewcommand{\optindexsection}{EISMINT-Greenland}
...
We do a one model year run using option \verb|-boot_from| to ``bootstrap'' from file \verb|eis_green_smoothed.nc|.  The executable ``\verb|pgrn|'' is special to EISMINT-Greenland:\ind{pgrn}
\begin{verbatim}
...
The script \verb|ssh2.sh| uses options \intextoption{ts\_file} and \intextoption{save\_file} to do the run with ``snapshots'' saved every 10000 model years, and the volume saved every 100 model years.  So out discussion will now assume that the user \emph{actually} did this to completion:
...
A final ``greenhouse warming'' experiment ``GWL3'' (option \intextoption{gwl3}) is described in the EISMINT-Greenland \cite{RitzEISMINT}.  It runs for 500 years with the temperature increasing by $0.035^\circ C/$year for the first 80 years, then at a rate of $0.0017^\circ C/$year for the last 420 years:
...
\item The summer peak day is in the right place.  The default for this value is
  ...
  important, the peak day can be changed using the \texttt{-pdd\_summer\_peak\_day}
