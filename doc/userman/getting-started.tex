...
The previous sections were not very ambitious.  You were just getting started!  Now we switch to a serious PISM capability, the ability to change the grid---specifically, to \emph{refine} the grid---at runtime.

Clearly it is possible to do longer model runs (in model years) using a coarse grid like the 20 km grid used first.  On the other hand it is only possible to pick up the detail in high quality data, for instance bed elevation and/or high-resolution climate data, using high resolution.  Furthermore, a 20 or 10 km grid is clearly inadequate to, for example, resolve the flow of the ice sheet through the kind of fjord-like, few-kilometer-wide topographical confinement which occurs under the Jakobshavn isbrae in west Greenland, which is perhaps both the fastest-flowing and most-important outlet glacier in the Greenland ice sheet \cite{Joughinetal08}.

Consider the following (incomplete) command, comparing it to the first PISM command on page \pageref{firstcommand}:
\begin{verbatim}
mpiexec -n 4 pismr -boot_file pism_Greenland_5km_v1.1.nc -Mx 301 -My 561 \
  ...
  ...
\end{verbatim}
Instead of a 20 km grid in the horizontal (\verb|-Mx 76 -My 141|)

FIXME in prev subsection we used regridding, but only to ``bring in'' fields from a run of the same resolution
...
\end{verbatim}
\normalsize
Before you run this script, a

\medskip
\centerline{\large\emph{WARNING:} the 5km run requires \underline{8 Gb of memory at minimum!}\normalsize}

\medskip
\noindent If you try it without that much memory then your machine will ``bog down'' and start using the hard disk for swap space.  The run will no complete and your hard disk will get a lot of wear!  If you have less than 8 Gb, comment out the last three lines of the above script (using the ``\verb|#|'' character at the beginning of the line) so that you only do the 20 km $\to$ 10 km refinement.
