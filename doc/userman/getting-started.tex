...
\end{verbatim}
This run takes about four hours on a 2012-era 4 core laptop.  Supposing you have an available supercomputer, for a shorter duration change ``\texttt{mpiexec -n 4}'' to ``\texttt{mpiexec -n N}'' where \texttt{N} is a substantially larger number, up to 200 or so.

FIXME from here: We see some fields from the result in Figure \ref{fig:secondoutputfiner}.
