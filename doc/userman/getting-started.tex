...
This section has emphasized the ``P'' in PISM, the nontrivial parallelism of splitting the solution of conservation equations, especially the stress balance equations, across processors.  The next section addresses the equally-significant, in terms of answering scientific equations, but easier and more common, parallel technique of multiple model runs with different parameter values in each run.  So we now switch to an ``embarrassingly-parallel'' parameter study mode.


\subsection{Getting serious II: a sliding parameter study}  \label{subsect:paramstudy}

FIXME: separate the assignment of parameters to each run from the mode in which we do these runs (the latter can be either sequential or parallel)
