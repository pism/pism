...
Running \verb|pisms -mismip ...| with options as shown here is perhaps fine for a few particular MISMIP runs, but hardly adequate to complete all 372 runs in the intercomparison.  We need automation, so in \verb|examples/mismip/| there is a shell script \verb|mismip.sh| which helps manage the runs.  There are several nested loops, naturally, but setting variables \verb|MODELRANGE| and \verb|GRIDRANGE| at the start will do some of the job of configuring what you want.  Do ``\verb|./mismip.sh 4 D|'', or similar, with the extra ``\verb|D|'' at the end, to see what \verb|pisms -mismip| calls will happen before actually doing it; ``\verb|D|'' stands for ``debug''.
