...
PISM has two applicable ice dynamics models, and they correspond to \verb|-model 1| and \verb|-model 2| options when running MISMIP.  The first is a pure SSA model which is ``category 2'' in the MISMIP classification.  Model 2 combines SIA and SSA velocities as described in \cite{BBssasliding}, and is ``category 3'' because it resolves ``vertical'' shear (i.e.~using SIA flow).
...
To do an actual MISMIP run, one uses \verb|pisms -mismip| with additional options.  Table \ref{tab:MISMIPoptions} shows special MISMIP options.  A first run is:
...
Using a few processors speeds things up a little, and a little extra output is nice, so next you might try:

\verb|$ mpiexec -n 4 pisms -model 1 -mismip 1a -initials EBU -step 1 \|

\verb|  -Mx 3 -Mz 15 -My 151 -extras|

\noindent This time it takes about 2 minutes on my 8 core machine.  In addition to the same files as before, this time we get an additional ASCII file \verb|EBU1_1a_M1_A1_extras|.  The \verb|_ss| file has the thickness and the \verb|_extras| file has the bed and surface elevation.  This information means we can produce a figure showing the profile for the final steady state:

\verb|$ ./figsMISMIP.py -p EBU1_1a_M1_A1 -e 1|

\noindent The result is Figure \ref{fig:MISMIPmodel1exper1aM1A1}.

The script \verb|makefigs.sh| will apply \verb|figsMISMIP.py| to each MISMIP output in the current directory.
...
For finer grids than mode 1 (i.e.~\verb|-My| $>301$, perhaps), it becomes more critical to set the tolerance parameters for the SSA velocity calculation.  We think that adding options

\verb|-ksp_rtol 1.0e-7 -ssa_rtol 1.0e-3|

\noindent is effective.  [CHECK THIS!]
