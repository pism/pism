...
\date{\today.  $\phantom{|}^\ast$\texttt{ffelb\@@uaf.edu}.  Based on PISM rev \PISMREV\,and PETSC rel \PETSCREL.  \\ Get PISM development version by ``\quad\texttt{svn co http://svn.gna.org/svn/pism/trunk pism-dev}\quad''} 
...
\noindent \textsl{You should have received a copy of the GNU General Public License\index{GPL (\emph{GNU Public License})} along with PISM; see \emph{\texttt{COPYING}} in PISM root directory.   If not, write to the Free Software Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301 USA}
\end{quote}
\vspace{1in}
\normalspacing
...
PISM requires a version of PETSc which is \texttt{\PETSCREL} or later.\footnote{A Debian package for PETSc may exist, but it should only be used if it is version \texttt{\PETSCREL} or later.}   The ``lite'' form of PETSc is fine if you are willing to depend on an internet connection for accessing the PETSc documentation. 
...
\noindent The full text description, available at the above site as \verb|.pdf|, is essential reading.  It is essential both to understand MISMIP results and many of the comments in this subsection.

PISM's version of MISMIP includes an attached ice shelf.  This is an important point because modeling the shelf is theoretically unnecessary in the flow line case considered in MISMIP.  In fact, \cite{SchoofMarine1} shows that the ice shelf needs not be modeled because its only effect is to transfer the force imbalance at the calving front directly to the ice at the grounding line.  But this analysis does not apply to general ice shelves with two horizontal dimensions.  These real ice shelves, which are the modelling goal in PISM, have ``buttressing'' and ``side drag'' and other forces not present in flow line circumstances.  Note PISM was first used to model a two horizontal dimension ice shelf, namely the Ross ice shelf example in section \ref{sect:ross}.  Instead of changing the ice dynamics in PISM to remove the ice shelf for an idealized flow line job, we keep the usual PISM ice shelf model to do MISMIP, and we consider the affect on grounding line dynamics.
...
PISM has two applicable ice dynamics models, the \verb|-model 1| and \verb|-model 2| options in Table \ref{tab:MISMIPoptions}.  Model 1 is a pure SSA model.  It is ``category 2'' in the MISMIP classification.  Model 2 combines SIA and SSA velocities as described in \cite{BBssasliding}, and is ``category 3'' because it resolves ``vertical'' shear (i.e.~using SIA flow).

There are many runs for the complete MISMIP, namely $62$ runs for each model and grid choice.  As there are two models and three (suggested) grid choices, a full suite is $6 \times 62 = 372$ runs.  As usual, however, the finest grid accounts for all the run time.  ``Grid mode 2'' in the MISMIP description is 1500 grid spaces in the flow line direction, a very fine grid which imposes severe time-step restrictions on the explicit methods in PISM.\footnote{For the intercomparison submission in Sept.~2008 we chose to only submit runs for a grid with 150 (``grid mode 1'') and 600 (``grid mode 3'') grid spaces in the flow line direction.  This was still 248 runs to submit, with 3 files each for a total of 744 text files.}
...
\verb|-extras| &  & if this flag is set then, once steady state is reached, surface \\
               &  & and bed elevation are written to an extra file with name ending \\
               &  & ``\verb|_extras|''; script \verb|figsMISMIP.py| can read this \\
...
\verb|-model| & 1 & model 1 is SSA only and model 2 combines SIA and SSA velocities; \\
              &   & see text \\
\verb|-steady_atol| & \verb|1.0e-4| & m/a; tolerance for steady state; criterion is \\
                    & & $\|dH/dt\|_\infty <$(\verb|steady_atol| value) \\
\verb|-step| & 1 & allowed values are $\{1,\dots,9\}$ for experiment 1, $\{1,\dots,13\}$ for \\
             &  & exper.~2, and $\{1,\dots,15\}$ for exper.~3; note typo in table 6 in  \\
             &  & \verb|mismip_4.pdf|: ``step 16'' is mentioned but no ``step 15'' \\
\hline
\verb|-My| &  & this option gets additional interpretation: \verb|-My 151| is grid mode 1 \\
           &  & so files have ``\verb|_M1_|''; \verb|-My 1501| is grid mode 2 (``\verb|_M2_|''); \\
           &  & any other value is ``\verb|_M3_|'' \\
\verb|-ssa_min_thk| & 5 & m; calving-front boundary condition is dealt with by  \\
                    &   & extending the ice beyond $|y|=1600$ km with a shelf  \\
                    &   & of this thickness; larger values may compensate for  \\
                    &   & excess floating flux (below)  \\
\hline\normalsize
\end{tabular}\end{table}
...
\verb|$ pisms -mismip 1a -initials EBU -model 1 -step 1 -Mx 3 -Mz 15 -My 151|

\noindent This will produce ASCII files \verb|EBU1_1a_M1_A1_t|, \verb|EBU1_1a_M1_A1_ss|, and \verb|EBU1_1a_M1_A1_f|, as well as a standard NetCDF output file \verb|EBU1_1a_M1_A1.nc|, in about 3 minutes.

Using a few processors speeds things up a little, and a little extra output is nice, so re-doing it:

\verb|$ mpiexec -n 4 pisms -model 1 -mismip 1a -initials EBU -step 1 \|

\verb|  -Mx 3 -Mz 15 -My 151 -extras|

\noindent This run takes about 2 minutes on an 8 core machine.  In addition to the same files as before, now we get an additional ASCII file \verb|EBU1_1a_M1_A1_extras|.  The \verb|_ss| file with thickness and the \verb|_extras| file with bed and surface elevation allow us to produce a figure showing the profile for the final steady state, Figure \ref{fig:MISMIPmodel1exper1aM1A1}:

\verb|$ ./figsMISMIP.py -p EBU1_1a_M1_A1 -e 1|
...
\verb|$ mpiexec -n 4 pisms -model 1 -mismip 1a -initials EBU -extras -step 2 \|

\verb|    -if EBU1_1a_M1_A1.nc|

\noindent The result is \verb|EBU1_1a_M1_A2_t|, etc.

Running \verb|pisms -mismip ...| with options as shown here is o.k.~for a few examples.  The script \verb|mismip.sh| automates the process for the hundreds of runs in the intercomparison.  It has several nested loops, naturally, but setting variables \verb|MODELRANGE| and \verb|GRIDRANGE| at the top of the file will mostly configure what you want.  Do ``\verb|./mismip.sh 4 D|'', or similar, to see what PISM calls will happen \emph{before} actually actually starting any; ``\verb|D|'' stands for ``debug''.

\verb|mismip.sh| illustrates how to reverse the order of steps in experiments 2a and 2b.

For significantly finer grids than mode 1, it becomes more critical to set the tolerance parameters for the SSA velocity calculation.  We think that adding options

\verb|-ksp_rtol 1.0e-7 -ssa_rtol 1.0e-3|

\noindent are effective.  [CHECK THIS!]

The implementation of MISMIP in PISM conforms closely to the intercomparison description.  However, that document specifies
\begin{quotation}
...
\end{quotation}
as a standard for ``steady state''.  Because the grounding line undergoes only discrete jumps on the grid, because it shows great stability, and because in two horizontal dimensions there is no well-defined ``rate of change of grounding line position'' which would be generally implementable in PISM, we do not check the grounding line movement criterion.  Steady state is achieved if the ice thickness criterion holds.  However, we report enough information, in the required ASCII files with names ending in \verb|_t|, to compute a grounding line rate (in this flow line case).

\begin{figure}[ht]
\includegraphics[width=4.0in,keepaspectratio=true]{figs/SM_1a_A1}
\caption{Analytical profile for steady state of experiment 1a, grid mode 1, step 1, from theory in \cite{SchoofMarine1}.  This is a boundary layer asymptotic matching result, but not the exact solution to the equations solved numerically by PISM.  The grounding line is at $1052.5$ km.  Compare Figure \ref{fig:MISMIPmodel1exper1aM1A1}.}
\label{fig:SMexper1aM1A1}
\end{figure}

There is an additional script \verb|solverSM.py| which computes the profile from the Schoof \cite{SchoofMarine1} asymptotic-matching boundary layer theory.  This script is a Python translation (using \verb|scipy| and \verb|pylab|) of the \Matlab codes in \url{http://homepages.ulb.ac.be/~fpattyn/mismip/MISMIP_distribution.tar}.  For example,

\verb|$ ./solverSM.py -e 1 --sliding=a -s 1|

\noindent produces Figure \ref{fig:SMexper1aM1A1}.

We see immediately that the PISM result does not put the grounding line in the same location as Schoof's boundary layer theory.  We believe the problem is with PISM's numerical solution, not with Schoof's boundary layer theory.  More evidence that the problem is numerical can be found by looking in more detail at the PISM result.  Using \verb|ncview| to examine the diagnostic flux variable \verb|cflx| (magnitude) in the file \verb|EBU1_1a_M1_A1.nc| produced earlier, we see Figure \ref{fig:cflx1aM1A1}.  \emph{This is a problem with PISM} because the flux should be a linear function.  Indeed, at steady state the flux $\bq$ solves $a=\Div\bq$ where $a$ is the accumulation rate.  In MISMIP, $a = 0.3$ m/a.  That is, it is constant.  At steady state we conclude that $|\bq| = a x$ if $x$ is the flowline variable.
...
We can learn more about the problem.  It turns out that the numerical flaw in the flux keeps the grounding line from moving properly.  On the other hand, the correct location for the grounding line is a steady state for the PISM numerical result; the problem is that the numerical result doesn't move the groundingline to its correct location if the ice sheet and shelf are initially 10 m as specified in MISMIP.

In fact, \verb|solverSM.py| has an option which allows the asymptotic-matching thickness result from the \cite{SchoofMarine1} theory to be written into a NetCDF file.  We can then initialize the PISM thickness from it, using \verb|-regrid| (section \ref{sect:usage}).  Furthermore we can do this at higher resolution (6km instead of 24 km):

\verb|$ ./solverSM.py  --My=601 -e 1 --sliding=a -s 1 -n SMthk_1a_M3_A1.nc|

\verb|$ mpiexec -n 8 pisms -model 1 -mismip 1a -initials EBU -extras \|

\verb|    -ksp_rtol 1.0e-7 -step 1 -skip 10 -Mx 3 -Mz 15 -My 601 \|

\verb|    -regrid SMthk_1a_M3_A1.nc -regrid_vars H|

\noindent The result, Figure \ref{fig:MISMIPmodel1exper1aM3A1FROMSM}, is satisfactory.  Inspection of the flux figure for this new run shows a smaller, but still significant, jump in flux to a too-high value for the floating shelf.  This too-high value of flux reflects the same fact as shown in the profile Figure  \ref{fig:MISMIPmodel1exper1aM3A1FROMSM}, namely, that the ice shelf is flowing, and therefore thinning, too fast.

\begin{figure}[ht]
\includegraphics[width=4.0in,keepaspectratio=true]{figs/profile_EBU1_1a_M3_A1}
\caption{A marine ice sheet profile in the MISMIP intercomparison; PISM model 1, experiment 1a, grid mode 3, step 1, but on a run started from the corresponding asymptotic-matching result.}
...
\verb|$ pismd -ross -bif ross.nc -ssa -ssaBC ross.nc \|

\verb|     -Mx 147 -My 147 -Mz 3 -Lz 1e3 -d cnmu -verbose -pause 10|

\noindent Secondly one might want to do the run in parallel and do it on a finer grid.  For example,

\verb|$ mpiexec -n 4 pismd -ross -bif ross.nc -ssa -ssaBC ross.nc \|

\verb|     -Mx 201 -My 201 -Mz 3 -Lz 1e3|
...
\optdef{ssa\und min\und thk}{5}  When ice free ocean is present and we are solving the SSA for the velocity in ice shelves, we extend a virtual ice shelf everywhere with this thickness.  This takes the place of a principled calving front force imbalance boundary condition.  Units of m.
