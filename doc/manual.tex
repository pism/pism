...
\subsection{Visualizing the climate inputs, without ice dynamics} 

\newcommand{\pcctestopt}[1]{\par\noindent\texttt{-#1}\quad\quad}

Executable \verb|pcctest|\index{PISM!pcctest} allows one to visualize climate
inputs without using the ice dynamics core of PISM. It takes the following
options:

\medskip
\begin{itemize}
\item[\texttt{-i}] specifies an input file, which has to be produced by
  PISM;
\item[\texttt{-o}] sets the output file name;
\item[\texttt{-ca}] chooses the constant atmosphere coupler;
\item[\texttt{-pdd}] chooses the PDD atmosphere coupler;
\item[\texttt{-co}] chooses the constant ocean coupler;
\item[\texttt{-ys}] sets the start time, in years;
\item[\texttt{-ye}] sets the final time, in years;
\item[\texttt{-dt}] sets the interval between saved climate snapshots.
\end{itemize}

\bigskip\noindent
Example usage:

First, set up an ice sheet state file:
\begin{verbatim}
$ mpiexec -n 2 pisms -eisII A -y 10000 -o state.nc
\end{verbatim}
next line extracts \verb|acab| and \verb|artm| from \verb|state.nc|, because
they are stored there by EISMINT II choices, and initializes
PISMConstAtmosCoupler; ``computes'', though no actual computation, surface mass
balance and surface temp at \verb|ys:dt:ye| and saves these in
\verb|camovie.nc|:
\begin{verbatim}
...
\bigskip
Similar for PISMPDDCoupler, but expects \verb|accum|, so we change
the name before running:
\begin{verbatim}
...
See section \ref{sect:green} for another example of using \verb|pcctest|.
...
Assuming that \verb|green20km_y1.nc| was produces by the run above (see section
...
\begin{verbatim}
$ ncrename -v acab,accum green20km_y1.nc foo.nc
...
The time-evolution of \verb|artm| and \verb|snowaccum| is quite boring, the
other two were used to create figure \ref{fig:pddseries}.

It shows the time-series of the accumulation rate (top graph) and the snow
...
\begin{enumerate}
\item the summer peak day is in the right place: recall that it should be
  on August 1 (day $243$)\footnote{the default; can be changed using the
    \texttt{-pdd\und summer\und peak\und day} option}, i.e. at approximately
  $243/365 \simeq 0.66$ years;

\item lows of the accumulation rate correspond to positive degree-days (highs
  of the temperature graph). Note that the show temperature graph does
  not show daily variations, so even though it has the maximum of about $266$
  Kelvin, the \emph{actual} snow temperature can be above freezing. (Recall
  that it is modeled as a standard normal process \emph{around} the yearly cycle.)
...
\begin{figure}[ht]
  ...
  \caption{Time series of the accumulation (ablation) rate and snow temperature. Map view on the right shows the location probed.}
  \label{fig:pddseries}
\end{figure}
