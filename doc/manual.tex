...
\subsection{Handling NetCDF files}  At a trivial level, PISM is just a program which takes a NetCDF file as input, does some computation, and produces a NetCDF file as output.  (An exception is that \verb|pisms| knows formulas to initialize EISMINT II experiments, for example.)  The user is in charge of creating NetCDF input files containing data on ice sheets worth modeling, and extracting some meaning from the NetCDF output files.
...
The allowed formats for a bootstrapping file are relatively simple to describe. 
\begin{enumerate}
...
\item All three-dimensional variables (depending on \verb|t,x,y,z| or \verb|t,x,y,z|) will be ignored in bootstrapping.
...
\subsection{Schoof's plastic till free boundary problem for ice streams}  The remainder of this section gives an incomplete view, at best, of the dynamics in PISM because the best model for whole ice sheets, using the SSA as a sliding law for the SIA is undocumented except for the preprint \cite{BBssasliding}.
...
$ pismd -ross -boot_from ross.nc -ssaBC ross.nc -Mx 147 -My 147 -Lz 1000 -Mz 11 \
        -ssa -ice_type custom -ice_custom_hardness 1.8e8 -o ross_out_1p8.nc     \
        -ssa_rtol 1e-6 -ksp_rtol 1e-10
\end{verbatim}
