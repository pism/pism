...
Section \ref{sect:green} gives an example of actual usage on the bed elevations in the EISMINT-Greenland data.  Here is a hypothetical example usage, of filling the missing values in the two variables \verb|topg| and \verb|artm|, using a convergence tolerance of $10^{-4}$, on data in the NetCDF file \verb|data.nc|:

\verb|fill_missing.py -i data.nc -v topg,artm --eps=1.0e-4 -o data_smoothed.nc|
...
\scriptoptdef{f}{file}{foo.nc}  The full name of the NetCDF file from which to read variables for filling.

\scriptoptdef{o}{out\und file}{fill\und missing\und out.nc}  Full name of output file.

\scriptoptdef{v}{variables}{blah}  Comma separated list of variables for which to fill missing values.  Note that each of the listed variables must have an attribute named \verb|missing_value|.
