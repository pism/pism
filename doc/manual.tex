...
\subsection{Grid}
\label{subsect:grid}
The PISM grid\index{PISM!grid} covering the computational box is equally spaced in horizontal ($x$ and $y$) directions. Vertical spacing is equal by default but can be changed to quadratic or Chebyshev using \intextoption{quadZ} and \intextoption{chebZ} options, respectively.

If \intextoption{chebZ} option is used, the vertical grid in the ice is Chebyshev spaced; it is very fine near the base of ice and coarser near the top of the computational box. If \intextoption{quadZ} is used, the spacing is a quadratic function; the intent is that the spacing is smaller near the base than near the top, but that the effect is less extreme than in the Chebyshev case. Please see the documentation of \verb|IceGrid::compute_vertical_levels()| in PISM's class browser for more info and formulas defining the spacing.
...
In this section we give an extended example of how to use PISM to model the Greenland ice sheet.  We use somewhat stale data from the 1990s ice sheet modelling intercomparison known as EISMINT-Greenland \cite{HuybrechtsEISMINT,RitzEISMINT}\footnote{EISMINT-Greenland is probably supposed to have bedrock thermal!  There is no mention in the text \cite{RitzEISMINT}, but there are bedrock thermal parameter values mentioned.  Easy to fix \dots}.  Though based on old data, EISMINT-Greenland serves as an excellent tutorial example.
