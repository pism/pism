...
See the \emph{Installation Manual} to install PISM.  Once installed, the \verb|pgrn|, \verb|pismd|, \verb|pismr|, \verb|pisms|, and \verb|pismv| executables will be in the \verb|bin| subdirectory of the PISM root directory.  This \emph{User's Manual} addresses the use of all five executables, but this \emph{Getting started} section only uses \verb|pisms|.
...
\verb|VisIt| & \href{http://visit.llnl.gov}{\t{visit.llnl.gov}} & advanced parallel visualization \\
...
\subsection{Principles}\index{PISM!principles}
\begin{enumerate}
...
\item climate inputs meet ice dynamics by a well-defined interface (in the source code),
\item the availability of different ice dynamics models, from a hierarchy of simplifying assumptions, is more important than any particular level of simplification,
\item everything available diagnostically (instantaneously) is available for evolution.
\end{enumerate}

\noindent We can not claim to follow these principles perfectly.  We hope, however, that the user will see our commitment to them in the process of becoming familiar with PISM and this \emph{Manual}.  The current section, in part, fleshes out these principles in no particular order.  Mostly, however, our goal here is to give an overview of ice dynamics models in PISM.
...
By default, PISM does not numerically solve the SSA when using the executables \verb|pismr|\index{pismr}, \verb|pismd|\index{pismd}, or \verb|pisms|\index{pisms}.  The user must add the flag \intextoption{ssa}.  Use of the executable \verb|pismv| for verifying the SSA balance of momentum equations is addressed in section \ref{sect:verif}.


\subsection{A hierarchy of simplifying assumptions for ice flow}\index{PISM!hierarchy of simplifying assumptions}   Table \ref{tab:modelhierarchy} describes this hierarchy, which applies to grounded ice.  (It makes sense to also talk about ice shelf model hierarchy, and about a special hierarchy of models for the transition from grounded to floating ice, but this would take us too far afield; not shown.)  Essentially these simplifications relate to shallowness.
...
\textbf{Model} & \textbf{Assumptions} & \textbf{Needed data} & \textbf{Ref.} \\ \hline
\emph{balance velocities} & \emph{steady state, and ice flows} & surf.~mass balance, ice & \cite{JoughinetalGrBal97} \\
 & \quad \emph{down the surface gradient} & \quad thickness, bed elevation & \\ \hline
isothermal SIA & non-sliding lubrication flow with fixed & (SAME) & \cite{EISMINT96,BLKCB} \\
 & \quad softness &  &  \\ \hline
thermomechanically & non-sliding lubrication flow with & PLUS surf.~temperature, & \cite{BBL} \\
\quad -coupled SIA & \quad temperature-dependent softness & \quad geothermal flux &  \\ \hline
SIA + SSA hybrid & SSA as a sliding law for the  & PLUS basal layer (till) & \cite{BBssasliding} \\
  & \quad thermomechanically-coupled SIA & \quad strength &  \\ \hline
\emph{Blatter-Pattyn} & \emph{``higher-order''; some bridging stresses} & (SAME) & \cite{Blatter,Pattyn03} \\ \hline \hline
full Stokes & not planned; ice is a fluid & (SAME) &  \\ \hline
\end{tabular}
\normalsize
\end{table}
...
\subsection{Evolutionary SIA ice sheet versus diagnostic SSA ice shelf modeling} \label{subsect:basicmodes}    FIXME:  THIS SUBSECTION IS TOO SPECIFIC; SOMETHING MOVED TO ``Usage''?
...
Sections \ref{sect:green} and \ref{sect:ross} illustrate the two traditional ice flow models, namely a time-stepping SIA-only model for the Greenland ice sheet and a diagnostic SSA model for the Ross ice shelf.
...
\subsection{Separation of climate inputs from ice dynamics}\label{subsect:separateclimate}  FIXME:  SHOW CARTOON.
