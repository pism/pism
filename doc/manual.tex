...
Within PISM, each grid point in the horizontal grid is either SIA \emph{or} SSA.  Each horizontal grid point corresponds to a whole vertical column of grid points in the three-dimensional grid, so it is perhaps better to say that \emph{each vertical column} of ice, centered at a horizontal grid point, is either SIA or SSA.  When ice is floating (according to the usual floatation criterion \cite{WeisGreveHutter}), the horizontal grid point is automatically marked as SSA.
...
One must generally decide which parts of the grounded ice sheet are modelled by the SIA and which by the SSA.  This is true for the linearly-viscous till model \cite{MacAyeal} but also in cases where Schoof's plastic-till model is only applied to part of the grounded ice sheet.  The rest of this section largely focuses on how the user makes this choice in practice when using PISM.  One way is to specify a \emph{mask}.
...
To demonstrate PISM's implementation of Schoof's model we describe an example based on EISMINT II experiment I.  This particular experiment is documented in \cite{EISIIdescribe}, but experiment I is not discussed in the published EISMINT II intercomparison because the results for experiment I were ``straightforward results with little variability between groups'' \cite{EISMINT00}.  Note that Section \ref{sect:simp} describes using PISM to perform all the EISMINT II experiments documented in \cite{EISIIdescribe}.

Experiment I is identical to experiment A except for having ``trough'' bed topography, shown in Figure \ref{fig:eis2I}.   As specified, experiment I has nothing to do with the SSA equations or plastic till failure, and indeed there is no basal motion at all.  However, we use it to build an ice sheet which ``ought'' to have an ice stream, that is, along the trough.

\begin{figure}[ht]
\includegraphics[height=2.6in,keepaspectratio=true]{figs/eis2Ibed}
%\includegraphics[height=2.6in,keepaspectratio=true]{figs/cbar_eisI}
\caption{Left: Bed elevation for EISMINT II experiment I.  Most of the bed is a plateau at 1000 m, but the trough descends to nearly zero elevation.  Right: The vertically-averaged horizontal velocity at the end of a ????? year run using EISMINT II experiment I boundary conditions, and starting from zero ice.}
\label{fig:eis2I}
\end{figure}
...
The goal is not to see that the error is zero at any stage on the refinement path, or even that the error is small in a predetermined absolute sense, generally.  Rather the goal is to see that the error is trending toward zero, and to measure the rate at which decays.  Knowing the error decay rate allows the modeller to have a sense of how fine a grid is necessary to achieve a small enough numerical error so that the numerical solution can be regarded as a (near) solution to the continuum model.  See \cite{BLKCB,BBL,Roache,Wesseling}.
...
\verb|Q|:\quad Map-plane view of basal driving stress $f_{\text{basal}} = \rho g H |\grad h|$.  Units of kPa.  Note that this quantity is the effective shear stress at the base in the SIA.  It is also the (magnitude of the) driving source term in the SSA, for both ice streams and ice shelves.in Compare to \verb|-d C|, the till yield stress, when using the plastic till ice stream model.
...
The following scripts are found in subdirectory \verb|pism/test/|.

\subsubsection*{\Large{\texttt{series.py}}}  This Python script postprocesses the standard output of \verb|pismr|, \verb|pisms|, or \verb|pgrn| (among others; output from \verb|pismd| and \verb|pismv| are not currently supported).
