...
There is a script that automates the process of checking the steady state condition so that it is not necessary to manually check the changes in volume.  This script is called \verb|ssl_exper.py| and is in the \verb|pism/test| directory.  When run, it will continue, saving states every 10,000 years, until the change in volume, between runs with a specified time interval, is less than a specified criterion.  It uses the \href{http://nco.sourceforge.net/}{NCO} to compare the thicknesses in saved model states.  It can be used with multiple processors by setting the option \verb|-n|. 

For illustration we do:

\verb|$  ssl_exper.py -n 8 -i green20km_Tsteady.nc -t 1 -c 0.01|

\noindent Here we specify that the criterion for steady state is weak so that this illustration takes less computer time.  Namely we ask for only 1\% change (``\verb|-c 0.01|'') between 1000 year runs (``\verb|-t 1|'').  With 8 processors this illustrative run with weak steady state criterion completes in a couple of hours.  The actual EISMINT-Greenland run would be

\verb|$  ssl_exper.py -n 8 -i green20km_Tsteady.nc -t 10 -c 0.0001|

The illustrative run gives this output:
...
In fact that is what happens.  If we run the SSL2 experiment with the specified ``0.01\% change in 10k years'' criterion, then [STATE WHAT HAPPENS AND FINAL VOLUME].  The ice thickness and homologous basal temperature at the end [???WHEN] of the SSL2 run are shown in Figure \ref{fig:ssl2thickThomol}.  The vertically-averaged horizontal velocity is shown in Figure \ref{fig:ssl2cbar}.
...
\caption{Ice thickness (meters; left) and homologous basal temperature (degrees C below 0; right) at end [???WHEN] of EISMINT-Greenland SSL2 run; in temperature graph pressure-melting areas are white.}
\label{fig:ssl2thickThomol}
\end{figure}

\begin{figure}[ht]
\includegraphics[height=3.9in,keepaspectratio=true]{figs/greencbar_SSL2}
\caption{Vertically-averaged horizontal velocity in meters per year at end [???WHEN] of EISMINT-Greenland SSL2 run.}
\label{fig:ssl2cbar}
\end{figure}

One may also give a \verb|--ssl3| option to \verb|ssl_exper.py| to run our version of the EISMINT-Greenland ``SSL3'' experiment \cite{RitzEISMINT}.  (Giving this option to \verb|ssl_exper.py| actually gives a \verb|-ssl3 -gk| option to \verb|pgrn|.)  Our particular interpretation to the SSL3 experiment is: \begin{itemize}
...
\end{itemize}
There is no assertion that these are superior modeling choices, and we give ``SSL3'' this interpretation merely for illustration.  Note that because of the basal sliding, with sliding velocity proportional to the driving shear stress at the base, the volume time series never quite settles down (not shown).
...
\verb|  pgrn -ccl3 -if green_SSL2_100k.nc -dTforcing grid_dT.nc \|

\verb|    -dSLforcing specmap_dSL.nc -ys -249900 -ye 0 -o green20_CCL3_y0|
\smallskip

[SHOW RESULT; give thickness??]
