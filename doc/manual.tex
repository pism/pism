...
\date{\today.  Support by email: \texttt{help\@@pism-docs.org}.  Based on PISM revision \PISMREV\,and PETSC release \PETSCREL.  Get stable version of PISM source code: \\ \centerline{\texttt{svn co http://svn.gna.org/svn/pism/tags/stable0.2 pism0.2} \quad}} 
...
In this section we give an extended example of how to use PISM to model the Greenland ice sheet.  We use somewhat stale data from the 1990s ice sheet modelling intercomparison known as EISMINT-Greenland \cite{HuybrechtsEISMINT,RitzEISMINT}\footnote{EISMINT-Greenland is probably supposed to have a bedrock thermal layer and possibly a bed deformation model.  There is no clear requirement in \cite{RitzEISMINT}, but bedrock thermal parameter values are given.  A bedrock thermal layer is easily added using \t{-Mbz 51}, for example, and a bed deformation model with \intextoption{bed\und def\und lc}.}.  Though based on old data, EISMINT-Greenland serves as an excellent tutorial example.
...
\subsubsection*{Bootstrapping from EISMINT-Greenland data} 
\label{sect:green-bootstrapping}
...
\begin{verbatim}
$ ncrename -v acab,snowaccum green20km_y1.nc foo.nc
