...
\item To build the latest manual from source you will need \LaTeX, but for most purposes the PDF manual at the PISM documentation page, \url{http://www.dms.uaf.edu/~bueler/manual.pdf}, should suffice.  (This is the only use of \LaTeX~in PISM.)
...
\verb|   export PETSC_ARCH=linux-gnu-c-debug|  \qquad  \emph{(or something like that)}

\noindent should be added to your \verb|.profile| or \verb|.bashrc| file; see the PETSc installation page.
...
\item  After \verb|configure.py| finishes, you will need to \verb|make all test| in the PETSc directory and watch the result.  If Xwindows is functional some example viewers will appear.
\end{enumerate}
...
\LaTeX & \url{http://www.latex-project.org/} & \emph{optional} & for building manual from source \\
...
\small\begin{quote}\begin{verbatim}
user@host:~/pism$ mpiexec -n 4 pisms -eisII F -if eisIIF2k.nc -y 198000\
