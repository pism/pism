...
\end{itemize}
For the rest of the discussion we will assume that these three files are in a directory \verb|eisROSS/|, and we will refer to these as the ``EISMINT-Ross'' data.  Note that these data are for a particular rectangular grid with $6.822$km spacing in both $x$ and $y$ directions, but that (as usual) the PISM computational grid is determined by the user.

The first step for using these data, as usual for PISM, is to convert them to a machine readable NetCDF file which includes the metadata needed to use it with PISM.  There is already a script \verb|eis_ross.py| for this purpose in subdirectory \verb|pism/test/|.  It  reads the above three EISMINT-Ross \verb|.dat| files and it creates a NetCDF file \verb|eis_ross.nc|.  It is used this way:

\verb|eis_ross.py --prefix=eisROSS/|

\noindent  The output file can be reconverted to text using \verb|ncdump| or it can be viewed graphically with \verb|ncview|.
