...
This \emph{User's Manual} describes how to download the PISM source code and install it.  It describes how to run it for certain simplified geometry situations and certain verification situations.  It also describes how to use PISM as a modest Greenland ice sheet model or a modest Ross ice shelf model.  And that is all.

Therefore most users will need to go beyond what is described in this \emph{User's Manual} if they are to advance the science of ice sheets.

Though this \emph{User's Manual} is the place to start, there are two additional documents to know about:
\begin{enumerate}
...
\large
\begin{center}
 \emph{WARNING}:  PISM is an ongoing project.  Ice sheet modelling is complicated and not mature (in 2007, anyway).  Please don't trust the results of PISM (or any other ice sheet model, for that matter) without a fair amount of exploration.  Please don't expect all your questions to be answered here.  But please do write to us with questions: 
...
\subsection{Evolution runs versus ``diagnostic'' runs}  The main goal of a numerical ice sheet model like PISM is to be a dynamical system which evolves over time a similarly as possible to the modelled ice sheet.  Such a goal assumes one starts with the right initial conditions and that one has the right climate and other inputs at each time step (i.e.~boundary conditions).  All of this is natural because underlying an ice sheet model like PISM are evolution-in-time partial differential equations.  We will describe this usual time-stepping behavior as an ``evolution run.''

Much of ice sheet, stream, and shelf modelling, however, requires ``diagnostic'' solution only of the partial differential equations which determine the velocity field.  These are the balance of momentum equations for a slowly flowing fluid \cite{Fowler}.  (As explained in the next section, the ``shallow ice approximation'' which underlies the EISMINT II experiments is only one of two forms of the balance of momentum equations solved by PISM.  The other is the ``shallow shelf approximation''.)  On the other hand, in a diagnostic computation the temperature field and basal conditions like the yield stress of the till, for instance, are held fixed.

In summary, the goal of a ``diagnostic run'' is to compute the velocity field.\footnote{In any case, from now on this defines ``diagnostic run'' in this manual.}  On the other hand, the NetCDF model state saved by PISM at the end of an evolution run does not contain the three-dimensional velocity field.  Instead, it contains those variables which are needed to restart the run, especially the geometry (e.g.~thickness and bed elevation) and the ice temperature field.  For these reasons there is a separate executable \verb|pismd| which only does a diagnostic computation of velocity and saves the full three-dimensional velocity field, but which does not do time-stepping.  For example,
...
\noindent then the horizontal velocity field computed in this single time step run is the same as that saved in \verb|eisIIF200k_fullvels.nc|.  

For the above example, one can use the one of the NetCDF Operators (``the NCO'' from now on) to check that the two saved NetCDF files really correspond to the same horizontal velocity field.  In particular  \verb|ncdiff -v cbar,csurf eisIIF200k_plus.nc eisIIF200k_fullvels.nc foo.nc| produces a new NetCDF file \verb|foo.nc| which has the differences of the vertically-averaged and ice surface values of the horizontal velocity.  One can then see that these variables are identically zero in \verb|foo.nc| using \verb|ncview|, for instance.
...
One can also force PISM to save the full velocity field at the end of any time-stepping run by addition the option \verb|-full3Dout|.  Adding this option has the effect of (roughly) doubling the size of the output model state NetCDF file.  (This option is not needed when using \verb|pismd| or \verb|pismv| for tests I though J as, for all of these diagnostic computations, saving the full three-dimensional velocity field is the default behavior.)
...
In any case, one must decide which parts of the grounded ice sheet are modelled by the SIA and which by the SSA.  The rest of this section largely focuses on how the user can make this choice in practice when using PISM.  One way to make the choice is to specify a \emph{mask}.  Alternatively to the mask technique, we also describe how to use a plasticity assumption to determine the region of fast flow through the solution of a free boundary problem.
...
I & velocity computation using SSA eqns and plastic till &  & \cite{SchoofStream} \\
...
\optdefrestrict{test}{A}{pismv}  Choose single character name of verification test.  Current (PISM revision \PISMREV) allowed values are A, B, C, D, E, F, G, H, I, J, L.  See the Verification section.
