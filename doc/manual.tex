...
\subsection{Initialization from a saved model state}  This phrase has a simple meaning in PISM.  If a previous PISM run has saved a NetCDF file using ``\verb|-o|'' then that file will contain complete initial conditions for continuing the run.  In that case the old file can be loaded with ``\verb|-if|''.  Thus the second of these two commands starts PISM by ``(full) initialization from an input file'' (in the language of this manual):

\verb|$ pisms -eisII A -y 100 -o foo|

\verb|$ pisms -eisII A -if foo.nc -y 100 bar|

\noindent Note that simplified-geometry experiments (section \ref{sect:simp}) and verification tests (section \ref{sect:verif}) do not need input files at all.  They initialize from formulas in the source code, but they can be continued from saved model states using \verb|-if|.
...
But most ice sheet modeling done using forward models like PISM would do something like this to get ``reasonable'' temperature and velocity fields within the ice: \begin{enumerate}
\item start from only the more-directly observable quantities of surface elevation, ice thickness, and ice surface temperature,
\item ``bootstrap'' as described in this section, to fill in temperatures at depth, and
\item \emph{either} do a long run holding the current geometry and surface temperature steady,  to evolve toward a ``more physical'' steady state which will have both a temperature field and velocities,
\item \emph{or} do a long run using an additional spatially-imprecise historical record from an ice core or a sea bed core (or both), to add apply forcing to the surface temperature or sea level (for instance), but with the same functional result of filling in a temperature and velocity fields.
\end{enumerate}
...
\verb|$ pgrn -bif eis_green_smoothed.nc -Mx 141 -My 83 -Lz 4000 -Mz 81 -quadZ \|
...
\begin{quote}
\begin{verbatim}
$  pgrn -bif eis_green_smoothed.nc -Mx 141 -My 83 -Lz 4000 -Mz 81 -quadZ -ocean_kill -y 1 -o green20km_y1
...
bootstrapping by PISM default method from file eis_green_smoothed.nc
  ...
  WARNING: vertical  dimension of target computational domain not a subset of
    ...
    or  bdy[6] = -0.0000 < Lz = -0.0000; ALLOWING ONLY 2D REGRIDDING ...
...
removing extra land (Ellesmere Island and Iceland) using EISMINT-Greenland rule
  ...
  [vertical grid spacing in ice not equal: 17.083333 m < dz < 82.916667 m]
  [fine equal spacing used in temperatureStep():  Mz = 236,  dzEQ =    17.02 m]
%ybp SIA SSA  # vgatdh Nr  +STEP
P         YEAR:     ivol   iarea    meltf     thick0     temp0
U        years 10^6_km^3 10^6_km^2 (none)          m         K
 $$$$            $$$$$ $$
S      0.00000:  2.82500  1.6708   0.2071   3042.000  270.5156
 $$$ SIA        v$atdh 0d  +0.18299
S      0.18299:  2.82515  2.2300   0.1683   3041.888  270.5156
 $$$ SIA        v$atdh 0d  +0.23544
S      0.41843:  2.82534  2.2296   0.1681   3041.703  270.5157
 $$$ SIA        v$atdh 0d  +0.26963
S      0.68806:  2.82521  1.6836   0.2057   3041.457  270.5159
 $$$ SIA        v$atdh 0d  +0.29947
S      0.98754:  2.82525  1.6840   0.2055   3041.174  270.5161
 $$$ SIA        v$atdh 0e  +0.01246
S      1.00000:  2.82526  2.2288   0.1680   3041.163  270.5163
done with run ... 
...
Note the choice of the height of the computational box (``\verb|-Lz 4000|''), of the number of vertical levels (``\verb|-Mz 81|''), and of a not-equally-spaced grid (``\verb|-quadZ|''; currently UNDOCUMENTED OPTION).  The messages to standard out show that the vertical spacing is less than 20 m near the base and more than 80 m at the top of the computational box (where it matters little).
