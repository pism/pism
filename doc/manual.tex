...
\optdef{low\und temp}{200.0}  Set the temperature which PISM will regard as ``too low''.  Units in Kelvin.  Note that a numerical error associated to the advection part of the conservation of energy scheme is for the maximum principle to be violated so that temperatures appear at the next step which are outside the range of current temperatures.  PISM attempts to count such violations.  See option \verb|-max_low_temps|.
...
\optdef{max\und low\und temps}{10}  Give the number (a nonnegative integer) of allowed low temp violations per time step.  Note these are reported in detail (i.e.~with location) if \verb|-verbose| is set.  Note that a numerical error associated to the advection part of the conservation of energy scheme is for the maximum principle to be violated so that temperatures appear at the next step which are outside the range of current temperatures.  PISM attempts to count such violations.  See option \verb|-low_temp| for the cutoff low temperature.
