...
\subsection{Installing prerequisities}\label{subsect:prereq}
...
\subsection{Installing PISM itself}\label{subsect:installPISM}
...
\subsection{Quick tests of the installation}\label{subsect:quicktest}
...
\subsection{Two models of ice flow: slow versus fast, but always shallow}  An important fact about PISM is that it can numerically solve two significantly different sets of equations which determine the velocity field within the ice.  In both cases the geometry of the ice, the temperature field within the ice, and the stress condition at the base of the ice are combined together with versions of the balance of momentum equations to determine the flow.  But there are two different versions of the balance of momentum equations:\begin{itemize}
...
\subsection{Schoof's plastic till free boundary problem for ice streams} \label{subsect:plastic}    In \cite{SchoofStream} Christian Schoof supposes a model for plastic failure of the basal till under ice sheets and generates an associated free boundary problem for the SSA.  This is a model for emergent ice streams within a larger ice sheet and ice shelf system.  It explains the existence of ice streams by the plastic failure of the till and the balance of (in-plane) stresses.
...
\subsection{Viewing the PISM state}  Basic graphical views of the changing state of a PISM ice model are available at the command line by using the options ``\t{-d}'' and ``\t{-dbig}'' with additional arguments.  For instance:
...
\subsection{Single character names (for each view)}  The single character diagnostic viewer and \Matlab views names are:
