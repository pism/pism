...
\noindent The full text description, available at the above site as \verb|.pdf|, is essential reading, both to understand MISMIP results and many of the next comments.

PISM's version of MISMIP includes an attached ice shelf.  There is a flow line analysis in \cite{SchoofMarine1}, a paper which strongly influenced the design of the MISMIP, which shows that the ice shelf need not be modeled because its only effect is to apply the force present at the calving front directly to the ice at the grounding line.  But this analysis does not apply to general ice shelves with two horizontal dimensions.  These real ice shelves, which are well-modeled in PISM as shown in section \ref{sect:ross}, have ``buttressing'' and ``side drag'' and other forces not present in flow line circumstances.  Instead of changing the ice dynamics in PISM to remove the ice shelf for this flow line job, instead we keep the usual PISM ice shelf model for MISMIP.

Because PISM is a map-plane model with two horizontal dimensions, we must adapt it to do a one horizontal dimension problem.  The flow line direction for MISMIP is taken to be ``$y$'', fairly arbitrarily.  We periodize the cross-flow direction ``$x$'', and use the minimum number of points in this direction which maintains function.  This number turns out to be ``\verb|-Mx 3|''; fewer points than this in the cross-flow direction confuses the finite difference schemes, apparently.
...
To do an actual MISMIP run, the executable \verb|pisms| is used, as with EISMINT II previously.  Table \ref{tab:MISMIPoptions} shows particular runtime options.  

A reasonable first MISMIP run is:

\verb|$ cd examples/mismip/|  \hfill (directory where some MISMIP-related scripts are)
...
\verb|$ mpiexec -n 4 pisms -mismip 1a -initials EBU -model 1 -step 1 \|

\verb|  -Mx 3 -Mz 15 -My 151 -extras|

\noindent This time it takes about 2 minutes on my 8 core machine.  In addition to the same files as before, this time we get an additional ASCII file \verb|EBU1_1a_M1_A1_extras|.  The \verb|_ss| file has the thickness and the \verb|_extras| file has the bed and surface elevation.  With all this information we can produce a nice figure, showing the ice sheet and ice shelf profile for the final steady state, by using the Python script \verb|figsMISMIP.py| (in directory \verb|examples/mismip/|):

\verb|$ ./figsMISMIP.py -p EBU1_1a_M1_A1 -e 1|

\noindent The result is shown in Figure \ref{fig:MISMIPmodel1exper1aM1A1}.  The script \verb|figsMISMIP.py| does not require \verb|_extras| output, but does produce a nicer picture if it is present.  Once you have done multiple runs, the script \verb|makefigs.sh| will apply \verb|figsMISMIP.py| to each MISMIP output in the current directory.

\begin{figure}[ht]
\includegraphics[width=5.0in,keepaspectratio=true]{figs/profile_EBU1_1a_M1_A1}
...
FIXME: describe how to continue to step 2, how to do other grid modes, -skip 10 and -ksprtol stuff, and exper 3b

FIXME: but all is not kosher!  show jump in cflx, compare hysteresis figure 3 in \verb|mismip_4.pdf| 

FIXME: mention convergence criterion and reported dxg/dt

Running \verb|pisms -mismip ...| with options as shown here is perhaps fine for a few particular MISMIP runs, but hardly adequate to manage all 372 runs in the intercomparison.  We need automation!  In \verb|examples/mismip/| there is a shell script \verb|mismip.sh| which helps manage the runs.  There are several nested loops, naturally, but setting variables \verb|MODELRANGE| and \verb|GRIDRANGE| at the start will do much of the job of configuring what you want.  Do ``\verb|./mismip.sh 4 D|'', or similar, with the extra ``\verb|D|'' at the end, to see what \verb|pisms -mismip| calls will happen \emph{before} actually actually starting any; yes, ``\verb|D|'' stands for ``debug''.
