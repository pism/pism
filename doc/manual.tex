...
\author[]{Ed Bueler \\ Jed Brown \\ Nathan Shemonski \\ Constantine Khroulev}

\date{\today.  \href{mailto:help@pism-docs.org}{help@pism-docs.org}.  Based on PISM rev \PISMREV\,and PETSC rel \PETSCREL.  \\ Get PISM development version by ``\quad\texttt{svn co http://svn.gna.org/svn/pism/trunk pism-dev}\quad''} 
...
This \emph{User's Manual} describes how to download\index{PISM!download source code} the PISM source code and install it\index{PISM!install} and the libraries it needs.  It describes how to run PISM for certain simplified geometry situations and it illustrates how PISM's numerical approximations are verified.  It describes how to use PISM as a Greenland ice sheet model or a Ross ice shelf model, based on basic publicly-available data.

Information about PISM is online at

\centerline{\href{http://www.pism-docs.org}{\t{www.pism-docs.org}}}

Users who want to advance the science of ice sheets will need to go beyond what is described in this \emph{User's Manual}.  For such users there are two additional documents to know about:
\begin{enumerate}
 \item  The \emph{PISM Reference Manual}\index{PISM!\emph{Reference Manual}}
describes the most important pieces of the source code.  It contains, or is intended to contain, the minimum documentation of the PISM source code parts in order to include all the continuum models and numerical methods of PISM.  A recent development version of the \emph{Reference Manual} is online (\href{http://www.pism-docs.org/refman-dev.pdf}{\t{www.pism-docs.org/refman-dev.pdf}}).  A current \emph{Reference Manual} can be generated from the source by doing \verb|cd doc| and then \verb|make|.  Then use a PDF viewer on \verb|refman.pdf|.
...
\item A directory called ``\verb|pism-dev/|'' will be created.  Note that in the future when you enter that directory, \verb|svn update| will update to the latest revision of PISM.\footnote{Of course, after \t{svn update} you will \t{make} and \t{make install} to recompile and relink PISM.}
\end{enumerate}

\item Build PISM:\footnote{Please report any problems you meet at this build stage by sending us the output: \t{help\@@pism-docs.org}.}\index{make}\index{PISM!install}
...
\verb|export PATH=/home/user/pism-dev/bin:/home/user/pism-dev/util:$PATH|
\end{enumerate}
\smallskip
...
\noindent If you would like us to confirm that PISM is working as expected please save the one page or so of output from this script and send it to us (\verb|help@pism-docs.org|).  See section \ref{sect:verif} for more on PISM verification.
\end{enumerate}
\smallskip
...
FIXME: need to document \verb|-quadZ|
...
\subsubsection*{Refinement}  To meaningfully verify a numerical code one must go down a grid refinement path and measure numerical error for each grid \cite{Roache}.  By ``a refinement path''\index{refinement path!definition} we mean the specification of a sequence of grid cell sizes which decrease toward the refinement limit of zero cell size.  For example, in the the two spatial and one time dimension case this means a sequence of triples $(\Delta x,\Delta y,\Delta t)$ which decrease toward the (unreachable) limit $(0,0,0)$.  By ``measuring the error for each grid'' we will mean computing a norm (or norms) of the difference between the numerical solution and the exact solution.   Concretely, we usually compute both the \emph{maximum} numerical error anywhere on the grid and the \emph{average} numerical error on the grid.
...
\end{verbatim}
\normalsize\end{quote}
Here we seek to measure the basic function of the isothermal shallow ice approximation components of PISM.  The exact solution used here is the Halfar similarity solution \cite{Halfar83}, in the case of no accumulation.  One specifies the number of grid points when running PISM, but this is equivalent to specifying the grid cell sizes if the overall dimensions of the computational box is fixed; see subsection \ref{subsect:coords}.  The refinement path is the sequence of triples $(\Delta x,\Delta y,\Delta t)$ with $\Delta x = \Delta y = 80,40,20,10$ and where $\Delta t$ is determined adaptively by a stability criterion (see subsection \ref{subsect:adapt}).  Note that the vertical grid spacing $\Delta z$ is fixed because this test is isothermal and no dependence of the error is expected from changing $\Delta z$.
...
PISM's version of MISMIP includes an attached ice shelf even though modeling the shelf is theoretically unnecessary in the flow line case (as considered in MISMIP).  In fact, \cite{SchoofMarine1} shows that the only effect on an ice shelf in the flow line case is to transfer the force imbalance at the calving front directly to the ice column at the grounding line.  This analysis does not apply to general ice shelves with two horizontal dimensions, however.  Real ice shelves, which are the modelling goal in PISM, have ``buttressing'' and ``side drag'' and other forces not present in flow line circumstances.  In fact, PISM was first used to model a two horizontal dimension ice shelf, namely the Ross ice shelf example in section \ref{sect:ross}.

Instead of changing the ice dynamics in PISM to remove the ice shelf for an idealized flow line job, we keep the usual PISM ice shelf model to do MISMIP, and we consider the affect on grounding line dynamics.  We make the base of the ice shelves have very small resistance, namely $10^4$ less linear drag than is typical of a Siple coast ice stream \cite{HulbeMacAyeal}, because this seems to have the effect of stabilizing the ice shelf.
...
\verb|-ssa_min_thk| & 5 & m; calving-front boundary condition is dealt with by  \\
                    ...
                    &   & of this thickness; changing this has some effect  \\
\hline\normalsize
\end{tabular}\end{table}
...
\verb|  -skip 1 -y 1 -o green20km_y1|
...
bootstrapping by PISM default method from file eis_green_smoothed.nc
  ...
  WARNING: till friction angle 'tillphi' not found; using default constant 15.000 deg
...
\verb|mpiexec -n 8 pgrn -ssl2 -if green20km_Tsteady.nc -y 10000 -ys 0 \|

\verb|     -o green_SSL2_10k|

\noindent We can continue for another ten thousand years by

\verb|mpiexec -n 8 pgrn -ssl2 -if green_SSL2_10k.nc -y 10000 \|

\verb|     -o green_SSL2_20k|
...
\verb|mpiexec -n 8 pgrn -ccl3 -if green_SSL2_110k.nc -dTforcing grip_dT.nc \|

\verb|      -dSLforcing specmap_dSL.nc -ys -249900 -ye 0 -o green_CCL3_y-0|
...
\verb|mpiexec -n 8 pgrn -ccl3 -if green_CCL3_y-0.nc -y 500 -o green_CCL3_y500|
...
\verb|mpiexec -n 8 pgrn -gwl3 -if green_CCL3_y-0.nc -y 500 -o green_GWL3_y500|
...
\verb|$  pismv -test C -Mx 61 -e 1.2 -o foo|
