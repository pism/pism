...
$ pisms -eisII A -if foo.nc -y 100 bar.nc
\end{verbatim}
\smallskip
...
\verb|$ pisms -if foo.nc -y 100 bar.nc|
...
$  pisms -eisII A -Mx 101 -My 101 -Mz 201 -y 1000 \
         -regrid foo.nc -regrid_vars HT -o bar.nc
\end{verbatim}
...
\begin{verbatim}
$ pismr -if foo.nc -y 10000 -o output.nc -save_to snapshots \
...
$ mpiexec -n 8 pisms -ismip H -run ST -if XYZ_ST_150k.nc \
...
\begin{verbatim}
$ pismd -ross -bif ross.nc -ssa -ssaBC ross.nc \
...
\begin{verbatim}
$ mpiexec -n 4 pismd -ross -bif ross.nc -ssa -ssaBC ross.nc \
...
\opt{o}  Give name of output file: \verb|-o foo.nc| writes an output file named \verb|foo.nc|.  See the description of option \verb|-if|.  Default name is \verb|unnamed.nc| under \verb|pismr|, \verb|simp_exper.nc| under \verb|pisms|, and \verb|verify.nc| under \verb|pismv|.
...
\opt{save\und to} Give the name of the NetCDF file to save model state
snapshots to. Requires setting \verb|-save_at|. See section \ref{sec:snapshots}
for more details. Note that in presence of the \verb|-split_snapshots| option
\verb|-save_to| value is interpreted as a \emph{basename}. That is,
\begin{verbatim}
$ pismv -test G -y 1000 -save_to foo -save_at 0:100:1000 -o verify.nc
\end{verbatim}
will produce files with names such as \verb|foo-000112.nc|.

\opt{save\und at} Takes two kinds of input: either a MATLAB-style range in the form
$t_{0}:\Delta t:t_{f}$ or a comma-separated list of times in years. Saving the
snapshots \emph{does not} affect the time-stepping determined by the adaptive
scheme. As a result, snapshots are saved \emph{on the first time-step after the
  time requested.} For example, if in a particular run PISM uses time-steps of
60 years, then \verb|-save_at 50,80,100| will produce snapshots at 60 and 120
years. In particular, we do not guarantee producing the exact number of
snapshots requested. Also note the comma-separated list above can be at most 20
numbers long. See section \ref{sec:snapshots} for more details.
