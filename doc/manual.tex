...
U        years 10^6_km^3 10^6_km^2 (none)          m         K
 $$$$            $$$$$ $$
S      0.00000:  0.00000  0.0000   0.0000      0.000  223.1500
 $$$ SIA        v$at$h 0m  +60.00000
S     60.00000:  0.01704  0.6281   0.0000     30.000  223.1500
. . .
 $$$ SIA        v$at$h 0e  +20.00000
S   2000.00000:  0.56790  0.6306   0.0000   1000.000  228.7242
done with run ... 
...
\verb|$ pisms -eisII F -if simp_exper.nc -y 1000 -d HTt|
...
\verb|pismd -if eisIIF200k.nc -o eisIIF200k_diag|

\noindent The result of this command is a NetCDF file \verb|eisIIF200k_diag.nc| which contains the full three-dimensional velocity field in the scalar NetCDF variables \verb|uvel|, \verb|vvel|, and \verb|wvel|.
...
\verb|pisms -eisII F -if eisIIF200k.nc -y 0.001 -o eisIIF200k_plus|

\noindent then the horizontal velocity field computed in this single time step run is the same as that saved in \verb|eisIIF200k_diag.nc|.\footnote{For this example, one can use a NetCDF Operator to check that the two saved NetCDF files contain essentially the same vertically-averaged horizontal velocity field; just do \t{ncdiff -v cbar} \dots}
...
\end{itemize}
PISM numerically solves both the SIA and SSA equations in parallel.  Time-stepping is allowed for both models.

The SIA equations are, as a rule, easier and quicker to numerically solve than the SSA.  The SIA equations are also easier to parallelize.
...
By default, PISM does not numerically solve the SSA when using the executables \verb|pismr|, \verb|pismd|, or \verb|pisms|.  The user must add the flag \verb|-ssa|.
...
PISM implements Schoof's model.  The user gives the options \verb|-ssa -plastic| to turn it on.  All grounded points are marked as SSA (i.e.~as \verb|MASK_DRAGGING|).  At these points a yield stress is computed from the amount of stored basal water, a thermodynamic quantity, and from a (generally) spatially-varying till strength.  See the description of option \verb|-plastic| in Appendix A for a detailed description of the yield stress computation.


\subsubsection*{A plastic till modification of EISMINT II experiment I}  To demonstrate the Schoof model we describe an example based on EISMINT II experiment I.  This EISMINT experiment is only documented in \cite{EISIIdescribe}, but not, however, in the published intercomparison \cite{EISMINT00}.\footnote{The latter reference says that for experiment I were ``straightforward results with little variability between groups'' but also that the results were ``difficult to interpret'' \cite{EISMINT00}.}  Note that Section \ref{sect:simp} describes using PISM to perform all the EISMINT II experiments documented in \cite{EISIIdescribe}, including experiment I.

Experiment I is identical to experiment A except for having ``trough'' bed topography.  
%This bed is shown in Figure \ref{fig:eis2I}.  
As specified, experiment I has nothing to do with the SSA equations or plastic till failure, and indeed there is no basal motion at all.  However, we use it to build an ice sheet which ``ought'' to have an ice stream, that is, along the trough.  The script \verb|test/eis2plastic.sh| gives a sequence which starts with the plain experiment I on a standard EISMINT II grid and builds an ice sheet modeled by SIA.  It then turns on the plastic till SSA and superposes the velocity fields.
...
\emph{This experiment is for now only documented in \cite{BuelerBrownSuper}, in preparation.}
...
\clearpage
\newpage
\section{Initialization and inverse problems: ``bootstrapping'' PISM}\label{sect:boot}  [THIS SECTION IS TO BE ADDED; see subsection \ref{sect:green} as an example of bootstrapping]

[Note \cite{JoughinMacAyealTulaczyk} is a representative inverse modeling result which could be incorporated in a forward model when using the SSA for ice streams.]
...
Table \ref{bootstrapEISgreen} shows the PISM output for a one model year run using option \verb|-bif| to ``bootstrap'' from file \verb|eis_green_smoothed.nc|

\verb|$ pgrn -bif eis_green_smoothed.nc -Mx 83 -My 141 -Mz 201 -ocean_kill -y 1 -o green20km_y1|
...
\begin{quote}
\begin{verbatim}
$ pgrn -bif eis_green_smoothed.nc -Mx 83 -My 141 -Mz 201 -ocean_kill -y 1 -o green20km_y1
...
S      0.00000:  2.82500  1.6708   0.2071   3042.000  270.5156
 $$$ SIA        v$atdh 0d  +0.18317
S      0.18317:  2.82515  2.2300   0.1683   3041.888  270.5156
 $$$ SIA        v$atdh 0d  +0.23546
S      0.41864:  2.82534  2.2300   0.1683   3041.702  270.5157
 $$$ SIA        v$atdh 0d  +0.26954
S      0.68818:  2.82522  1.6852   0.2056   3041.457  270.5159
 $$$ SIA        v$atdh 0d  +0.29922
S      0.98739:  2.82527  1.6840   0.2055   3041.172  270.5161
 $$$ SIA        v$atdh 0e  +0.01261
S      1.00000:  2.82528  2.2232   0.1659   3041.161  270.5163
done with run ... 
...
These runs took [HOW LONG?] on an 4 processor (8 core) Opteron cluster bought in 2006.
...
For these runs we add ``\verb|-tempskip 3|'' to speed things up.  The effect of this option is to allow the explicit time-stepping scheme to not update the temperature field for up to three time-steps of the mass continuity scheme, if this is allowed by the stability conditions; see subsection \ref{subsect:adapt} for more detail.
...
The run that completed the 110k model year run is 
