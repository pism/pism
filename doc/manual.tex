...
\date{\today.  $\phantom{|}^\ast$\texttt{ffelb\@@uaf.edu}.  \\ Based on PISM version ``\texttt{stable0.1}''\,and PETSC release \PETSCREV.  \\ Get PISM by Subversion: \qquad \texttt{svn co http://svn.gna.org/svn/pism/branches/stable0.1 pism}.} 
...
\item  Configuring PETSc takes many minutes even when everything goes smoothly.   Note that a previously installed PETSc can be reconfigured with a new \verb|PETSC_ARCH| if necessary.
...
\item \label{getPISMstep} Get the source for the stable branch of PISM using the Subversion version control system:
