...
\begin{table}[h]
\caption{Dependencies for PISM, listed alphabetically.  These are needed to build a fully-functional PISM from source and to do all the examples in the manual.  \scriptsize (Note that ``developers' versions,'' which include the relevant header files, \protect{\emph{are}} needed for FFTW, GSL, and NetCDF, if using Debian repositories.)\normalsize}\label{tab:PISMdepends}
\small
...
\begin{table}[h]
\caption{Additional dependencies for PISM, listed alphabetically.  These are \protect{\large \emph{needed only for serious developers of PISM}\normalsize}.}\label{tab:PISMdepends_superdev}
\small
\begin{tabular}{@{}llll}\hline
\textbf{Library} & \textbf{Site} & \textbf{Comment} \\
\textbf{/Program} &  &  &  \\ \hline
\LaTeX & \href{http://www.latex-project.org/}{\t{www.latex-project.org}} & only used for rebuilding manuals (both the User's Manual  \\
 &  & and the Reference Manual) from source \\
\texttt{doxygen} & \href{http://www.stack.nl/~dimitri/doxygen/}{\t{www.doxygen.org}} & only used for rebuilding the Reference Manual from source \\
\texttt{ruby} & \href{http://www.ruby-lang.org/en/}{\texttt{www.ruby-lang.org}} & only used when NetCDF format for PISM output file changes \\
\hline
\normalsize
\end{tabular}
\end{table}
...
For now, however, and to avoid issues of data formats (and data flaws) in a first use of PISM, this section describes how to use PISM for experiment F in the EISMINT II thermomechanical coupling intercomparison \cite{EISMINT00}.  In this experiment one models an angularly-symmetric shallow, grounded ice sheet on a flat bed with relatively cold surface temperatures.  Unfortunately one happens to be approximating an unstable equilibrium of the relevant (thermomechanically-coupled) partial differential equations, so one inevitably gets ``spokes'' in the basal temperature field \cite{BBL,PayneBaldwin,SaitoEISMINT}.
...
\subsection{Evolution runs versus ``diagnostic'' runs}  The goal of a numerical ice sheet model like PISM is, ostensibly, to be a dynamical system which evolves over time just like the modelled ice sheet, assuming one starts with the right initial conditions and one has the right climate and other inputs at each time (boundary conditions).  For example, the EISMINT II simplified geometry experiments above are posed as initial value problems for evolution-in-time partial differential equations.

Much of ice sheet modelling, however, requires ``diagnositic'' solution of the partial differential equations which determine the velocity field.  These are the balance of momentum equations for a slowly flowing fluid \cite{Fowler}.  As explained in the next section, the ``shallow ice approximation'' which underlies the EISMINT II experiments is only one of two forms of the balance of momentum equations solved by PISM; the other is the ``shallow shelf approximation''.
...
\subsection{Two models of ice flow: slow versus fast, but always shallow}  An important fact about PISM is that there are two significantly different sets of equations which determine the velocity field within the ice.  Roughly speaking, in both cases the geometry of the ice, the temperature field within the ice, and the stress condition at the base of the ice together determine the flow.  But there are two different versions of the balance of momentum equations:\begin{itemize}
...
The SIA equations can confidently be applied to the grounded part of ice sheets for which the basal ice is frozen to the bedrock \cite{Fowler}.  These equations are, as a rule, easier to numerically solve than the SSA.  Certainly they are easier to parallelize.

The SSA equations can confidently be applied to ice shelves because in shelves there is no (deviatoric) stress at the base of the (floating) ice \cite{Morland,MorlandZainuddin}.  These equations are harder to solve numerically and harder to parallelize.  

Nonetheless PISM numerically solves both the SIA and SSA equations in parallel.

Ice sheets have faster-flowing \emph{grounded} parts, however, usually called ``ice streams'' or ``outlet glaciers'' in the context of the Greenland and Antarctic ice sheets.  Describing these faster-flowing grounded parts of ice sheets requires something more than the SIA.  Ultimately one might use the full Stokes equations which represent the balance of \emph{all} tensor components \cite{Fowler}.  Alternatively one might use a ``higher-order'' (but also still shallow) stress balance \cite{Blatter}.
...
Because of the typical magnitudes of the (vertically-averaged) horizontal velocities in these two cases, for the rest of this manual we identify the SIA as \emph{slow flow} and all cases of the SSA as \emph{fast flow}.

We emphasize, however, that both the SIA and SSA models are \emph{shallow}.  That is, the partial differential equations in these two approximations come by (different) small-parameter arguments from the Stokes equations of a non-Newtonian fluid.  For these small-parameter argument in the SIA see \cite{Fowler}.  For the corresponding SSA argument, see the appendices of \cite{SchoofStream}.  (The references are, of course, the right place to examine the continuum equations and their physical motivation.  This manual naturally avoids direct discussuion of the continuum model.)

Within PISM, each grid point in the horizontal grid is either SIA \emph{or} SSA.  One must decide which parts of the grounded ice sheet are modelled by the SIA and which by the SSA.  This section discusses from a practical point of view how the user can make this choice when using PISM.  We address how to make the choice through specifying a \emph{mask}, or, alternatively, how to use a plasticity assumption to determine the region of fast flow through the solution of a free boundary problem.
...
In addition to the seven experiments published in \cite{EISMINT00}, there were an additional four experiments described in the EISMINT II intercomparison description 
\cite{EISIIdescribe}, labeled I, J, K, and L.  These experiments were similar to A and C but with non-flat topography.  In the PISM directory the topography files are at \verb|test/eisII/trough.nc| and \verb|test/eisII/mound.nc|.  See table \ref{tab:eisII} for how to run these experiments.
...
\verb|-Mmax| & 0.5 [ABDFGHIK], 0.25 [CJL] & m$/$a & max value of accumulation rate \\
\verb|-Rel| & 450 [ABFGHIK], 425 [CDJL] & km & radial distance to equilibrium line \\
...
\verb|-Tmin| & 238.15 [ACDGHIJKL], 243.15[B], 223.15[F] & K & max of surface temperature \\ \hline
