...
\vspace{1.5in}
\begin{quote}
\textsl{Copyright (C) 2004--2009 Ed Bueler and Jed Brown and Constantine Khroulev and Nathan Shemonski}
\medskip
...
PISM's job is to approximate ice flow.  In addition (subsection \ref{subsect:beddef}) PISM includes an optional bed deformation component approximating the movement of the Earth's crust and upper mantle in response to changing ice load.  Also the temperature of a thin layer of bedrock is included in the conservation of energy model for the ice and the till layer.

Thus PISM ``handles'' the ice and bedrock, when used with models for the atmosphere and ocean, as cartooned in figure \ref{fig:climatecartoon}.  A structural principle for PISM is that, within the ice dynamics/bed deformation core code, there is no\footnote{This principle is a change from \t{stable0.1}.} parameterization or other model for boundary mass or energy balance.  Instead these parameterizations are handled by non-core components called \t{PISMAtmosphereCoupler} and \t{PISMOceanCoupler}.  The latter type is special to the undersides of ice shelves.  The advantage of this arrangement is that ``attaching'' PISM to other components of a global or regional climate (circulation) model is intended to be a matter only of changing a small amount of the source code, without any change to the ice dynamics core.

\begin{figure}[ht]
\hspace{2.0in}
\includegraphics[height=2.0in,keepaspectratio=true]{figs/climate_cartoon}
\caption{PISM approximates ice flow and bed deformation, so it ``handles'' the region inside the dashed curve.  Components called \t{PISMAtmosphereCoupler} and \t{PISMOceanCoupler} provide climate inputs, which may be outputs of atmosphere and ocean models.}
\label{fig:climatecartoon}
\end{figure}

A PISM source code release \emph{does} contain instances of these components, as source code outside the core.  For example, \t{PISMAtmosphereCoupler}s exist for the simplified geometry experiments in section \ref{sect:simp}, and for the Greenland model in section \ref{sect:green}.

Most of this is actually invisible to the PISM user.  Options given to the PISM executables (e.g.~\t{pismr} or \t{pisms}) are passed internally to the \t{PISM...Coupler} components, so it looks to the user as if control of climate inputs is the same as ice dynamics.  Indeed, an important kind of \t{PISMAtmosphereCoupler}, namely a positive degree day scheme for surface mass balance, is described in subsection \ref{subsect:pdd} as a part of PISM.

Nonetheless this structure is important to describe here because, on the one hand, some users may be interested in coupling PISM to other models.  On the other hand, the PISM authors do not claim expertise in modeling atmosphere, ocean, or even snow processes, and so separation has a code-reliability purpose as well.
...
\clearpage
\newpage
\section{Initialization versus bootstrapping}\label{sect:boot}  

\subsection{``Initialization from a saved model state''}  This phrase\index{initialization!from saved model state} has a simple meaning in PISM.  If a previous PISM run has saved a NetCDF file using ``\verb|-o|'' then that file will contain complete initial conditions for continuing the run.  The output file from the last run can be loaded with ``\intextoption{i}'', as in this example:\index{pisms}
...
Note that simplified-geometry experiments (section \ref{sect:simp}) and verification tests (section \ref{sect:verif}) do not need input files at all because they initialize from formulas in the source code.  They can be continued from saved model states using \verb|-i|, however.  As in the above example, however, specifying which simplified geometry experiment (or verification test) is in use \emph{is} necessary, so that the run can continue with the climate inputs specified for the simplified-geometry experiment.  For instance, if the second line above is replaced by

\verb|$ pisms -i foo.nc -y 100 -o bar.nc|
...
When using \intextoption{boot\und from} you will need to specify both grid dimensions (using \intextoption{Mx}, \intextoption{My} and \intextoption{Mz}) and the height of the computational box for the ice with \intextoption{Lz}. This additional specification is natural: the data used in ``bootstrapping'' is two-dimensional (i.e. 3D fields are ignored).

\subsection{Input file formats}
\subsubsection{Input files}
PISM produces CF-1.4 compliant NetCDF\index{PISM!NetCDF file format}\index{NetCDF} files; the easiest way of learning about the format is by running
\begin{verbatim}
$ pismv -test G -y 1 -o foo.nc
$ ncdump -h foo.nc | less
\end{verbatim}
and looking at the output. It is worth mentioning, though, that CF metadata conventions require using the reference date in the units string of a time (\verb|t|) variable, but \emph{PISM completely ignores this reference date.}

\noindent The file used with ``\intextoption{i}'' must have this format.

\subsubsection{Bootstrapping file format}
...
\item The names of variables do not need to match the corresponding variables in a PISM output file; if they do not, the \verb|standard_name| attribute is used to identify a variable.
\item PISM expects all the input fields (NetCDF variables) to be in MKS \emph{or} have the units specified using the \verb|units| attribute.\footnote{PISM uses UDUNITS to convert data present in an input file to MKS. This means that having ice thickness in kilometers or temperature in Celsius (for example) \emph{is} allowed.} 
\item Any of the other two dimensional variables (depending on \verb|x,y|) may be missing, and for those missing values some heuristic will be applied. Please see table \ref{tab:modelhierarchy} for lists of data necessary for bootstrapping.
\end{enumerate}
...
Here is another example: suppose you have an output of a PISM run on a fairly coarse grid (stored in \verb|foo.nc|) and you want to continue this run on a finer grid. This can be done using \intextoption{regrid\und from} with \intextoption{boot\und from}:
\begin{verbatim}
$ pismr -boot_from foo.nc -Mx 501 -My 501 -Mz 51 -Mbz 51 -quadZ \
        -regrid_from foo.nc -regrid_vars BeT -y 100 -o bar.nc
\end{verbatim}
In this case all the model-state 2D variables present in \verb|foo.nc| will be interpolated onto the new grid during \emph{bootstrapping}, while three-dimensional variables are filled using heuristics mentioned in section \ref{sect:boot}. Then temperature in bedrock (\verb|B|)\footnote{Assuming that foo.nc has more than one bedrock layer.}, age of the ice (\verb|e|) and ice temperature (\verb|T|) will be interpolated from \verb|foo.nc| onto the new grid during \emph{regridding}, overriding values set at the bootstrapping stage.
...
\verb|c| & CFL for temperature/age advection \cite{BBL} \\
...
THESE WERE IN THE GETTING STARTED SECTION; NEEDED?
...
\label{subsect:plastic}
The remainder of this section gives an incomplete view, at best, of the dynamics in PISM because the best model for whole ice sheets, using the SSA as a sliding law for the SIA is undocumented except for the preprint \cite{BBssasliding}.

FIXME!!!

\intextoption{topg\und to\und till}

SSA\index{SSA (shallow shelf approximation)} equations with additional basal resistance are used in PISM to describe faster-flowing grounded ice.  The SSA version of the balance of momentum equations with linearly-viscous till are solved as a fixed-boundary problem (at a particular time step), and thus the locations and extent of ice streams must be determined by some other criterion.  By contrast, for the SSA with plastic till the locations of ice streams is determined as part of the free boundary problem.  We believe that the plastic till SSA of Schoof \cite{SchoofStream} should be regarded as the best currently implementable ``sliding law'' for the SIA \cite{BBssasliding}.\index{SIA (shallow ice approximation)!sliding law}
...
\verb|pismv -test I -Mx 121 -My 5 -d ncqu|
...
\end{verbatim}
\noindent Table \ref{tab:rossoptions} lists some of the options which are useful for this kind of diagnostic velocity computation. %Jed
...
-ice_type custom -ice_custom_hardness 1.5e8"
...
trying "mpiexec -n 2 pismd -ross -boot_from ross.nc -ssaBC ross.nc -riggs riggs.nc
-ksp_rtol 1e-08 -ssa_rtol 1e-05 -Mx 147 -My 147 -Lz 1000 -Mz 3 -ssa
-ice_type custom -ice_custom_hardness 1.8e8"
...
trying "mpiexec -n 2 pismd -ross -boot_from ross.nc -ssaBC ross.nc -riggs riggs.nc
-ksp_rtol 1e-08 -ssa_rtol 1e-05 -Mx 147 -My 147 -Lz 1000 -Mz 3 -ssa
-ice_type custom -ice_custom_hardness 1.9e8"
...
\begin{comment}
  \scripthead{util/nc\und template.py}\index{nc\und template.py} This Python
  script reads \verb|pism_state.cdl| and runs \verb|ncgen| to create an empty
  NetCDF file in PISM-readable format with given grid dimensions and variables
  defined.

  The intended use is this: suppose we have latitude, longitude, bed elevation,
  ice thickness and accumulation/ablation rate data and we need to convert it
  to PISM-readable format. One way to do this is to hand-edit
  \verb|src/netcdf/pism_state.cdl| to remove all the unneeded variables, run
  \verb|ncgen| on the resulting file, and then fill it with data. Another way
  is to manually define all the variables in the script converting data to
  PISM's format. Alternatively, one can run

  \verb|nc_template.py -x 101 -y 101 -v lat,lon,topg,thk,acab \|

  \verb| -p pism_state.cdl -o input.nc|

  \noindent to create \verb|input.nc| and then fill this file with data. This
  defines all the variables \emph{and} metadata.

  \opthead \verb|nc_template.py| takes the following options:

  \scriptoptdef{p}{pism-state}{pism\und state.cdl} the location of
  \verb|pism_state.cdl|
  ...
  \scriptoptdef{v}{variables}{all mapping, climate and all 2D model\und state
    variables} comma-separated list of variables

  Variables \verb|x|, \verb|y|, \verb|z|, \verb|zb|, \verb|t| are always
  included; if \verb|-v| if omitted, then the output file will have these
  variables, plus all the variables with \verb|pism_intent| equal to
  \verb|mapping|, \verb|climate_steady|, \verb|climate_snapshot| and all the 2D
  \verb|model_state| variables.
\end{comment}
