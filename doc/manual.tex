...
\scripthead{util/nc2mat.py}\index{nc2mat.py} This Python script reads specified variables from a NetCDF file and writes them to an output file in the MATLAB binary data file format (\verb|.mat|, supported by MATLAB version 5 and later). It depends on \verb|pycdf| and SciPy.
...
ice thickness and accumulation/ablation rate data and we need to convert it to PISM-readable format. Then we can run
\verb|nc_template.py -x 100 -y 100 -v |
