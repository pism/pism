...
\label{sec:saving-time-series}
 It is also possible to save time-series of some scalar diagnostic quantities.
...
Note that this (similarly to the snapshot-saving code) does not affect the time-stepping mechanism; PISM will save exactly the number of time-series records requested, \emph{linearly interpolated onto requested times}.
...
Scalar time-series take very little storage space (compared to spatially-varying data), so omitting the \intextoption{ts\und vars} makes PISM save \emph{all} the variables listed in table \ref{tab:time-series}.
...
Sometimes it is useful to have PISM save a handful of diagnostic quantities every 10 years (or even every year). One can use snapshots (section \ref{sec:snapshots}), but doing so can easily fill your hard-drive. The set of options described here allows saving a \emph{subset} of variables present in an output file.
...
Note that similar to the snapshot-saving mechanism, extra variables are saved \emph{on the first time-step after a requested time}. We do not guarantee saving at all the requested times.
