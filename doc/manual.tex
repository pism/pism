...
\optdef{constant\und nuH}{30.0}  This option is used then the velocities in ice shelves and streams (see \verb|-mv| below) are computed with a constant value for the product of viscosity $\nu$ and thickness $H$.  Comparing the option \verb|-constant_hardness| above, where the ice is shear-thinning with constant coefficient, for this option the ice is linearly viscous and effectively has both constant viscosity and constant thickness (for computing its strength).  Generally the viscosity is computed by a nonlinear iteration, but for this option the iteration does not occur.  If \verb|-constant_nuH| is set then \verb|-constant_hardness| is ignored.  The argument is given in units of MPa a, and the default value is $30$ MPa a, the value given in \cite{Ritzetal2001}.
