...
a macroscopic measure for the abundance of (partly micorscale) crevasses and rifts that 
...
(e.g. :cite:`Lemaitre2012Damage` and :cite:`Borstad2013Creep`), where a damage state 
...
.. list-table:: Options which turn on fracture model and softening effect.
   ...
   * - :opt:`-fracture_weighted_healing`
     - adds a term `1-D` to the healing term (similar to the source term), assuming 
       that highly damaged ice heals slower; can be combined with :opt:`-constant_healing
