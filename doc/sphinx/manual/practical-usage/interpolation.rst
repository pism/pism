...
interpolation *on a plane* in the projected coordinate system.) It works best if input and
...
used by PISM. See CDO_ (specifically, `cdo remapcon`) and GDAL_ (see `gdalwarp -r average`
and similar). The downside of this approach is that one has to store and archive
...
- the input (usually forcing) file contains projection information (again, see
  :ref:`sec-projections`).
