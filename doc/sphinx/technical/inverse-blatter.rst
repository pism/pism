...
The user-facing driver is ``examples/inverse/pismi_blatter.py``.
...
2. **Adjoint solve**: solve the 3D linear system
   ...
   using ``KSPSolveTranspose`` on the assembled Blatter Jacobian. This requires
   a **transpose-compatible preconditioner**: the geometric multigrid smoother
   must support transpose application. SOR does not; use Jacobi or
   Chebyshev-Jacobi (``-bp_mg_levels_pc_type jacobi``).
...
.. list-table::
   ...
   * - ``-bp_mg_levels_ksp_type chebyshev``
     -
     - smoother type (must support transpose for adjoint)
   * - ``-bp_mg_levels_pc_type jacobi``
     -
     - smoother preconditioner (must support transpose)
   * - ``-bp_snes_rtol 0.001``
     -
...
.. warning::
   ...
   The MG smoother **must support transpose application** (``PCApplyTranspose``)
   for the adjoint solve. The common SOR smoother does **not** support transpose.
   Use ``-bp_mg_levels_pc_type jacobi`` instead.
...
The Jacobian assembly for the adjoint solve uses ``SNESComputeJacobian`` to
properly handle the DMDA local-to-global data flow, rather than calling
``compute_jacobian`` directly (which expects local arrays from inside a SNES
callback context).
...
- ``examples/inverse/pismi_blatter.py`` — user-facing driver script
...
- **Adjoint smoother**: the multigrid smoother must support transpose; this
  rules out SOR (the standard Blatter smoother). Use Jacobi instead, which is
  a weaker smoother and may slow convergence.
- **Computational cost**: each TAO iteration requires a full 3D Blatter forward
  solve (`\sim 4\text{--}10` Newton iterations) plus an adjoint solve.
