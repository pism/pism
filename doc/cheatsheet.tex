...
\begin{tabular}{@{}p{0.3\linewidth}p{0.65\linewidth}@{}}
\texttt{-config} & read from an alternative config file\\
\texttt{-config_override} & select a file to use for overriding config
settings; useful for parameter studies\\
\texttt{-dump_config} & save config parameters used in a run to a file
\end{tabular}
...
\texttt{-extra_vars} & list of variables to save\\
...
\begin{tabular}{@{}p{0.3\linewidth}p{0.65\linewidth}@{}}
  ...
  \texttt{-ts_vars} & Comma-separated list of variables;
  supported variables include \texttt{ivol}, \texttt{iarea}, \texttt{iareag}, \texttt{iareaf},
  \texttt{dt}, \texttt{divoldt}, \texttt{dimassdt}\\
 \texttt{-ts_append} & Append time series to file if it already exists.
\end{tabular}
...
\texttt{-view_sounding} & a list of 3D fields to \\
