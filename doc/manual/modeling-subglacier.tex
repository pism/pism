...
\begin{table}
 ...
 \begin{tabular}{lp{0.6\linewidth}}
    ...
    \intextoption{till_cohesion} & Set the value of the till cohesion ($c_{0}$) in the plastic till model.  The value is a pressure, given in Pa.\\
...
Following \cite{Tulaczyketal2000}, based on laboratory experiments with till extracted from an ice stream in Antarctica, \cite{BuelervanPelt2015} propose the following parameterization which is used in PISM.  It is based on the ratio $s=W_{til}/W_{til}^{max}$ where $W_{til}=$ \texttt{tillwat} is the effective thickness of water in the till and $W_{til}^{max}=$\texttt{hydrology_tillwat_max} is the maximum amount of water in the till (see subsection \ref{subsect:subhydro}):
\begin{equation}
N_{til} = \min\left\{P_o, N_0 \left(\frac{\delta P_o}{N_0}\right)^s \, 10^{(e_0/C_c) \left(1 - s\right).}\right\} \label{eq:computeNtil}
\end{equation}
Here $P_o$ is the ice overburden pressure, which is determined entirely by the ice thickness and density, and the remaining parameters are set by options in Table \ref{tab:effective-pressure}.  While there is experimental support for the default values of $C_c$, $e_0$, and $N_0$, the value of $\delta=$\texttt{till_effective_fraction_overburden} should be regarded as uncertain, important, and subject to parameter studies to assess its effect.

FIXME: EVOLVING CODE:  If the \texttt{tauc_add_transportable_water} configuration flag is set (either in the configuration file or using the \intextoption{tauc_add_transportable_water} option), then the above formula becomes FIXME
...
\begin{table}
 ...
 \begin{tabular}{lp{0.6\linewidth}}
    ...
    \intextoption{till_effective_fraction_overburden} & $= \delta$ in \eqref{eq:computeNtil}, dimensionless, with default value 0.02 \cite{BuelervanPelt2015} \\
    \intextoption{till_reference_effective_pressure} & $= N_0$ in \eqref{eq:computeNtil}, in Pa, with default value 1000.0 \cite{Tulaczyketal2000} \\ \bottomrule
  \end{tabular}
...
See Table \ref{tab:hydrology} for options which apply to all hydrology models.  Note that the primary water source for these models is the energy conservation model which computes the basal melt rate \texttt{basal_melt_rate_grounded}.  There is, however, also option \intextoption{hydrology_input_to_bed_file} which allows the user to \emph{add} water directly into the subglacial layer, in addition to the computed \texttt{basal_melt_rate_grounded} values.  Thus \texttt{-hydrology_input_to_bed_file} allows the user to model drainage directly to the bed from surface runoff, for example.  Also option \intextoption{hydrology_bmelt_file} allows the user to replace the computed \texttt{basal_melt_rate_grounded} rate by values read from a file, thereby effectively decoupling the hydrology model from the ice dynamics (esp.~conservation of energy).
...
\begin{table}
 ...
 \begin{tabular}{lp{0.4\linewidth}}
    ...
    \textbf{Option} & \textbf{Description}
    \\\midrule
    \fileopt{hydrology_bmelt_file} & Specifies a NetCDF file which contains a time-independent field \texttt{basal_melt_rate_grounded} which has units of water thickness per time.  This rate \emph{replaces} the conservation-of-energy computed rate \texttt{basal_melt_rate_grounded}. \\
    \txtopt{hydrology_const_bmelt}{(m/s)} & If \texttt{-hydrology_use_const_bmelt} is set then use this to set the constant rate (water thickness per time). \\
    \fileopt{hydrology_input_to_bed_file} & Specifies a NetCDF file which contains a time-dependent field \texttt{inputtobed} which has units of water thickness per time.  This rate is \emph{added to} the \texttt{basal_melt_rate_grounded} rate. \\
    ...
    \txtopt{hydrology_tillwat_decay_rate}{(m/a)} &  Water accumulates in the till at the basal melt rate \texttt{basal_melt_rate_grounded}, minus this rate. \\
    \intextoption{hydrology_use_const_bmelt} & Replace the conservation-of-energy basal melt rate \texttt{basal_melt_rate_grounded} with a constant. \\
    \bottomrule
  \end{tabular}
...
\end{equation}
where $m=$\texttt{basal_melt_rate_grounded} (kg $\text{m}^{-2}\,\text{s}^{-1}$), $\rho_w$ is the density of fresh water, and $C=$\texttt{hydrology_tillwat_decay_rate}.  At all times bounds $0 \le W_{til} \le W_{til}^{max}$ are satisfied.
...
% FIXME  -hydrology distributed is not documented except by  \cite{BuelervanPelt2015}; controlled by options
...
\end{verbatim}                  % balance $ to make emacs happy
...
To include ``measured'' uplift rates during initialization, use the option \intextoption{uplift_file} to specify the name of the file containing the field \texttt{dbdt} (CF standard name: \texttt{tendency_of_bedrock_altitude}).

Use the \intextoption{topg_delta_file} option to apply a correction to the bed topography field read in from an input file. This sets the bed topography $b$ at the beginning of a run as follows:
\begin{equation}
  \label{eq:bedcorrection}
  b = b_{0} + \Delta b.
\end{equation}
Here $b_{0}$ is the bed topography (\texttt{topg}) read in from an input file and $\Delta b$ is the \texttt{topg_delta} field read in from the file specified using this option.
