...
The time-series file \texttt{ts_g20km_10ka.nc} is also growing.  It contains spatially-averaged ``scalar'' diagnostics like the total ice volume or the ice-sheet-wide maximum velocity (variable \texttt{volume_glacierized} and \texttt{max_hor_vel}, respectively).  It can be viewed
\begin{verbatim}
$ ncview ts_g20km_10ka.nc
\end{verbatim}
The growing time series for \texttt{volume_glacierized} is shown in Figure \ref{fig:growing} (right).  Recall that our intention was to generate a minimal model of the Greenland ice sheet in approximate steady-state with a steady (constant-in-time) climate.  The measurable steadiness of the \texttt{volume_glacierized} time series is a possible standard for steady state \cite[for example]{EISMINT00}.
...
\caption{Two views produced by \texttt{ncview} during a PISM model run.  Left: \texttt{thk}, the ice sheet thickness, a space-dependent field, from file \texttt{ex_g20km_10ka.nc}.  Right: \texttt{volume_glacierized}, the total ice sheet volume time-series, from file \texttt{ts_g20km_10ka.nc}.}
\label{fig:growing}
\end{figure}
...
\caption{Time series of modeled ice sheet volume \texttt{volume_glacierized} on 20km and 10km grids.  The present-day ice sheet has volume about $2.9\times 10^6\,\text{km}^3$ \cite{BamberLayberryGogenini}, the initial value seen in both runs.}
\label{fig:ivolboth}
\end{figure}
...
\caption{Time series of modeled ice sheet volume \texttt{volume_glacierized} from constant-climate (blue; \texttt{ts_g20km_10ka_hy.nc}) and paleo-climate (red; \texttt{ts_g20km_25ka_paleo.nc}) spinup runs.  Note that the paleo-climate run started with the ice geometry at the end of the constant-climate run.}
...
\caption{Time series of ice volume \texttt{volume_glacierized} from the three runs in our grid sequencing example: 20\,km for 10\,ka = \texttt{ts_g20km_10ka_hy.nc}, 10\,km for 2\,ka = \texttt{ts_g10km_gridseq.nc}, and 5\,km for 200\,a = \texttt{ts_g5km_gridseq.nc}.}
...
\caption{Time series of ice volume \texttt{volume_glacierized} from nine runs in our parameter study example, with parameter choices $(q,e)$ given.}
...
\begin{figure}[ht]
\centering
\mbox{\includegraphics[height=2.5in,keepaspectratio=true]{p10km-01-1-csurf.png} \includegraphics[height=2.5in,keepaspectratio=true]{p10km-01-3-csurf.png} \includegraphics[height=2.5in,keepaspectratio=true]{p10km-01-6-csurf.png} \qquad \hspace{1.81in}}
...
\mbox{\includegraphics[height=2.5in,keepaspectratio=true]{p10km-1-1-csurf.png} \includegraphics[height=2.5in,keepaspectratio=true]{p10km-1-3-csurf.png} \includegraphics[height=2.5in,keepaspectratio=true]{p10km-1-6-csurf.png} \qquad \hspace{1.81in}}
