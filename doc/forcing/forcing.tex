...
\begin{verbatim}
$ pclimate -i g20km_pre100.nc -atmosphere searise_greenland,paleo_precip \
...
\begin{figure}
  ...
  \begin{tikzpicture}[ auto, thick, font=\small, node distance=3mm,
    block/.style={rectangle, draw, text width=2in, text centered,
    ...
    \node [block] {Compute snow accumulation ``Accumulation''};
    \node [block, join] {Compute the number of positive degree days (\PDD) };
    \node [block, join] {Update snow depth \mbox{$\Hs \gets \Hs + \text{Accumulation}$}};
    \node [block, join] {Compute maximum possible snow melt \mbox{$\Ms^{\text{max}} := \PDD\times F_{s}$}};
    \node (melt all snow) [decision, join] {$\Ms^{\text{max}} > \Hs?$};

    \begin{scope}[start branch=yes]
      \node (ice melt case) [block, on chain=going below right] {\mbox{Melt all available snow} $\Ms := \Hs$};
    \end{scope}
    \begin{scope}[start branch=no]
      \node (no ice melt case) [block, on chain=going below left] {Melt some of the snow \mbox{$\Ms := \Ms^{\text{max}}$}};
    \end{scope}
    \node (dummy) {};
    \node (refreeze) [block] {Refreeze some snow $\text{Refreeze} := \Ms \times \theta_{\text{refreeze}}$};
    \node [block, join] {$\PDD_{\text{excess}} := \PDD - \Ms / F_{s}$};
    \node [block, join] {Melt some ice \mbox{$\Mi := \PDD_{\text{excess}} \times F_{i}$}};
    \node [block, join] {$\text{Runoff} := \Ms + \Mi - \text{Refreeze}$};
    \node [block, join] {Compute surface mass balance $\text{SMB} := \text{Accumulation} - \text{Runoff}$};
    \node [block, join] {Update snow depth $\Hs \gets \Hs - \Ms$};
    % special edges:
    \path[line] (melt all snow) -| node [right] {Yes} (ice melt case);
    \path[line] (melt all snow) -| node [left] {No} (no ice melt case);
    \path[line] (ice melt case) |- (refreeze);
    \path[line] (no ice melt case) |- (refreeze);
  \end{tikzpicture}
...
By default, PISM computes the number of PDDs, snow accumulation and
runoff every week. (This frequency is controlled by the
...
snow (\config{pdd_factor_snow}) or the corresponding factor for
ice (\config{pdd_factor_ice}). See figure \ref{fig:pdd-model} for the
flow-chart describing the computation performed each PDD time-step.

Note that \config{pdd_refreeze} fraction of the snow melt refreezes,
but all ice melt runs off.

Since PISM does not have a firn model, the snow depth is set to zero
once a year, on \config{pdd_balance_year_start_day}. Default is $274$,
corresponding to October 1$^{\text{st}}$.
...
\config{air_temp_all_precip_as_rain} (default of $1^\circ C$) is
...
$-1^\circ C$) is interpreted as snow.
