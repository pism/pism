...
This document describes PISM's climate forcing code and options
that control it.  Please see the PISM User's Manual, section
\ref*{userman-sec:climate-inputs}, for an overview  of PISM's surface
(atmosphere) and ocean climatic inputs and interfaces.  At these interfaces
the surface mass and energy balances are determined and/or passed to the ice
dynamics code.  PISM has well-defined separation of climate forcing
from ice dynamics, and most options mentioned in the User's Manual
control the ice dynamics part.

This manual distinguishes between those parts of PISM which are related
to coupling and climate forcing, namely ``components'', and the actual
physical ``models'' which the components incorporate.  Choosing components by
options always implies a choice of physical model for how climate forcing fields
become the fields used by the ice dynamics parts of PISM.  These physical models may be
very simple, however, with no actual changes to climate input fields in some cases.

Setting up PISM's climate interface requires selecting one \emph{surface} and one
\emph{ocean} component.  The surface component may use an \emph{atmosphere}
component also.
...
the first keyword \emph{has} to correspond to a model component, the rest can be
``modifier'' components.  Any of these options can be omitted to use the
default atmosphere, surface or ocean model components, although one has to
choose a model component to use a modifier.  Model components and modifiers
are chained as in Figure \ref{fig:climate-input-data-flow-repeated}.
% which is Figure~\ref*{userman-fig:climate-input-data-flow} of the User's Manual
...
\begin{figure}
  ...
  \caption{PISM climate input data flow. Colored arrows correspond to interfaces in
    Figure \ref*{userman-fig:climate-inputs} in the User's Manual.}
  \label{fig:climate-input-data-flow-repeated}
\end{figure}

Section~\ref{sec:known-use-cases} describes examples of components and modifiers.  First, however, we must address how PISM handles model time (Section~\ref{sec:model-time}).
...
forcing  code,  on  the other  hand,  uses  time  to provide  correct  boundary
conditions to other components.  For example of the issues related to time
...
it as ``periodic''. Each component has a unique command-line option
...
option. For example, to prescribe constant (from year to year) climate which includes
...
Use the \mbox{\txtopt{reference_date}{\texttt{YYYY-MM-DD}}} command-line option
or the \config{reference_date} configuration parameter to set the reference
date, i.e. the date corresponding to time $0$.
...
\clearpage\newpage
\section{Usage cases and corresponding options}
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    ...
    See  also   &  \texttt{-atmosphere  given}:
    \ref{sec:atmosphere-given}, \\
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    ...
    & \texttt{dSLforcing}: \ref{sec:delta-sea-level} \\
    \bottomrule
  \end{tabular}
\end{center}
...
$ pclimate -i state.nc -surface given -ys 0.0 -ye 2.5 -dt 0.1 -o movie.nc
\end{verbatim}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PSSimple}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & none\\
    Software component: & \class{PSElevation} \\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & none \\
    Software component: & \class{PSTemperatureIndex}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PSConstantPIK}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PSdTforcing}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PSLapseRates}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PSForceThickness}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PSAnomaly}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & \variable{air_temperature}, \\
    &\variable{precipitation} [m/s ice equivalent]\\
    Software component: & \class{PAGivenClimate}\\
    \bottomrule
  \end{tabular}
\end{center}
...
This model can be used to drive a temperature-index (PDD) climatic mass balance
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PASeariseGreenland}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & \variable{lon}, \variable{lat},
    \variable{precipitation} \mbox{[m/s ice equivalent]} \\
    Software component: & \class{PAEismintGreenland}\\
    \bottomrule
  \end{tabular}
\end{center}

This model implements the latitude, longitude, and surface elevation-dependent
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PAConstantPIK}\\
    \bottomrule
  \end{tabular}
\end{center}

This model reads a time-independent precipitation field from an input
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & \variable{delta_T} \\
    Software component: & \class{PAdTforcing}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{PAdPforcing}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & \variable{surface_altitude} (CF standard name) \\
    Software component: & \class{PALapseRates}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & \variable{air_temperature_anomaly}, \\
    &\variable{precipitation_anomaly} [m/s ice equivalent]\\
    Software component: & \class{PAAnomaly}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\item \fileopt{atmosphere_anomaly_file} specifies a file containing variables
  \variable{air_temperature_anomaly} and
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & none \\
    Software component: & \class{POConstant}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{POGivenClimate}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & none \\
    Software component: & \class{POConstantPIK}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\end{verbatim}
Enables all three modifiers described below.
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{POdSLforcing}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{itemize}
\item \fileopt{ocean_delta_SL_file}: specifies the name of the file containing forcing data. This file has to contain the \variable{delta_SL} variable using units of ice-equivalent thickness per time.
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    Software component: & \class{POdTforcing}\\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    NetCDF variables: & none \\
    Software component: & \class{POdSBMFforcing}\\
    \bottomrule
  \end{tabular}
\end{center}
