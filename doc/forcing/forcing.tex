...
the \texttt{pdd_std_dev} configuration parameter. The default value is $5.0$ degrees
\cite{RitzEISMINT}.
...
It uses a set of three equations describing (1) the heat and (2) the salinity balance as well as (3) the pressure and salinity dependent freezing point in the boundary layer \cite{Hellmeretal1998}. Temperature and salinity in that layer are equilibrated by fluxes from the adjacent mixed ocean waters and the ice and by the melting or refreezing of ice.
Inputs are potential temperature and salinity fields, for example from an ocean model.
No ocean circulation is modeled and there is therefore no feedback from derived melt waters on the ocean.
