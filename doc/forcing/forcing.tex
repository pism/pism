...
The length of the period (in years) is specified using \texttt{-..._period}
option.
...
parameter of command-line options described below.
...
\config{reference_date} configuration parameter.
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    ...
    Software components: & \class{PSGivenClimate} \\
...
these data need to be converted into top-of-the-ice temperature and climatic mass balance.
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    \toprule
    Variable names: & \variable{precipitation}, \variable{air_temperature} \\
    Options: & \texttt{-atmosphere given -atmosphere_given_file forcing.nc -surface~pdd} \\
    Software components: & \class{PAGivenClimate}, \class{PSTemperatureIndex} \\
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    ...
    Software components: & \class{PASeariseGreenland}, \class{PSTemperatureIndex} \\
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    ...
    Software components: & \class{PAGivenClimate}, \class{PAAnomaly}, \class{PSSimple}\\
    See  also   &  \texttt{-atmosphere  given}:  \ref{sec:atmosphere-given},
...
The \texttt{simple} surface model re-interprets precipitation as climatic mass balance, which is useful in cases when there is no melt (Antarctic simulations is an example).
...
\begin{center}
  \begin{tabular}{lp{0.7\textwidth}}
    ...
    \variable{air_temperature}, \variable{delta_T}, \variable{delta_SL}\\
    Options: & \texttt{-surface given,delta_T \mbox{-delta_T delta_T.nc}
      \mbox{-ocean constant,delta_SL} -delta_SL delta_SL.nc} \\
    Software   components:  &   \class{PSGivenClimate},  \class{PSdTforcing},
    \class{POConstant}, \class{POdSLforcing}\\
    See   also    &   \texttt{-surface~given}:   \ref{sec:surface-given},
    \texttt{delta_T}: \ref{sec:surface-delta-T}, \texttt{-ocean~constant}: \ref{sec:ocean-constant}, \texttt{dSLforcing}: \ref{sec:delta-sea-level}\\
    \bottomrule
  \end{tabular}
\end{center}
...
surface mass balance \variable{climatic_mass_balance} and surface temperature \variable{air_temperature} from \texttt{state.nc}.
...
in variables \variable{climatic_mass_balance}, \variable{air_temperature}, reported back to us as the time- and space-dependent
...
model uses a variable called \variable{precipitation}, and a calculation of melting, to get the
surface mass balance \variable{climatic_mass_balance}.
...
\end{verbatim}%$
produces \texttt{foo.nc}. Viewing in with \texttt{ncview} shows an annual cycle in the variable \variable{air_temperature} and a noticeable decrease in the surface mass balance during summer months (see variable \variable{climatic_mass_balance}). Note that \variable{air_temperature} is constant in time: this is the temperature \emph{at the ice surface but below firn} and it does not include seasonal variations \cite{Hock05}.
...
\index[options]{SA@\surface!\texttt{simple}}
...
the firn.) This implies that there is no melt. Though primitive, this model may
...
User's Manual, table \ref*{userman-tab:ice-dynamics-bc}). Variables
\variable{ice_surface_temp} (ice temperature at the ice surface but below firn)
and \variable{climatic_mass_balance} (top surface mass flux into the ice) are
read from the file, so this choice will cause PISM to stop if they are not
present in the input file.

The name of the file PISM will read \variable{ice_surface_temp} and
\variable{climatic_mass_balance} from is specified using the
\fileopt{surface_given_file} option.
...
This model was created to force PISM with sampled (possibly periodic) climate
data, e.g. using monthly records of \variable{ice_surface_temp} and
\variable{climatic_mass_balance}.

Option \txtopt{surface_given_period}{years} makes PISM interpret data in
\texttt{-surface_given_file} as periodic. In this case the time in the NetCDF
file is understood as the time \emph{from the beginning of a period}, i.e. from
the beginning of a year with \texttt{-surface_given_period 1}, from the
beginning of a decade with \texttt{-surface_given_period 10}, etc.
...
Note that PISM cannot compute month lengths, so you need to provide this
information using the time variable or the \variable{time_bounds} variable. In
this case it is a good idea to use units of ``\texttt{days}''.

To force PISM using monthly records with longer periods, just add more records
to the \texttt{-surface_given_file} and change the
\texttt{-surface_given_period} value.
...
\index[options]{SA@\surface!\texttt{elevation}}
...
This surface model parameterizes the ice surface temperature $T_{h}$ =
\variable{air_temperature} and the surface mass balance $m$ =
...
\index[options]{SA@\surface!\texttt{pdd}}

\index{temperature-index surface processes model} \index{positive degree day surface processes model} \index{PDD (positive degree day model)}

The default PDD used by PISM, turned on by option \texttt{-surface pdd}, is based on \cite{CalovGreve05} and EISMINT-Greenland inter-comparison (see User's Manual, section \ref*{userman-sec:eismint-greenland}, and \cite{RitzEISMINT}).

Our PDD implementation is meant to be used with an atmosphere model
implementing a cosine yearly cycle such as \texttt{searise_greenland} (see
section \ref{sec:atmosphere-searise-greenland}) and \texttt{eismint_greenland}
(see section \ref{sec:atmosphere-eismint-greenland}), but is not restricted to
parameterizations like this one. A PDD generally adds ``white noise'' to the
seasonal cycle to simulate additional daily variability associated to the
vagaries of weather. This additional random variation is quite significant, as
the seasonal cycle may never reach the melting point but that point may be
reached with some probability, in the presence of the daily variability, and
thus melt may occur. Concretely, a normally-distributed, mean zero random
temperature increment is added to the seasonal cycle. There is no assumed
spatial correlation of daily variability. The standard deviation of the daily
variability is controlled by the \intextoption{pdd_std_dev} option, and the
corresponding configuration parameter has the same name. The default value is
5.0 degrees \cite{RitzEISMINT}.
...
\index[options]{SA@\surface!\texttt{pik}}

This surface model implements the setup used in \cite{Martinetal2011}. The
...
\index[options]{SB@\surfacemods!\texttt{delta_T}}
...
\index[options]{SB@\surfacemods!\texttt{lapse_rate}}
...
\index[options]{SB@\surfacemods!\texttt{forcing}}
...
In this case \texttt{foo.nc} contains fields \variable{climatic_mass_balance} and \variable{air_temperature},
...
\index[options]{SB@\surfacemods!\texttt{anomaly}}
...
\item \txtopt{surface_anomaly_period}{years} specifies the period of the
  forcing data, in model years; see section \ref{sec:periodic-forcing}
\item \txtopt{surface_anomaly_reference_year}{year} specifies the reference
  year; see section \ref{sec:periodic-forcing}.
\end{itemize}

See also to \texttt{-atmosphere~...,anomaly} (section
\ref{sec:atmosphere-anomaly}), which is similar, but applies anomalies at the
atmosphere level.
...
\index[options]{\atmosphere!\texttt{searise_greenland}}
...
It expects variables \variable{precipitation}, latitude (\variable{lat}), and longitude (\variable{lon}) to be present in an input file.

\subsection{Reading atmosphere boundary conditions from a file}
\label{sec:atmosphere-given}
\optsection{\atmosphere!\texttt{given}}
\index[options]{\atmosphere!\texttt{given}}
\index{One-way coupling to a climate model}

This atmosphere model reads forcing fields
\begin{itemize}
\item \variable{air_temperature} and
\item \variable{precipitation} (in units of ice-equivalent thickness per time)
\end{itemize}
given by the user from a file and provides them to a surface model.

The name of the file PISM will read \variable{air_temperature} and \variable{precipitation} from is specified using the \fileopt{atmosphere_given_file} option.

A file \texttt{foo.nc} used with \texttt{-atmosphere given -atmosphere_given_file foo.nc} should contain several records;\footnote{If this file contains one record (i.e. fields corresponding to one time value only), \texttt{-atmosphere given} is essentially equivalent to \texttt{-atmosphere constant}.} the \variable{time} variable should describe what model time these records correspond to.

This model was created to force PISM with sampled (possibly periodic) climate data, e.g. using monthly records of \variable{air_temperature} and \variable{precipitation}.
...
\index[options]{\atmospheremods!\texttt{delta_T}}
...
\index[options]{\atmospheremods!\texttt{lapse_rate}}
...
\index[options]{\atmospheremods!\texttt{anomaly}}
...
PISM Ocean models provide sub-shelf ice temperature (\variable{shelfbtemp}) and sub-shelf mass flux (\variable{shelfbmassflux}) to the ice dynamics core.
...
\index[options]{OA@\ocean!\texttt{constant}}
...
This ocean model reads sub-shelf ice temperature \variable{shelfbtemp} and the sub-shelf mass flux \variable{shelfbmassflux} from a file. It takes the following command-line options.
\begin{itemize}
...
\subsection{PIK}
\label{sec:ocean-pik}
\index[options]{OA@\ocean!\texttt{pik}}
...
\begin{verbatim}
-ocean constant,delta_SL,delta_T,delta_SMB ...
\end{verbatim}
...
\index[options]{OB@\oceanmods!\texttt{delta_SL}}

The ocean \texttt{delta_SL} modifier implements sea level forcing using scalar offsets.

It takes the following command-line options:
\begin{itemize}
\item \fileopt{ocean_delta_SL_file}: specifies the name of the file containing forcing data. This file has to contain the \variable{delta_T} variable using units of ice-equivalent thickness per time.
...
\index[options]{OB@\oceanmods!\texttt{delta_T}}

This modifier implements forcing using sub-shelf ice temperature offsets.

It takes the following command-line options:
\begin{itemize}
\item \fileopt{ocean_delta_T_file}: specifies the name of the file containing forcing data. This file has to contain the \variable{delta_T} variable using units of ice-equivalent thickness per time.
...
\index[options]{OB@\oceanmods!\texttt{delta_SMB}}
