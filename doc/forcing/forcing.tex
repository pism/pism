...
The current manual describes the PISM ``climate forcing'' components that
implement those interfaces.  It documents the options that control
these components.  Notice that we distinguish between portions of PISM code
which are related to coupling and climate forcing, namely ``components'',
and the physical models which the components incorporate.  Models
are science while components are just pieces of code.  Options
choose components and these implement models for how climate forcing fields
...
ocean component.  The surface component may use an atmosphere
component also; see Figure \ref{fig:climate-input-data-flow-repeated}.  Command-line options \texttt{-atmosphere}, \texttt{-surface} and
...
explicitly choose a model component to use a modifier.  Model components and modifiers
...
command-line options, respectively, and FIXME: ARGUMENTS IN MODEL TIME?
Note that here \texttt{-y 1} has the
...
date, i.e. the date corresponding to time $0$.  For example, to run for $20$
years starting on January 1, $1989$, and override time read from an input file, do
\begin{verbatim}
...
FIXME: I THINK YOU INTEND TO SAY: THIS IS A USAGE EXAMPLE BUT INSTEAD YOU SHOW A TABLE FIRST
