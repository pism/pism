...
\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    ...
    &\variable{climatic_mass_balance} \massfluxunits \\
...
\subsection{Modifier: Spatial SMB gradients}
\label{sec:spatial-smb-gradients}
\optsection{SB@\surfacemods!\texttt{spatial_smb_gradients}}

\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    \toprule
    Command-line option: & \texttt{-surface~...,spatial_smb_gradients} \index[options]{SB@\surfacemods!\texttt{spatial_smb_gradients}} \\
    NetCDF variables: & \variable{surface_altitude} (CF standard name), \\
    C++ class: & \class{PSLapseRates}\\
    \bottomrule
  \end{tabular}
...
The \texttt{spatial_smb_gradients} modifier allows correcting ice-surface temperature and a parameterization of surface mass balance gradients \cite{Edwards2014}. It uses the following options.
...
\item \intextoption{temp_lapse_rate} gives the temperature lapse rate, in
  ...
  \begin{displaymath}
    \gamma = -\frac{dT}{dz}.
  \end{displaymath}
\item \intextoption{spatial_smb_gradients} \texttt{grad_ns,grad_ps,grad_nn,grad_pn,ns_lat}, where \texttt{grad_ns} is the SBM gradient south of \texttt{ns_lat} if SMB$<$0, \texttt{grad_ps} is the SBM gradient south of \texttt{ns_lat} if SMB$>$0, \texttt{grad_ns} is the SBM gradient north of \texttt{ns_lat} if SMB$<$0, \texttt{grad_ns} is the SBM gradient north of \texttt{ns_lat} if SMB$>0$, and \texttt{ns_lat} is the latitude that separates the north and south values. Units for the gradients are $kg/(m^2\,yr)$.
\item \fileopt{surface_lapse_rate_file} specifies the file containing the reference
  surface elevation field (standard name: \variable{surface_altitude}). This file
  can contain several surface elevation records to use lapse rate corrections
  relative to time-dependent surface. If one record is provided, the reference
...
\subsection{Modifier: Scalar melange back pressure fraction}
\label{sec:delta-subshelf-smb}
\optsection{OB@\oceanmods!\texttt{frac_MBP}}

\begin{center}
  \begin{tabular}{rp{0.5\linewidth}}
    \toprule
    Command-line option: & \texttt{-ocean~...,frac_MBP} \index[options]{OB@\oceanmods!\texttt{frac_MBP}} \\
    NetCDF variables: & \variable{frac_MBP} \\
    C++ class: & \class{PO_frac_MBP}\\
    \bottomrule
  \end{tabular}
\end{center}

This modifier implements forcing using melange back pressure fraction offsets. The variable \texttt{frac_MBP} should take on values from 0 to 1; it is understood as the fraction of the maximum melange back pressure possible at a given location. (We assume that melange back pressure cannot exceed the pressure of the ice column at a calving front.)
...
\begin{itemize}
\item \fileopt{ocean_frac_MBP_file}: specifies the name of the file containing forcing data. This file has to contain the \variable{frac_MBP} variable using units of ``1'' (a dimensionless parameter)
\item \intextoption{ocean_frac_MBP_period} specifies the length of the period of the forcing data, in model years; see section~\ref{sec:periodic-forcing}.
\item \intextoption{ocean_frac_MBP_reference_year} specifies the reference date; see section~\ref{sec:periodic-forcing}.
\end{itemize}
