...
Note that the ``\texttt{make all}'' puts executables, including \texttt{pross},\index{pross} \texttt{pismr},\index{pismr} \texttt{pismv},\index{pismv} and \texttt{pisms},\index{pisms} in the \texttt{pism-dev/bin/} subdirectory.  Object files created during the build process (located in the \texttt{build} subdirectory) are not deleted, so do ``\texttt{make clean}'' to delete these if space is an issue.
