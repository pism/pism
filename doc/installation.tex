...
\newcommand{\PETSCREL}{2.3.3 or 3.0.0 or 3.1.0}
...
\subsubsection{Installing PISM using the CMake build system}
\label{sec:install-cmake}

It is also possible to build and install PISM using CMake.

To use this route, get PISM sources as described above.
\begin{verbatim}
$  svn co http://svn.gna.org/svn/pism/tags/stable0.3 pism0.3
$  cd pism0.3
\end{verbatim}

Then create a directory to build in (CMake allows and recomends out-of-source builds) and make sure that \texttt{PETSC_ARCH} and
\texttt{PETSC_DIR} variables are set.
\begin{verbatim}
$  mkdir cmake-build
$  cd cmake-build
\end{verbatim}

Now, run the following commands to configure, build and install PISM.
\begin{verbatim}
$  cmake ..
$  make install
\end{verbatim}%$

The default install prefix is \texttt{/usr/local}. To install executables in \texttt{/home/user/bin}, libraries in
\texttt{/home/user/lib}, etc do
\begin{verbatim}
$  PISM_INSTALL_PREFIX=/home/user cmake ..
$  make install
\end{verbatim}%$

Running \texttt{ccmake} instead of \texttt{cmake} displays a GUI allowing changing the installation directory and other build
parameters.
...
\item The use of \href{http://www.macports.org/}{MacPorts} or \href{http://www.finkproject.org/}{Fink} is recommended, as it significantly simplifies installing many
  ...
  (\href{http://www.finkproject.org/download/index.php}{Fink homepage}, install set the environment:
...
$  make all test
\end{verbatim}
\item Finally, to build PISM, you should use CMake. Run
\begin{verbatim}
$  mkdir cmake-build
$  cmake ..
...
Please refer to \ref{sec:pism-itself} for details.
...
FIXME: describe modifying compiler and linker flags with CMake.
