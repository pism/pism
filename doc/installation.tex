...
Now, run CMake to configure PISM and build.
\begin{verbatim}
$  cmake ..
$  make
\end{verbatim}
CMake build system does \emph{not} install executables and libraries by default, so the final step is
\begin{verbatim}
$  DESTDIR=/some/path make install
\end{verbatim}%$
with \texttt{/some/path} replaced with a directory you want PISM in.
