...
package manager to get the prerequisites for PISM.
...
Regarding the final group, note that some form of MPI and
BLAS and LAPACK are needed, but that if you will configure PETSc with
option \texttt{--download-mpich=1} and so on (see below), which PETSc generally recommends
and which works well for PISM, then you may not need to get these packages.
\begin{center}
