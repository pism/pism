...
Optional libraries are needed for certain PISM features, namely earth deformation, cell-area correction, and parallel I/O.  These libraries are recommended, but not strictly required: \bigskip
...
\subsubsection{Installing prerequisites using a single meta-package (Debian)}
\label{sec:deb-pism-toolkit}

If you are using a Debian-based Linux system such as Ubuntu, you can download and
...
Even if you cannot use the meta-package above you may still be able to use your
...
option \texttt{--download-mpich=1} or similar, which PETSc generally recommends
and which works well for PISM if you do not already use OpenMPI.
...
\item PISM optionally uses the \href{http://www.fftw.org/}{FFTW-3
...
\item You will need a version of \href{http://www-unix.mcs.anl.gov/mpi/}{MPI (=
  ...
  configure process (see \ref{subsec:prereq-petsc}).
  In any case, the goal is to have the PETSc installation use the
...
\item If there is an existing MPI\index{MPI (\emph{Message Passing Interface})} installation, for example at \texttt{/home/user/mympi/}, one can point PETSc to it by adding the option ``\texttt{--with-mpi-dir=/home/user/mympi/}''.  The path used in this option must have MPI executables \texttt{mpicxx} and \texttt{mpicc}, and either \texttt{mpiexec} or \texttt{mpirun}, in sub-directory \texttt{bin/} and MPI library files in sub-directory \texttt{lib/}.  On the other hand, the user who only needs MPI and PETSc for running PISM, and other users as well, may want to follow the PETSc recommendation to have PETSc download MPI\index{MPI (\emph{Message Passing Interface})} into a place it understands; do \texttt{--download-mpich=1}.  If you get messages suggesting that PETSc cannot configure using your existing MPI, you might try also try this option.
...
$  make install
\end{verbatim}
Here \texttt{pism0.5} is the directory containing PISM source code while \texttt{\textasciitilde/pism} is the directory PISM will be installed into. All the temporary files created during the build process will be in \texttt{pism0.5/build} created above.
...
$  ./config/configure.py --download-mpich=1 --download-f2cblaslapack=yes \
