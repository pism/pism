...
Note that the ``\verb|make all|'' puts executables, including \verb|pross|,\index{pross} \verb|pismr|,\index{pismr} \verb|pismv|,\index{pismv} and \verb|pisms|,\index{pisms} in the \verb|pism-dev/bin/| subdirectory.  Object files created during the build process (located in the \verb|build| subdirectory) are not deleted, so do ``\verb|make clean|'' to delete these if space is an issue.
...
The above ``\verb|-n 4|'' run should work even if there is only one actual processor (core) on your machine, in which case MPI will just run multiple processes on the one processor.  This run will also produce a NetCDF output file \verb|verify.nc|, which can be read and viewed by NetCDF tools.

\item Try an EISMINT II run using the PETSc viewers (under the X window system):

\verb|$ pisms -y 5000 -view_map thk,temppabase,csurf|
...
\verb|$ mpiexec -n 2 pisms -y 5000 -view_map thk,temppabase,csurf -display :0|

\item Run a a basic suite of software tests

\verb|$ (cd test/software && make)|
