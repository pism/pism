...
import numpy as np
from pylab import clf, figure, hold
...
xcf = (
    (Hcf**-4 - H0**-4) * U0 * H0 / (4 * C * spa) * 0.001
)  # find 250m front position on 5km grid
ucf = (
    U0 * H0 * (xcf * 1000.0 * 4 * C / (U0 * H0 / spa) + H0 ** (-4)) ** (0.25)
)  # exact (maximal) velocity at front
...
for k in range(len(xan)):
    if xan[k] <= xcf:
        ...
        Uan[k] = (
            U0
            * H0
            * (k * dxan * 1000.0 * 4 * C / (U0 * H0 / spa) + H0 ** (-4)) ** (0.25)
        )
...
hold(True)
plt.subplots_adjust(
    left=0.14, bottom=0.14, right=0.86, top=None, wspace=0.08, hspace=0.15
)
plt.clf()
title = (
    "steady state ice thickness and velocity for "
    + str(int(dx / 1000.0))
    + " km after "
    + str(t)
    + " yrs"
