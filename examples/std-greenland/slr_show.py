...
parser.description = (
    "A script for PISM output files to show time series plots using pylab."
)
parser.add_option(
    "-a",
    dest="t_a",
...
NCNAMES = [
    model + "_G_D3_C1_E0.nc",
    model + "_G_D3_C2_E0.nc",
    model + "_G_D3_C3_E0.nc",
    model + "_G_D3_C4_E0.nc",
    model + "_G_D3_C1_S1.nc",
    model + "_G_D3_C1_S2.nc",
    model + "_G_D3_C1_S3.nc",
    model + "_G_D3_C1_M1.nc",
    model + "_G_D3_C1_M2.nc",
    model + "_G_D3_C1_M3.nc",
    model + "_G_D3_C1_T1.nc",
]
...
for j in range(n - 1):
    ax.plot(
        t, -(ivolshift[:, j] / 1.0e9) * scale, dashes[j], color=colors[j], linewidth=3
    )
...
for j in range(n - 1):
    ax.plot(
        t, -(ivolshift[:, j] / 1.0e9) * scale, dashes[j], color=colors[j], linewidth=2
    )
ax.set_xlabel("years from 2004")
ax.set_ylabel("sea level rise relative to control (m)")
ax.legend(labels, loc="upper left")
