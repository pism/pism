...
NAME=cont.nc
cmd="$MPIDO pism -i $INNAME -skip -skip_max 20 $CLIMATE $PHYS $CALVING -scalar_file ts_$NAME -scalar_times 0:yearly:$DURATION -y $DURATION -o $NAME"
#$PISM_DO $cmd
echo
...
cmd="$MPIDO pism -i $INNAME -no_mass -energy none -stress_balance none $CLIMATE -spatial_file ex_$NAME -spatial_times 0:monthly:$DURATION -spatial_vars $EXVAR -scalar_file ts_$NAME -scalar_times 0:daily:$DURATION -hydrology routing -hydrology_bmelt_file $INNAME -ys 0 -y $DURATION -max_dt 0.03 -o $NAME"
$PISM_DO $cmd
echo
