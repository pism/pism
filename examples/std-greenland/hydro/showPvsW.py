...
parser = argparse.ArgumentParser(
    description="show scatter plot P versus W from a PISM run"
)

parser.add_argument(
    "filename", help="file from which to get  P = bwprel  and  W = bwat"
)
parser.add_argument(
    "-o",
    default=None,
...
parser.add_argument(
    "-c", default=None, help="name of variable to use to color points in scatter plot"
)
parser.add_argument(
    "-cmin", type=float, default=None, help="crop color values below at this value"
)
parser.add_argument(
    "-cmax", type=float, default=None, help="crop color values above at this value"
)
parser.add_argument(
    "--colorbar", action="store_true", help="whether to show colorbar() beside figure"
)
...
parser.add_argument("-wmin", type=float, default=None, help="lower limit on W axis")
parser.add_argument("-wmax", type=float, default=None, help="upper limit on W axis")
...
if args.s != None:
    print(
        "  reading '%s' field, for point selection, from file %s ..."
        % (args.s, args.filename)
    )
    try:
...
if args.c != None:
    print(
        "  reading '%s' field, for point color, from file %s ..."
        % (args.c, args.filename)
    )
    try:
...
plt.text(
    0.07,
    0.25,
    "%d" % int(args.smin) + r"$ < |\mathbf{v}_b| < $" + "%d" % int(args.smax),
)
if args.colorbar:
    plt.colorbar()
plt.xlabel(r"$W$  (m)")
