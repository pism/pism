...
def preprocess_ice_velocity():
    ...
    if not os.path.exists(input_filename):
        print(
            "Please download the InSAR velocity dataset from https://nsidc.org/data/NSIDC-0484/versions/2"
        )
        print(
            "See overview of MEaSUREs data products at https://nsidc.org/data/measures/aiv"
        )
        exit(1)
    ...
    if not os.path.exists(output_filename):
        ...
        cmd = "ncks -d x,3500,7500 -d y,6000,10000 -O %s %s" % (
            input_filename,
            output_filename,
        )
        run(cmd)
        ...
        run(
            "cdo setmisstoc,0 -setattribute,VX@_FillValue=-1.0f -setattribute,VY@_FillValue=-1.0f {} {}".format(
                output_filename, tmp_filename
            )
        )
        run(
            "cdo chname,VX,vx -chname,VY,vy {} {}".format(tmp_filename, output_filename)
        )
...
def preprocess_albmap():
    ...
    commands = [
        "wget -nc %s" % url,  # download
        "unzip -n %s.zip" % input_filename,  # unpack
        # modify this command to cut out a different region
        "ncks -O -d x1,439,649 -d y1,250,460 %s %s"
        % (input_filename, output_filename),  # cut out
        "ncks -O -v usrf,lsrf,topg,temp,acca,mask %s %s"
        % (output_filename, output_filename),  # trim
        "ncrename -O -d x1,x -d y1,y -v x1,x -v y1,y %s"
        % output_filename,  # fix metadata
    ...
    nc.projection = (
        "+proj=stere +ellps=WGS84 +datum=WGS84 +lon_0=0 +lat_0=-90 +lat_ts=-71 +units=m"
    )
    nc.close()
...
if __name__ == "__main__":
    ...
    albmap_velocity = (
        os.path.splitext(albmap)[0] + "_velocity.nc"
    )  # ice velocity on the ALBMAP grid
    output = "Ross_combined.nc"

    commands = [
        "./nc2cdo.py %s" % velocity,
        "./nc2cdo.py %s" % albmap,
