...
parser = argparse.ArgumentParser(
    description="Create run script for a MISMIP3d experiment.",
...
parser.add_argument(
    "--mpiname", metavar="NAME", default="mpiexec", help="name of mpi executable"
)
parser.add_argument(
    "-n",
    metavar="N",
...
parser.add_argument(
    "-s",
    "--subgl",
    action="store_true",  # thus defaults to False
...
accumrate=%s"""
    % (args.e, args.m, args.r, args.accumrate)
)
...
if args.e in {"Stnd", "P10S", "P75S"}:
    ...
    print(
        "# NOT creating bootstrap file since experiment %s starts from previously-saved state"
        % args.e
    )
...
print("")
print(
    'listexvar="thk,topg,velbar_mag,flux_mag,mask,dHdt,usurf,hardav,velbase,velsurf,velbar,wvelbase,wvelsurf,tauc,deviatoric_stresses,climatic_mass_balance$gl_mask"'
)
if args.e == "Stnd":
...
print("")
print(
    'stressbalance="-ssa_method fd -ssa_flow_law isothermal_glen -ssafd_ksp_rtol 1e-7"'
)
print(
    'basal="-yield_stress constant -pseudo_plastic -pseudo_plastic_q 0.333333333 -pseudo_plastic_uthreshold 3.155693e+07"'
)
...
print("")
print(
    'opts="-config_override MISMIP3D_conf.nc $stressbalance $basal $calvingfront $subgl $modelopt -energy none -gradient eta -options_left -ts_file ts_%s.nc -ts_times 0:1:$duration -extra_file ex_%s.nc $extrastuff -ys 0 -ye $duration -o_order zyx -o_size big -o %s.nc $STRONGKSP"'
    % (args.e, args.e, args.e)
)
...
if args.e == "Stnd":
    ...
    print(
        'cmd="$pismr -i $infile -bootstrap -Mx $Mx -My 3 -Mz 15 -Lz 6000 -tauc 1.0e7 -front_retreat_file $infile $opts"'
    )
elif args.e == "P10S":
    print("infile=MISMIP3D_P10S_initialSetup.nc")
    print(
        'cmp="$pismr -i $infile -bootstrap -Mx $Mx -My $My -Mz 15 -Lz 6000 -front_retreat_file $infile $opts"'
    )
elif args.e == "P10R":
    ...
    print(
        'cmd="$pismr -i $infile -bootstrap -Mx $Mx -My $My -Mz 15 -Lz 6000 -front_retreat_file $infile $opts"'
    )
elif args.e == "P75R":
