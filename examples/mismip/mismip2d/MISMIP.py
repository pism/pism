...
def b(experiment, x):
    ...
    if experiment in ("1a", "1b", "2a", "2b"):
        return -720.0 + 778.5 * (x / 7.5e5)

    if experiment in ("3a", "3b"):
        xx = x / 7.5e5
        return -(729.0 - 2184.8 * xx**2.0 + 1031.72 * xx**4.0 - 151.72 * xx**6.0)
...
def b_slope(experiment, x):
    ...
    if experiment in ("3a", "3b"):
        ...
        return -(
            -2184.8 * (2.0 / 7.5e5) * xx
            + 1031.72 * (4.0 / 7.5e5) * xx**3.0
            - 151.72 * (6.0 / 7.5e5) * xx**5.0
        )
...
def x_g(experiment, step, theta=0.0):
    ...
    delta_x = 10.0  # Finite difference step size (metres) for gradient calculation
    tolf = 1.0e-4  # Tolerance for finding zeros
    eps = np.finfo(float).eps
    normf = tolf + eps
    toldelta = 1.0e1  # Newton step size tolerance
    dx = toldelta + 1.0
...
def thickness(experiment, step, x, theta=0.0):
    ...
    def surface(h, x):
        ...
        rho_g = rho_i() * g()
        s = a() * np.abs(x)
        return b_x - (C(experiment) / rho_g) * s ** m(experiment) / h ** (
            m(experiment) + 1
        )
    ...
    thk_grounded = scipy.integrate.odeint(
        surface, [h_f], x_grid, atol=1.0e-9, rtol=1.0e-9
    )
...
def plot_profile(experiment, step, out_file):
    ...
    plot(
        x / 1e3,
        np.r_[
            thk_grounded - b(experiment, x_grounded),
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-s", "--step", dest="step", type="int", default=1, help="MISMIP step number"
    )
