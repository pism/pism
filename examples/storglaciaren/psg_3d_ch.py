...
parser.add_argument(
    ...
    default=abspath(script_directory),
)
parser.add_argument(
    "--o_dir", dest="output_dir", help="output directory", default="output"
)
parser.add_argument(
    "--o_size",
    dest="osize",
...
parser.add_argument(
    "--cryohydrologic_warming",
    dest="cryohydrologic_warming",
    action="store_true",
    help="Turn on cryohydrologic warming",
    default=False,
)
...
cmd = "ncgen -o {output} {input_dir}/{config}.cdl".format(
...
try:
    ...
    try:
        combinations = np.genfromtxt(
            ensemble_file, dtype=None, delimiter=",", skip_header=1, encoding=None
        )
    except:
        combinations = np.genfromtxt(
...
for n, combination in enumerate(combinations):
    ...
    full_exp_name = "_".join(
        ["_".join(["_".join([k, str(v)]) for k, v in name_options.items()])]
    )
    ...
    with open(script_combined, "w") as f_combined:
        ...
        for start in range(simulation_start_year, simulation_end_year, restart_step):
            ...
            with open(script, "w") as f:
                ...
                all_params = " \\\n  ".join(
                    ["-{} {}".format(k, v) for k, v in all_params_dict.items()]
                )
