...
parser.add_argument(
    "-m", "--desired_misfit", required=True, type=float, help="desired final misfit"
)
parser.add_argument(
    "-i", "--iter_max", required=True, type=int, help="maximum number of iterations"
)
parser.add_argument(
    "-z",
    "--morozov",
...
try:
    ...
    fail(
        'Unable to open inversion file "%s"\nPerhaps the inversion run failed to converge, or terminated unexpectedly.'
        % inv_filename
    )
...
if misfit is None:
    fail(
        'Inversion file %s missing misfit history variable "inv_ssa_misfit".\nPerhaps the inversion run failed to converge, or terminated unexpectedly.'
        % inv_filename
    )
...
if iter_count > args.iter_max:
    fail(
        "Inversion took an excessive number of iterations: %d > %d"
        % (iter_count, args.iter_max)
    )
...
if args.misfit_tolerance:
    ...
    if d_misfit > args.misfit_tolerance:
        fail(
            "Final misfit too far from desired misfit: |final-desired| = |%0.4g-%0.4g| = %0.4g > %0.4g = desired"
            % (last_misfit, desired_misfit, d_misfit, args.misfit_tolerance)
        )
else:
    if last_misfit >= desired_misfit:
        fail(
            "Desired final misfit not met: computed = %0.4g >= %0.4g = desired"
            % (last_misfit, desired_misfit)
        )
...
if args.morozov:
    ...
    if penultimate_misfit < desired_misfit:
        fail(
            "Penultimate misfit too small: computed = %0.4g < %0.4g = desired"
            % (penultimate_misfit, desired_misfit)
        )
