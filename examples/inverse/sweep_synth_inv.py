#!/usr/bin/env python3
"""Generate inversion run scripts for synthetic observations.

Usage:
    python sweep_synth_inv.py debug    # mpirun scripts
    python sweep_synth_inv.py sbatch   # SLURM scripts
"""
...
for sb, params in solvers.items():
    for penalty, h1, l2, hscale, vscale in itertools.product(
        ...
        obs = f"synth_obs_{sb}_g{res}m_RGI2000-v7.0-C-01-04374_id_0_{start}_{end}.nc"
        outfile = f"inv_synth_{sb}_it_{max_iter}_p_{penalty}_h1_{h1}_l2_{l2}_ls_{hscale}_vs_{vscale}.nc"
        jobname = f"inv_synth_{sb}_p_{penalty}_h1_{h1}_l2_{l2}_ls_{hscale}_vs_{vscale}"
