...
parser.add_option("-i", "--input_file", type="string", help="input file")
parser.add_option(
    "-c",
    "--tauc_cap",
    type="float",
...
not_sliding = np.logical_and((abs(u_computed) < 10.0), (abs(v_computed) < 10.0))
tauc[not_ice] = 0
tauc_true[not_ice] = 0
tauc_diff[not_sliding] = 0.0


# difference figure
pp.clf()
pp.imshow(
    tauc_diff.transpose() / tauc_true.transpose(),
    origin="lower",
    vmin=-options.tauc_error_cap,
...
pp.figure()
im = pp.imshow(
    velbase_mag_computed.transpose(),
    origin="lower",
