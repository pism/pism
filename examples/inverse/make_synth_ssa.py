...
if __name__ == "__main__":
    ...
    usage = """  %s -i IN.nc -Mx number -My number [-o file.nc]
  or (at python prompt)
  ...
  """ % (
    ...
    design_prior_const = (
        design_prior_const.value() if design_prior_const.is_set() else None
    )

    noise = PISM.OptionReal(
        sys, "-rms_noise", "pointwise rms noise to add (in m/a)", "m / year", 0.0
    )
    noise = noise.value() if noise.is_set() else None

    misfit_weight_type = PISM.OptionKeyword(
        "-misfit_type", "Choice of misfit weight function", "grounded,fast", "grounded"
    ).value()

    fast_ice_speed = PISM.OptionReal(
        sys,
        "-fast_ice_speed",
    ...
    generate_ssa_observed = PISM.OptionBool(
        "-generate_ssa_observed", "generate observed SSA velocities"
    )
    ...
    if design_var == "tauc":
        ...
        hardav_prior = PISM.model.createAveragedHardnessVec(
            grid,
            name="hardav_prior",
    ...
    if design_var == "tauc":
        ...
        ice_factory = PISM.IceFlowLawFactory(
            grid.com, "stress_balance.ssa.", config, EC
        )
        ...
        averaged_hardness_vec(
            flow_law, vecs.land_ice_thickness, vecs.enthalpy, vecs.hardav
        )
        ...
        hardav_true.metadata(0).long_name(
    ...
    if generate_ssa_observed:
        ...
        vel_sia_observed.metadata(0).long_name(
            "x-component of the 'observed' SIA velocities"
        )

        vel_sia_observed.metadata(1).set_name("v_sia_observed")
        vel_sia_observed.metadata(1).long_name(
            "y-component of the 'observed' SIA velocities"
        )

        vel_surface_observed = PISM.model.create2dVelocityVec(
            grid, "_surface_observed", "observed surface velocities", stencil_width=1
        )
        vel_surface_observed.copy_from(vel_sia_observed)
        vel_surface_observed.add(1.0, vel_ssa_observed)
    ...
    if not noise is None:
        u_noise = PISM.vec.randVectorV(
            grid, noise / math.sqrt(2), final_velocity.stencil_width()
        )
