...
def run():
    ...
    usage = """  pismi.py [-i IN.nc [-o OUT.nc]]/[-a INOUT.nc] [-inv_data inv_data.nc] [-inv_forward model]
   ...
   """
    ...
    do_final_plot = PISM.OptionBool(
        "-inv_final_plot", "perform visualization at the end of the computation"
    )
    Vmax = PISM.OptionReal(
        context.unit_system,
        "-inv_plot_vmax",
    ...
    prep_module = PISM.OptionString(
        "-inv_prep_module", "Python module used to do final setup of inverse solver"
    )
    ...
    zeta.metadata(0).long_name("zeta_inv").units("1").output_units("1").standard_name(
        "zeta_inv"
    )
    if do_restart:
    ...
    if reason.failed():
        PISM.logging.logError(
            "Inverse solve FAILURE:\n%s\n" % reason.nested_description(1)
        )
        quit()
...
try:  # pragma: no cover
    cov.stop()
    report = PISM.OptionBool(
        "-report_coverage", "save coverage information and a report"
    )
    if report:
        cov.save()
