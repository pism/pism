...
#
# Uses REAL observed velocities (ITS_LIVE) from obs_RGI2000-v7.0-C-01-04374_0.nc
#
# Usage:
#   bash sweep_obs_inv.sh debug    # generates scripts that run directly with mpirun
#   bash sweep_obs_inv.sh sbatch   # generates scripts for SLURM submission
#   bash sweep_obs_inv.sh          # defaults to debug
...
for sb in ssa hybrid blatter; do
    if [ "$sb" = "hybrid" ]; then
        inv_flag="-inv_design tauc -remove_sia"
            ...
            for l2 in 0 1 10; do
                for hscale in 1e3; do
                    for vscale in 100; do
                        ...
                        jobname=inv_obs_${sb}_p_${penalty}_h1_${h1}_l2_${l2}_ls_${hscale}_vs_${vscale}
