...
def test_linear_periodic():
    ...
    period = 1.0
    x_p = numpy.linspace(0, 0.9, 10) + 0.05
    y_p = numpy.sin(2 * numpy.pi * x_p)
    ...
    numpy.testing.assert_almost_equal(yy, yy_p)
...
def test_nearest_neighbor():
    ...
    xx = numpy.linspace(-0.9, 0.9, 10)
    yy = numpy.ones_like(xx) * (xx > 0)

    zz = PISM.Interpolation(PISM.NEAREST, x, xx).interpolate(y)

    numpy.testing.assert_almost_equal(yy, zz)
