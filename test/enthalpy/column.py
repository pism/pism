...
k = config.get_double("ice.thermal_conductivity")
c = config.get_double("ice.specific_heat_capacity")
...
def errors_advection_up(plot_results=True, T_final=1000.0, dt=100, Mz=101):
    ...
    config.set_double("ice.thermal_conductivity", 0.0)
    column = EnthalpyColumn(Mz, dt)
    config.set_double("ice.thermal_conductivity", k)
