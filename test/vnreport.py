...
from numpy import array, ceil, floor, log10, polyfit, polyval, unique
from pylab import (
    axis,
    clf,
    close,
...
class Plotter:
    ...
    def plot(self, x, vars, testname, plot_title):
        ...
        if sum(mask) < 2:
            print(
                "Skipping Test %s %s (not enough data to plot)" % (testname, plot_title)
            )
            return
        ...
        for v, c in zip(vars, colors):
            ...
            print(
                "Test {} {}: convergence rate: O(dx^{:1.4f})".format(
                    testname, name, p[0]
                )
            )
    ...
    def plot_tests(self, list_of_tests):
        for test_name in list_of_tests:
            ...
            if test_name in ["A", "B", "C", "D", "E", "F", "G", "H", "L"]:
                self.plot(
                    "dx",
                    ["maximum_thickness", "average_thickness"],
                    test_name,
                    "ice thickness errors",
                )
                self.plot(
                    "dx", ["relative_volume"], test_name, "relative ice volume errors"
                )
                self.plot(
                    "dx", ["relative_max_eta"], test_name, r"relative max eta errors"
                )
            ...
            if test_name == "E":
                self.plot(
                    "dx",
                    ["maximum_basal_velocity", "average_basal_velocity"],
                    "E",
                    r"basal velocity errors",
                )
                self.plot(
                    "dx",
                    ["maximum_basal_u", "maximum_basal_v"],
                    "E",
                    "basal velocity (ub and vb) errors",
                )
                self.plot(
                    "dx",
                    ["relative_basal_velocity"],
                    "E",
                    "relative basal velocity errors",
                )
            ...
            if test_name in ["F", "G"]:
                self.plot(
                    "dx",
                    ["maximum_sigma", "average_sigma"],
                    test_name,
                    "strain heating errors",
                )
                self.plot(
                    "dx",
                    [
                    ...
                    test_name,
                    "ice temperature errors",
                )

                self.plot(
                    "dx",
                    ["maximum_surface_velocity", "maximum_surface_w"],
                    test_name,
                    "maximum ice surface velocity errors",
                )
                self.plot(
                    "dx",
                    ["average_surface_velocity", "average_surface_w"],
                    test_name,
                    "average ice surface velocity errors",
                )

            # test I: plot only the u component
            if test_name == "I":
                self.plot(
                    "dy", ["relative_velocity"], test_name, "relative velocity errors"
                )
                self.plot(
                    "dy", ["maximum_u", "average_u"], test_name, "velocity errors"
                )

            # tests J and M:
            if test_name in ["J", "M"]:
                self.plot(
                    "dx", ["relative_velocity"], test_name, "relative velocity errors"
                )
                self.plot(
                    "dx",
                    [
            ...
            if test_name == "O":
                ...
                self.plot(
                    "dz", ["maximum_basal_melt_rate"], "O", "basal melt rate errors"
                )
            ...
            if test_name == "V":
                ...
                self.plot(
                    "dx", ["maximum_u", "average_u"], test_name, "velocity errors"
                )
...
available_tests = unique(
    array(list(map(lambda x: chr(int(x)), input_file.variables["test"][:])))
)
...
if len(available_tests) == 1:
    ...
    if tests_to_plot == None:
        print(
            """Please choose tests to plot using the -t option.
