...
# C and F.  Intended to do minimal amount of computation to show convergence 
...
do
   ./verify -test C -Mx $myMx -My $myMx -Mz 31 -y 15208.0 > _temp_result.txt 
   sed '/  history =/,+1!d' _temp_result.txt | sed 1d
   sed '/Actual ERRORS/,+4!d' _temp_result.txt
   date
done

echo "+++++++++ continuing with thermocoupled test F and Mx=61,91,121:"

for myMx in 61 91 121
do
   ./verify -test F -Mx $myMx -My $myMx -Mz $myMx -y 25000.0 > _temp_result.txt 
   sed '/  history =/,+1!d' _temp_result.txt | sed 1d
   sed '/Actual ERRORS/,+4!d' _temp_result.txt
   date
done
