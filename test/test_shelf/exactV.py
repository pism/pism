...
parser.add_argument(
    ...
    dest="variant",
    type=int,
    help="choose the 'model variant', choose from 0, 1, 2",
    default=2,
)
parser.add_argument(
    "-Mx", dest="Mx", type=int, help="number of grid points", default=201
)
parser.add_argument("-y", dest="y", type=float, help="run length", default=300)
parser.add_argument(
    "-s", dest="step_plot", action="store_true", help="use 'plt.step()' to plot"
)
...
if options.variant == 0:
    ...
    run_pismv(Mx, run_length, opt + " -max_dt 1", "out.nc")
    plot_pism_results(
        "out.nc", "Figure 6 (c-d) (-part_grid)", "green", same_figure=True
    )
elif options.variant == 2:
    opt += " -cfbc -part_grid -part_grid_reduce_frontal_thickness"
    run_pismv(Mx, run_length, opt, "out.nc")
    plot_pism_results(
        "out.nc", "Figure 6 (e-f) (-part_grid, reduce frontal thickness)", "blue"
    )

    opt = opt + extras
    run_pismv(Mx, run_length, opt + " -max_dt 1", "out.nc")
    plot_pism_results(
        "out.nc", "Figure 6 (e-f) (-part_grid)", "green", same_figure=True
    )
else:
