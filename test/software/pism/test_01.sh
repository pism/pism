...
files="baz.nc foo.nc joe.nc bar.nc"
dir=`pwd`
...
test_01 ()
{
    ...
    run -n 1 pisms -no_cold -Mx 6 -My 6 -Mz 5 -y 5000 -max_dt 500.0 -o baz.nc

    # run for ten years, fixed time step
    run -n 1 pismr -i baz.nc $OPTS -y 10 -o foo.nc

    # chain two five year runs, fixed time step
    run -n 1 pismr -i baz.nc $OPTS -y 5 -o joe.nc
