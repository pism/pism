...
files="test-G-out.txt verify.nc"
dir=`pwd`
...
run_test ()
{
    ...
    pismv -Mx 31 -My 31 $OPTS   > test-G-out.txt
    pismv -Mx 41 -My 41 $OPTS  >> test-G-out.txt

    # compare results
    diff test-G-out.txt - > /dev/null <<END-OF-OUTPUT
NUMERICAL ERRORS evaluated at final time (relative to exact solution):
geometry  :    prcntVOL        maxH         avH   relmaxETA
               0.780389   32.430771    7.148950    0.016288
...
NUMERICAL ERRORS evaluated at final time (relative to exact solution):
geometry  :    prcntVOL        maxH         avH   relmaxETA
               0.782888   30.763939    7.242053    0.015469
temp      :        maxT         avT    basemaxT     baseavT
