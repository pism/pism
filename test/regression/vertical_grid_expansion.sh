...
$PISM_PATH/pism_nccmp -x -v timestamp,step_counter,model_years_per_processor_hour,wall_clock_time,pism_config -r -t 1e-7 fixed-vertical-grid.nc ok-vertical-grid.nc
if [ $? != 0 ];
then
    exit 1
fi
