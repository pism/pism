...
def run_pism(opts):
    Mx = 21
    My = Mx
    cmd = f"{opts.PISM_PATH}/pismr -config_override testPconfig.nc -i inputforP_regression.nc -bootstrap -Mx {Mx} -My {My} -Mz 11 -Lz 4000 -hydrology distributed -report_mass_accounting -y 0.08333333333333 -max_dt 0.01 -no_mass -energy none -stress_balance ssa+sia -ssa_dirichlet_bc -o end.nc -verbose 1"
