...
$PISM_PATH/pism -eisII I -Mx 121 -My 61 -Mz 21 -y 0 -o foo-05.nc

# Bootstrap from this file and run for 0 years:
$MPIEXEC -n 2 $PISM_PATH/pism $OPTS -o bar-05.nc

# Change the variable order in foo-05.nc to z,y,x:
ncpdq -O -a z,y,x foo-05.nc foo-05.nc

# Bootstrap from this file and run for 0 years:
$MPIEXEC -n 2 $PISM_PATH/pism $OPTS -o baz-05.nc

set +e

# Compare bar-05.nc and baz-05.nc:
$PISM_PATH/pism_nccmp -x -v timestamp,step_counter,model_years_per_processor_hour,wall_clock_time,pism_config bar-05.nc baz-05.nc
if [ $? != 0 ];
then
    exit 1
fi
