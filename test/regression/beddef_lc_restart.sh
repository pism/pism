...
Mx=41
My=61
options="-bed_def lc -spatial_times 0,100,200,300 -spatial_vars dbdt,topg,thk -stress_balance none -energy none -bed_deformation.lc.update_interval ${dt} -max_dt ${dt}"
...
${mpi} ${pismr} ${options} -i out0.nc -o out1.nc -spatial_file ex1.nc -ys 0 -ye 150 -bootstrap ${grid}
# This run reads the last bed deformation update time from its input file and updates the
# bed at years 200 and 300.
${mpi} ${pismr} ${options} -i out1.nc -o out2.nc -spatial_file ex2.nc -ye 300

# run straight
#
# This run updates bed elevation at years 100, 200, and 300.
${mpi} ${pismr} ${options} -bootstrap ${grid} -i out0.nc -o out3.nc -spatial_file ex.nc -ys 0 -ye 300
