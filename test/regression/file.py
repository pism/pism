...
class File(TestCase):
    def test_empty_filename(self):
        for backend in backends:
            try:
                f = PISM.File(
                    ctx.com(), "", backend, PISM.PISM_READONLY, ctx.pio_iosys_id()
                )
                fail(backend)
            except RuntimeError:
                pass
    ...
    def test_nrecords(self):
        ...
        for F in self.files:
            for backend in backends:
                f = PISM.File(
                    ctx.com(), F, backend, PISM.PISM_READONLY, ctx.pio_iosys_id()
                )
                assert f.nrecords() == 1
                f.close()
    ...
    def test_dimension_type(self):
        ...
        for backend in backends:
            ...
            assert (
                f.dimension_type("unknown_axis", ctx.unit_system()) == PISM.UNKNOWN_AXIS
            )
    ...
    def test_define_variable(self):
        ...
        for backend in backends:
            ...
            try:
                f.define_variable(
                    "var_{}".format(backend), PISM.PISM_DOUBLE, ["y", "x"]
                )
                fail(backend)
            except RuntimeError:
                pass
            ...
            try:
                f.define_variable(
                    "var_{}_1".format(backend), PISM.PISM_DOUBLE, ["y", "x", "z"]
                )
                fail(backend)
            except RuntimeError:
                pass
    ...
    def test_find_variable(self):
        ...
        for backend in backends:
            ...
            assert f.find_variable(
                "other_name", "standard_name"
            ).found_using_standard_name
    ...
    def test_write_attribute_number(self):
        ...
        for backend in backends:
            ...
            f.write_attribute(
                "PISM_GLOBAL", "new_attribute", PISM.PISM_DOUBLE, (1.0, 2.0)
            )
    ...
    def test_write_attribute_string(self):
        ...
        for backend in backends:
            ...
            assert (
                f.read_text_attribute("PISM_GLOBAL", "new_global_attr") == "test_global"
            )
            f.close()
    ...
    def test_read_double_attribute(self):
        ...
        for backend in backends:
            ...
            assert f.read_double_attribute("PISM_GLOBAL", "global_double_attr") == (
                12.0,
            )
    ...
    def test_read_text_attribute(self):
        ...
        for backend in backends:
            ...
            assert (
                f.read_text_attribute("PISM_GLOBAL", "global_text_attr")
                == "test_global"
            )
    ...
    def setUp(self):
        ...
        vec.metadata(0).long_name("dummy variable for testing").units(
            "Kelvin"
        ).output_units("Celsius").standard_name("standard_name")
        vec.set(1.0)
        ...
        f = PISM.File(
            ctx.com(), self.file_with_time, PISM.PISM_NETCDF3, PISM.PISM_READWRITE
        )
...
class StringAttribute(TestCase):
    ...
    def test_read_string_attribute(self):
        ...
        for backend in backends:
            ...
            assert self.attribute == f.read_text_attribute(
                "PISM_GLOBAL", "string_attribute"
            )
            assert self.attribute == f.read_text_attribute(
                "PISM_GLOBAL", "text_attribute"
            )
            # multi-valued string attributes are turned into comma-separated lists
            assert "{0},{0}".format(self.attribute) == f.read_text_attribute(
                "PISM_GLOBAL", "string_multi_value"
            )
            f.close()
