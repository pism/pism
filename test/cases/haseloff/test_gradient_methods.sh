...
do
    mpiexec -n $NN pism -i $INFILE -bootstrap $GRID -gradient $method $OPTS -spatial_file ex-$method.nc -o o-$method.nc
done;
...
do
    mpiexec -n $NN pism -i $INFILE -bootstrap $GRID -gradient $method $OPTS -spatial_file ex-no-smoother-$method.nc -o o-no-smoother-$method.nc
done;
