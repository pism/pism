...
def aggregate(data, total_time):
    ...
    for event in data:
        if data[event][0] / float(total_time) < 0.01:
            print(
                "Lumping '%s' (%f%%) with others..."
                % (event, 100.0 * data[event][0] / total_time)
            )
            del d[event]
...
def plot(data, total, grand_total):
    ...
    labels = [
        better_name(name) + " " + comment for name, comment in zip(names, comments)
    ]
...
big = figure("Time-stepping loop", big_events, total_time)

energy = figure("Energy step", small_events["energy"], big["energy"][0], total_time)

stressbalance = figure(
    "Stress balance",
    small_events["stress_balance"],
    big["stress_balance"][0],
    total_time,
)
...
sia = figure(
    "SIA",
    small_events["stress_balance.modifier"],
    stressbalance["stress_balance.modifier"][0],
    total_time,
)

io = figure("I/O during run", small_events["io"], big["io"][0], total_time)
