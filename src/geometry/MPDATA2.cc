...
static double flux_in(const stencils::Star<double> &u, const stencils::Star<double> &psi, double dx,
                      double dy, double dt) {
  return dt * ((pp(u.w) * psi.w - np(u.e) * psi.e) / dx + (pp(u.s) * psi.s - np(u.n) * psi.n) / dy);
}

static double flux_out(const stencils::Star<double> &u, const stencils::Star<double> &psi,
                       double dx, double dy, double dt) {
  return dt * ((pp(u.e) * psi.c - np(u.w) * psi.c) / dx + (pp(u.n) * psi.c - np(u.s) * psi.c) / dy);
}

static double beta_up(const stencils::Star<double> &u, const stencils::Star<double> &psi,
                      const stencils::Star<double> &psi_old, double dx, double dy, double dt) {
...
static double beta_down(const stencils::Star<double> &u, const stencils::Star<double> &psi,
                        const stencils::Star<double> &psi_old, double dx, double dy, double dt) {
  const double eps = 1e-15;
...
static void limit(double dt, const array::Scalar2 &x_old, const array::Scalar2 &x,
                  const array::Staggered1 &velocity, array::Staggered &result) {
  ...
  for (auto p : grid->points()) {
    ...
    double beta_d{ 0.0 };
    {
      stencils::Star<double> X = x.star(i, j), X_old = x_old.star(i, j), u = velocity.star(i, j);
    ...
    // east
    {
      stencils::Star<double> X = x.star(i + 1, j), X_old = x_old.star(i + 1, j),
    ...
    // north
    {
      stencils::Star<double> X = x.star(i, j + 1), X_old = x_old.star(i, j + 1),
...
static void compute_interface_velocity(const array::CellType &cell_type,
                                       const array::Vector &velocity, array::Staggered &result) {
  ...
  for (auto p : grid->points()) {
    ...
    int W = static_cast<int>(cell_type.icy(i, j)), W_n = static_cast<int>(cell_type.icy(i, j + 1)),
 ...
 * interfaces.
 */
static void compute_corrective_velocity(double dt, const array::Staggered &velocity,
                                        const array::Scalar2 &x, array::Staggered &result) {
  using std::fabs;

  auto grid = x.grid();

  const double eps = 1e-15, dx = grid->dx(), dy = grid->dy();
  ...
  for (auto p : grid->points()) {
    ...
    // eastern interface
    {
      double u     = velocity(i, j, 0),
             v_bar = 0.25 * (velocity(i + 1, j, 1) + velocity(i, j, 1) + velocity(i, j - 1, 1) +
                             velocity(i + 1, j - 1, 1));
      // double X_e2 = x(i + 2, j);
      ...
      double U = ((fabs(u) * dx - dt * u * u) * (X.e - X.c) / ((X.e + X.c + eps) * dx) -
                  0.5 * dt * u * v_bar * (X.ne - X.se + X.n - X.s) /
                      ((X.ne + X.se + X.n + X.s + eps) * dy));
    ...
    // northern interface
    {
      double v     = velocity(i, j, 1),
             u_bar = 0.25 * (velocity(i, j + 1, 0) + velocity(i, j, 0) + velocity(i - 1, j, 0) +
                             velocity(i - 1, j + 1, 0));
      // double X_n2 = x(i, j + 2);
      ...
      double V = ((fabs(v) * dy - dt * v * v) * (X.n - X.c) / ((X.n + X.c + eps) * dy) -
                  0.5 * dt * v * u_bar * (X.ne - X.nw + X.e - X.w) /
                      ((X.ne + X.nw + X.e + X.w + eps) * dx));
 ...
 */
static void step(double dt, const array::Staggered1 &velocity, const array::Scalar1 &x_old,
                 array::Scalar &x) {
  ...
  for (auto p : grid->points()) {
    ...
    double Q_e = upwind(f.c, f.e, u.e), Q_w = upwind(f.w, f.c, u.w), Q_n = upwind(f.c, f.n, u.n),
...
void MPDATA2::update(double dt, const array::CellType &cell_type, const array::Scalar &x,
                     const array::Vector &velocity, bool nonoscillatory) {
