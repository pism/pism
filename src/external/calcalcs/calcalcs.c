...
calcalcs_cal *ccs_init_calendar(const char *calname) {
  ...
  else if ((strcasecmp(calname, "julian") == 0) || (strcasecmp(calname, "proleptic_julian") == 0)) {
    retval = (calcalcs_cal *)malloc(sizeof(calcalcs_cal));
    if (retval == NULL) {
      fprintf(stderr, "Error, cannot allocate space for the calcalcs calendar\n");
      return (NULL);
  ...
  else if ((strcasecmp(calname, "noleap") == 0) || (strcasecmp(calname, "no_leap") == 0) ||
...
int ccs_isleap(calcalcs_cal *calendar, int year, int *leap) {
  ...
  if (calendar == NULL)
    return (CALCALCS_ERR_NULL_CALENDAR);
  if (calendar->sig != CCS_VALID_SIG)
    return (CALCALCS_ERR_INVALID_CALENDAR);

  if (calendar->mixed) {
    if (year >=
        calendar
            ->year_x) /* Q: did any countries transition during a year that had different leap status before and after??? Let's hope not! */
  ...
  if (calendar->mixed) {
    if (date_ge(year, month, day, calendar->year_x, calendar->month_x, calendar->day_x))
      ...
      sprintf(
          ...
          year, month, day, calendar->name, calendar->early_cal->name, calendar->year_px,
          calendar->month_px, calendar->day_px, calendar->late_cal->name, calendar->year_x,
          calendar->month_x, calendar->day_x);
...
int ccs_date2doy(calcalcs_cal *calendar, int year, int month, int day, int *doy) {
  int ierr, jd0, jd1, doy_px, jd_args, xition_date_first_day_of_year, ndays_elapsed;
  calcalcs_cal *c2use;
  ...
  if (calendar->mixed) {
    ...
    xition_date_first_day_of_year =
        ((year == calendar->year_x) && (calendar->month_x == 1) && (calendar->day_x == 1));
    if ((year > calendar->year_x) || xition_date_first_day_of_year)
      ...
      if ((ierr = ccs_date2jday(calendar->late_cal, year, month, day, &jd_args)) != 0)
        return (ierr);
      ndays_elapsed =
          jd_args - calendar->jday_x + 1; /* if this IS the transition day, ndays_elapsed==1 */
...
int ccs_doy2date(calcalcs_cal *calendar, int year, int doy, int *month, int *day) {
  int ierr, leap, jd0, jd1, tyear, doy_px, jd_want, xition_date_first_day_of_year, ndays_max;
  calcalcs_cal *c2use;
...
int ccs_dayssince(calcalcs_cal *calendar_orig, int year_orig, int month_orig, int day_orig,
  ...
  if (calendar_orig == NULL)
    return (CALCALCS_ERR_NULL_CALENDAR);
  if (calendar_orig->sig != CCS_VALID_SIG)
    return (CALCALCS_ERR_INVALID_CALENDAR);

  if (calendar_new == NULL)
    return (CALCALCS_ERR_NULL_CALENDAR);
  if (calendar_new->sig != CCS_VALID_SIG)
    return (CALCALCS_ERR_INVALID_CALENDAR);

  /* Figure out which calendar of the ORIGINAL calendar to use if it's a mixed calendar
	 */
  if (calendar_orig->mixed) {
    if (date_ge(year_orig, month_orig, day_orig, calendar_orig->year_x, calendar_orig->month_x,
                calendar_orig->day_x))
      c2use_orig = calendar_orig->late_cal;
    else if (date_le(year_orig, month_orig, day_orig, calendar_orig->year_px,
                     calendar_orig->month_px, calendar_orig->day_px))
      ...
      sprintf(
          ...
          year_orig, month_orig, day_orig, calendar_orig->name, calendar_orig->early_cal->name,
          calendar_orig->year_px, calendar_orig->month_px, calendar_orig->day_px,
          calendar_orig->late_cal->name, calendar_orig->year_x, calendar_orig->month_x,
          calendar_orig->day_x);
      return (CALCALCS_ERR_DATE_NOT_IN_CALENDAR);
    }
  } else
...
static void ccs_gxd_add_country(char *code, char *longname, int year, int month, int day) {
  ...
  ccs_xition_dates[ccs_n_country_codes]->longname =
      (char *)malloc(sizeof(char) * (strlen(longname) + 1));
...
int ccs_get_xition_date(const char *country_code, int *year, int *month, int *day) {
  ...
  sprintf(error_message,
          ...
          country_code);
  for (i = 0; i < ccs_n_country_codes; i++) {
    if ((strlen(error_message) + strlen(ccs_xition_dates[i]->code) +
         strlen(ccs_xition_dates[i]->longname) + 10) < CCS_ERROR_MESSAGE_LEN) {
...
static void ccs_dump_xition_dates(void) {
  ...
  for (i = 0; i < ccs_n_country_codes; i++) {
    printf("Code: %s     Transition date: %04d-%02d-%02d   Country/Region: %s\n",
           ccs_xition_dates[i]->code, ccs_xition_dates[i]->year, ccs_xition_dates[i]->month,
           ccs_xition_dates[i]->day, ccs_xition_dates[i]->longname);
    if (i % 3 == 2)
      printf("\n");
  }
}
...
int c_isleap_julian(int year, int *leap) {
  ...
  if (year < -4714) {
    sprintf(
        error_message,
        "ccs_isleap: year %d is out of range for the Julian calendar; dates must not be before 4713 B.C.",
        year);
...
int c_isleap_gregorian(int year, int *leap) {
  ...
  if (year < -4714) {
    sprintf(
        error_message,
        "ccs_isleap: year %d is out of range for the Gregorian calendar; dates must not be before 4713 B.C.",
        year);
    return (CALCALCS_ERR_OUT_OF_RANGE);
  }
...
int c_isleap_gregorian_y0(int year, int *leap) {
  if (year < -4714) {
    sprintf(
        error_message,
        "ccs_isleap: year %d is out of range for the Gregorian_y0 calendar; dates must not be before 4713 B.C.",
        year);
    return (CALCALCS_ERR_OUT_OF_RANGE);
  }
...
int c_date2jday_gregorian(int year, int month, int day, int *jday) {
  ...
  if ((month < 1) || (month > 12) || (day < 1) || (day > 31)) {
    sprintf(error_message, "date %04d-%02d-%02d does not exist in the Gregorian calendar", year,
            month, day);
    return (CALCALCS_ERR_DATE_NOT_IN_CALENDAR);
  }
...
int c_jday2date_gregorian(int jday, int *year, int *month, int *day) {
  ...
  *year = jday / 366 - 4714;
  ...
  yp1 = *year + 1;
  if (yp1 == 0)
    yp1 = 1; /* no year 0 in the Gregorian calendar */
...
int c_date2jday_julian(int year, int month, int day, int *jday) {
  ...
  if ((month < 1) || (month > 12) || (day < 1) || (day > 31)) {
    sprintf(error_message, "date %04d-%02d-%02d does not exist in the Julian calendar", year, month,
            day);
    return (CALCALCS_ERR_DATE_NOT_IN_CALENDAR);
  }
...
int c_jday2date_julian(int jday, int *year, int *month, int *day) {
  ...
  *year = jday / 366 - 4713;
  ...
  yp1 = *year + 1;
  if (yp1 == 0)
    yp1 = 1; /* no year 0 in the Julian calendar */
...
int c_date2jday_360_day(int year, int month, int day, int *jday) {
  ...
  if (day > 30) {
    sprintf(error_message, "date %04d-%02d-%02d does not exist in the 360_day calendar", year,
            month, day);
    return (CALCALCS_ERR_DATE_NOT_IN_CALENDAR);
  }
...
int c_date2jday_noleap(int year, int month, int day, int *jday) {
  if ((month == 2) && (day == 29)) {
    sprintf(error_message, "date %04d-%02d-%02d does not exist in the noleap calendar", year, month,
            day);
    return (CALCALCS_ERR_DATE_NOT_IN_CALENDAR);
  }
...
static int set_xition_extra_info(calcalcs_cal *cal) {
  ...
  /* This is the date of the day BEFORE the transition day,
	 ...
	 */
  ierr = ccs_jday2date(cal->early_cal, (cal->jday_x - 1), &(cal->year_px), &(cal->month_px),
                       &(cal->day_px));
  if (ierr != 0) {
    sprintf(error_message,
