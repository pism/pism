...
int utInvCalendar2_cal(int year, int month, int day, int hour, int minute, double second,
  ...
  if ((prev_units != NULL) && (prev_calendar != NULL) &&
      (strcmp(prev_calendar, cal2use->name) == 0) && (ut_compare(prev_units, user_unit) == 0)) {
...
static cv_converter *get_user_to_day_converter(ut_unit *uu, int y0, int mon0, int d0, int h0,
  ...
  sprintf(daystr, "days since %04d-%02d-%02d %02d:%02d:%f", y0, mon0, d0, h0, min0, s0);
...
static void get_origin(ut_unit *dataunits, int *y0, int *mon0, int *d0, int *h0, int *min0,
  ...
  if (udu_ref_date == NULL) {
    ...
    sprintf(ustr, "seconds since %04d-%02d-%02d %02d:%02d:%f", y0lib, mon0lib, d0lib, h0lib,
            min0lib, s0lib);
...
static void unknown_cal_emit_warning(const char *calendar_name) {
  ...
  unknown_cal_emitted_warning_for[n_unkcal] =
      (char *)malloc(sizeof(char) * (strlen(calendar_name) + 1));
