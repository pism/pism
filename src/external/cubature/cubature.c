 ...
 *  in p.
 */
static double evalR_Rfs(integrand f, void *fdata, unsigned dim, double *p, const double *c,
                        const double *r) {
  double sum = 0;
  unsigned i;
...
static unsigned rule15gauss_evalError(rule *r, integrand f, void *fdata, const hypercube *h,
  ...
  if (r == NULL) {
  } /* should quash unused parameter pedantic msg */
...
static int ruleadapt_integrate(rule *r, integrand f, void *fdata, const hypercube *h,
  ...
  for (i = 0; i < maxIter; ++i) {
    region R, R2;
    if (i >= minIter && (regions.ee.err <= reqAbsError || relError(regions.ee) <= reqRelError)) {
      status = 0; /* converged! */
      break;
    }
...
int adapt_integrate(integrand f, void *fdata, unsigned dim, const double *xmin, const double *xmax,
                    unsigned maxEval, double reqAbsError, double reqRelError, double *val,
