...
SeaRISEGreenland::SeaRISEGreenland(std::shared_ptr<const Grid> g) : YearlyCycle(g) {
  // empty
}
...
void SeaRISEGreenland::init_impl(const Geometry &geometry) {
  ...
  m_log->message(
      2, "* Initializing SeaRISE-Greenland atmosphere model based on the Fausto et al (2009)\n"
  ...
  if (not precip_file.empty()) {
    m_log->message(2, "  * Reading precipitation from '%s'...\n", precip_file.c_str());
...
void SeaRISEGreenland::update_impl(const Geometry &geometry, double t, double dt) {
  ...
  const double d_ma = m_config->get_number("atmosphere.fausto_air_temp.d_ma"),     // K
  ...
  const array::Scalar &h = geometry.ice_surface_elevation, &lat_degN = geometry.latitude,
  ...
  if (lon_degE.metadata().has_attribute("missing_at_bootstrap")) {
    throw RuntimeError(
        PISM_ERROR_LOCATION,
        "longitude variable was missing at bootstrap;\n"
        "SeaRISE-Greenland atmosphere model depends on longitude and would return nonsense!");
  }

  array::AccessScope list{ &h, &lat_degN, &lon_degE, &m_air_temp_mean_annual,
                           &m_air_temp_mean_summer };

  for (auto p : m_grid->points()) {
    const int i = p.i(), j = p.j();
    m_air_temp_mean_annual(i, j) =
        d_ma + gamma_ma * h(i, j) + c_ma * lat_degN(i, j) + kappa_ma * (-lon_degE(i, j));
