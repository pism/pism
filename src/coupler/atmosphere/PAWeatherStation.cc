...
PAWeatherStation::PAWeatherStation(IceGrid &g, const PISMConfig &conf)
...
PetscErrorCode PAWeatherStation::allocate() {
  ...
  m_precipitation.set_dimension_units(grid.time->units_string(), "");
  m_precipitation.set_units("m / second", "m / year");
  m_precipitation.set_attr("long_name",
                             "ice-equivalent precipitation rate");

  m_air_temperature.set_dimension_units(grid.time->units_string(), "");
  m_air_temperature.set_units("Kelvin", "Kelvin");
  m_air_temperature.set_attr("long_name",
...
PetscErrorCode PAWeatherStation::init(PISMVars &vars) {
  PetscErrorCode ierr;
...
PetscErrorCode PAWeatherStation::init_timeseries(double *ts, unsigned int N) {
...
PetscErrorCode PAWeatherStation::precip_time_series(int i, int j,
  ...
  for (unsigned int k = 0; k < m_precip_values.size(); ++k) {
    result[k] = m_precip_values[k];
  }
  return 0;
}
...
PetscErrorCode PAWeatherStation::temp_time_series(int i, int j,
  ...
  for (unsigned int k = 0; k < m_air_temp_values.size(); ++k) {
    result[k] = m_air_temp_values[k];
  }
  return 0;
}
...
void PAWeatherStation::add_vars_to_output(std::string keyword,
...
PetscErrorCode PAWeatherStation::define_variables(std::set<std::string> vars,
                                                  const PIO &nc, PISM_IO_Type nctype) {
  PetscErrorCode ierr;
...
PetscErrorCode PAWeatherStation::write_variables(std::set<std::string> vars,
                                                 const PIO &nc) {
  PetscErrorCode ierr;
