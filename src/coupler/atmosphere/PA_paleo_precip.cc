...
PaleoPrecip::PaleoPrecip(IceGrid::ConstPtr g, AtmosphereModel* in)
  ...
  options::String precipexpfactor_file("-atmosphere_paleo_precip_expfactor_file", "Read paleo-precip exponential factor from file");
  m_precipexpfactor_file_set = precipexpfactor_file.is_set();
...
void PaleoPrecip::mean_precipitation_impl(IceModelVec2S &result) const {
  ...
  result.scale(exp(m_precipexpfactor_scalar * m_current_forcing));
}
