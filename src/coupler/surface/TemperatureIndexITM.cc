...
TemperatureIndexITM::TemperatureIndexITM(IceGrid::ConstPtr g,
  ...
  if (not albedo_input.filename.empty()) {
    m_log->message(2,
                   " Albedo is read in from %s...",
                   albedo_input.filename.c_str());
...
double TemperatureIndexITM::get_lambda_paleo(double time){
  ...
  lambda_m = -2.*((ecc/2. + (pow(ecc,3))/8. ) * (1.+beta) * sin(-peri_rad) -
        (pow(ecc,2))/4. * (1./2. + beta) * sin(-2.*peri_rad) + (pow(ecc,3))/8. *
        (1./3. + beta) * sin(-3.*peri_rad)) + delta_lambda;
  lambda = ( lambda_m + (2.*ecc - (pow(ecc,3))/4.)*sin(lambda_m - peri_rad) +
        (5./4.)*(ecc * ecc) * sin(2.*(lambda_m - peri_rad)) + (13./12.)*(pow(ecc,3))
        * sin(3.*(lambda_m - peri_rad)) );
  return lambda; 
}
