...
PlumeModel::PlumeModel(std::shared_ptr<const Grid> grid)
  : CompleteOceanModel(grid, std::shared_ptr<OceanModel>()),
    m_basal_melt_rate(m_grid, "plume_basal_melt_rate"),
    m_grounding_line_elevation(grid, "plume_grounding_line_elevation"),
    m_shelf_base_elevation(grid, "plume_shelf_base_elevation"),
    m_local_slope(grid, "plume_local_slope"),
    m_fresh_water_melt_rate(grid, "plume_fresh_water_melt_rate"),
    m_discharge_flux(grid, "plume_discharge_flux"),
    m_disch_q0(grid, "plume_disch_q0"),
    m_disch_L5(grid, "plume_disch_L5"),
    m_disch_s(grid, "plume_disch_s"),
...
void PlumeModel::update_melt_rate(const Inputs &inputs, const PlumePhysics &physics,
...
void PlumeModel::build_discharge_field(const Inputs &inputs,
...

  {
    ...
    m_log->message(3,
                   "Plume discharge: %d outflow(s), %d with 5L' > 0;"
                   " max q_sg0 = %.3e m2/s, max 5L' = %.1f m\n",
                   N, n_active, max_q0, max_radius);
  }

  // Pass 2: distribute q_sg onto floating cells (method selected by ocean.plume.discharge_method).
  m_discharge_flux.set(0.0);
  ...
  if (m_discharge_method == DISCHARGE_ALONG_FLOW) {
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
      ...
      if (residual < rtol) {
        m_log->message(3,
                       "Plume discharge along-flow transport converged at iteration %03d"
                       " (max rel. change %f)\n", iter, residual);
        break;
      }
    }
...
void PlumeModel::compute_melt_rate(const Inputs &inputs,
  ...
  for (auto p : m_grid->points()) {
    ...
    if (cell_type.floating_ice(i, j)) {
      ...
      // via ocean.plume.add_fresh_water_melt. When disabled, the melt rate reduces to the
