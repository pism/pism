...
Picop::Picop(std::shared_ptr<const Grid> grid)
  : CompleteOceanModel(grid, std::shared_ptr<OceanModel>()),
    ...
    m_grounding_line_elevation(m_grid, "grounding_line_elevation"),
    m_geometry(grid),
    m_n_basins(0),
    m_n_boxes(0),
    m_n_shelves(0) {
...
void Picop::compute_grounding_line_elevation(const Geometry &geometry,
                                            array::Scalar &grounding_line_elevaton) const {

  const array::Scalar &bed = geometry.bed_elevation, &H = geometry.ice_thickness, &cell_type = geometry.cell_type,
    &surface = geometry.ice_surface_elevation, &z_s = geometry.sea_level_elevation;

  array::AccessScope l{ &bed, &H, &surface, &z_s, &cell_type};
...
void Picop::compute_ocean_input_per_basin(const PicoPhysics &physics,
...
void Picop::set_ocean_input_fields(const PicoPhysics &physics,
 ...
 */
void Picop::beckmann_goosse(const PicoPhysics &physics,
                           const array::Scalar &ice_thickness,
                           const array::Scalar &shelf_mask,
...
void Picop::process_box1(const PicoPhysics &physics,
                        const array::Scalar &ice_thickness,
                        const array::Scalar &shelf_mask,
...
void Picop::process_other_boxes(const PicoPhysics &physics,
                               const array::Scalar &ice_thickness,
                               const array::Scalar &shelf_mask,
                               const array::Scalar &box_mask,
                               array::Scalar &basal_melt_rate,
 ...
 */
void Picop::compute_box_average(int box_id,
 ...
 */
void Picop::compute_box_area(int box_id,
                            const array::Scalar &shelf_mask,
                            const array::Scalar &box_mask,
                            std::vector<double> &result) const {
  result.resize(m_n_shelves);
