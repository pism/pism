  ...
  for (auto p : grid->points()) {
    ...
    bool potential_partially_filled_cell =
        ((M.c == MASK_GROUNDED or M.c == MASK_ICE_FREE_OCEAN) and
         (M.w == MASK_FLOATING or M.e == MASK_FLOATING or M.s == MASK_FLOATING or
          M.n == MASK_FLOATING or M.sw == MASK_FLOATING or M.nw == MASK_FLOATING or
...
void Pico::update_impl(const Inputs &inputs, double t, double dt) {
  ...
  // Physical part of PICO
  {
    ...
    // prepare ocean input temperature and salinity
    {
      ...
      compute_ocean_input_per_basin(physics, m_geometry.basin_mask(),
                                    m_geometry.continental_shelf_mask(), *m_salinity_ocean,
                                    *m_theta_ocean, basin_temperature,
                                    basin_salinity); // per basin

      set_ocean_input_fields(physics, ice_thickness, cell_type, m_geometry.basin_mask(),
                             m_geometry.ice_shelf_mask(), basin_temperature, basin_salinity,
                             m_Toc_box0,
    ...
    beckmann_goosse(physics,
                    ...
                    m_Toc_box0,                  // input
                    m_Soc_box0,                  // input
                    m_basal_melt_rate, *m_shelf_base_temperature, m_Toc, m_Soc);
    ...
    process_box1(physics,
                 ...
                 m_Toc_box0,                  // input
                 m_Soc_box0,                  // input
                 m_basal_melt_rate, *m_shelf_base_temperature, m_T_star, m_Toc, m_Soc,
                 m_overturning);
...
void Pico::set_ocean_input_fields(const PicoPhysics &physics, const array::Scalar &ice_thickness,
                                  ...
                                  array::Scalar &Toc_box0, array::Scalar &Soc_box0) const {
 ...
 */
void Pico::beckmann_goosse(const PicoPhysics &physics, const array::Scalar &ice_thickness,
  ...
  array::AccessScope list{ &ice_thickness, &cell_type,       &shelf_mask,
                           &Toc_box0,      &Soc_box0,        &Toc,
  ...
  for (auto p : m_grid->points()) {
    ...
    if (cell_type.floating_ice(i, j)) {
      if (shelf_mask.as_int(i, j) > 0) {
        ...
        basal_melt_rate(i, j) = physics.melt_rate_beckmann_goosse(
            physics.theta_pm(Soc_box0(i, j), pressure), Toc_box0(i, j));
...
void Pico::process_box1(const PicoPhysics &physics, const array::Scalar &ice_thickness,
  ...
  array::AccessScope list{
    &ice_thickness, &shelf_mask, &box_mask,    &T_star,          &Toc_box0,         &Toc,
  ...
  for (auto p : m_grid->points()) {
    ...
    if (box_mask.as_int(i, j) == 1 and shelf_id > 0) {
      ...
      auto Toc_box1 =
          physics.Toc_box1(box1_area[shelf_id], T_star(i, j), Soc_box0(i, j), Toc_box0(i, j));
...
void Pico::process_other_boxes(const PicoPhysics &physics, const array::Scalar &ice_thickness,
                               const array::Scalar &shelf_mask, const array::Scalar &box_mask,
                               array::Scalar &basal_melt_rate, array::Scalar &basal_temperature,
                               array::Scalar &T_star, array::Scalar &Toc,
  ...
  array::AccessScope list{ &ice_thickness, &shelf_mask, &box_mask,        &T_star,
  ...
  for (int box = 2; box <= m_n_boxes; ++box) {
    ...
    for (int s = 1; s < m_n_shelves; ++s) {
      use_beckmann_goosse[s] =
          (salinity[s] == 0.0 or temperature[s] == 0.0 or overturning[s] == 0.0);
    }
    ...
    for (auto p : m_grid->points()) {
      ...
      if (box_mask.as_int(i, j) == box and shelf_id > 0) {
        ...
        const double S_previous = salinity[shelf_id], T_previous = temperature[shelf_id],
...

        {
          ...
          Toc(i, j)    = physics.Toc(box_area[shelf_id], T_previous, T_star(i, j), overturning_box1,
                                     S_previous);
          Soc(i, j)    = physics.Soc(S_previous, T_previous, Toc(i, j));

          // main outputs: basal melt rate and temperature
          basal_melt_rate(i, j) =
              physics.melt_rate(physics.theta_pm(Soc(i, j), pressure), Toc(i, j));
          basal_temperature(i, j) = physics.T_pm(Soc(i, j), pressure);
        }
      }
 ...
 */
void Pico::compute_box_average(int box_id, const array::Scalar &field,
                               const array::Scalar &shelf_mask, const array::Scalar &box_mask,
 ...
 */
void Pico::compute_box_area(int box_id, const array::Scalar &shelf_mask,
