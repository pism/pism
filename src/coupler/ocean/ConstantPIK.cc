...
PIK::PIK(std::shared_ptr<const Grid> g) : CompleteOceanModel(g) {
  // empty
}
...
void PIK::init_impl(const Geometry &geometry) {
  ...
  m_log->message(2, "* Initializing the constant (PIK) ocean model...\n");
 ...
 */
void PIK::melting_point_temperature(const array::Scalar &depth, array::Scalar &result) const {
...
void PIK::mass_flux(const array::Scalar &ice_thickness, array::Scalar &result) const {
  const double melt_factor = m_config->get_number("ocean.pik_melt_factor"),
  ...
  for (auto p : m_grid->points()) {
    ...
    // details]
    double shelfbaseelev = -(ice_density / sea_water_density) * ice_thickness(i, j),
    ...
    double ocean_heat_flux =
        melt_factor * sea_water_density * c_p_ocean * gamma_T * (T_ocean - T_f); // in W/m^2
