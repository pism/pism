...
void FrontalMelt::compute_retreat_rate(const Geometry &geometry,
  ...
  //todo,FIXME:Shall_we_use_it?;const bool do_consider_holes = m_config->get_flag("geometry.label_holes");
  ...
  try {
    for (Points p(*m_grid); p; p.next()) {
      ...
      //todo,FIXME:Shall_we_use_it?;if (cell_type.ice_free_open_ocean(i, j, do_consider_holes) and cell_type.next_to_ice(i, j)) {
