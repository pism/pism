...
MohrCoulombPointwise::MohrCoulombPointwise(std::shared_ptr<const Config> config) {
  ...
  m_reference_effective_pressure =
      config->get_number("basal_yield_stress.mohr_coulomb.till_reference_effective_pressure");
  m_reference_void_ratio =
      config->get_number("basal_yield_stress.mohr_coulomb.till_reference_void_ratio");
  m_compressibility_coefficient =
      config->get_number("basal_yield_stress.mohr_coulomb.till_compressibility_coefficient");
}

double MohrCoulombPointwise::effective_pressure(double delta, double P_overburden,
                                                double water_thickness) const {

  double s = water_thickness / m_W_till_max, N0 = m_reference_effective_pressure,
...
double MohrCoulombPointwise::yield_stress(double delta, double P_overburden, double water_thickness,
                                          double phi) const {
...
double MohrCoulombPointwise::till_friction_angle(double delta, double P_overburden,
