...
PetscErrorCode InvSSAForwardProblem_dep::rangeIP_core(PISMVector2 **A, PISMVector2**B, PetscScalar *OUTPUT)
{
  ...
  for (i=grid.xs; i<grid.xs+grid.xm; i++) {
    for (j=grid.ys; j<grid.ys+grid.ym; j++) {
      ...
      IP += (A[i][j].u*B[i][j].u+A[i][j].v*B[i][j].v)*misfit_weight*units_factor;
    } // j
  } // i
...
PetscErrorCode InvSSAForwardProblem_dep::assemble_TStarA_rhs( PISMVector2 **R, PISMVector2 **RHS)
{
  ...
  for (i=grid.xs; i<grid.xs+grid.xm; i++) {
    for (j=grid.ys; j<grid.ys+grid.ym; j++) {
      ...
      RHS[i][j].u = R[i][j].u*misfit_weight*units_factor/m_range_l2_area;
      RHS[i][j].v = R[i][j].v*misfit_weight*units_factor/m_range_l2_area;
    } // j-loop
  } // i-loop
