...
SSAForwardProblem::SSAForwardProblem(IceGrid &g, IceBasalResistancePlasticLaw &b,
...
SSAForwardProblem::~SSAForwardProblem() {
  PetscErrorCode ierr = this->destruct();
  CHKERRCONTINUE(ierr);
  assert(ierr == 0);
}

PetscErrorCode SSAForwardProblem::construct() {
  PetscErrorCode ierr;
...
PetscErrorCode SSAForwardProblem::destruct() {
  PetscErrorCode ierr;
...
PetscErrorCode SSAForwardProblem::set_zeta(IceModelVec2S &new_zeta )
{
  PetscErrorCode ierr;
  PetscInt i,j,q;
...
PetscErrorCode SSAForwardProblem::linearize_at( IceModelVec2S &zeta, bool &success) {
...
PetscErrorCode SSAForwardProblem::assemble_residual(IceModelVec2V &u, IceModelVec2V &RHS) {
  PetscErrorCode ierr;
...
PetscErrorCode SSAForwardProblem::assemble_residual(IceModelVec2V &u, Vec RHS) {
  PetscErrorCode ierr;
...
PetscErrorCode SSAForwardProblem::assemble_jacobian_state(IceModelVec2V &u, Mat Jac) {
  PetscErrorCode ierr;
...
PetscErrorCode SSAForwardProblem::apply_linearization(IceModelVec2S &dzeta, IceModelVec2V &du) {
  ...
  if (reason < 0) {
    SETERRQ1(grid.com, 1,
      "SSAForwardProblem::apply_linearization solve failed to converge (KSP reason %s)\n\n", KSPConvergedReasons[reason]);
  }
  else
  {
    verbPrintf(4,grid.com,"SSAForwardProblem::apply_linearization converged (KSP reason %s)\n", KSPConvergedReasons[reason] );
  }
...
PetscErrorCode SSAForwardProblem::apply_linearization_transpose(IceModelVec2V &du, IceModelVec2S &dzeta) {
  ...
  // No need to do an extra copy if du doesn't have ghosts.
  IceModelVec2V *du_global = &du;
  if(du_global->has_ghosts()) {
    ierr = m_du_global.copy_from(du); CHKERRQ(ierr);
    du_global = &m_du_global;
  }

  // call PETSc to solve linear system by iterative method.
  ierr = KSPSetOperators(m_ksp, m_J_state, m_J_state, SAME_NONZERO_PATTERN); CHKERRQ(ierr);
  ierr = KSPSolve(m_ksp, du_global->get_vec(), du_global->get_vec()); CHKERRQ(ierr); // SOLVE
  ...
  ierr = m_du_local.copy_from(*du_global); CHKERRQ(ierr);
