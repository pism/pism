...
void IP_SSATaucTaoTikhonovProblemLCL::construct() {
  ...
  double stressScale = grid->ctx()->config()->get_double("inverse.design_param_tauc_scale");
  m_constraintsScale = grid->Lx()*grid->Ly()*4*stressScale;

  m_velocityScale = grid->ctx()->config()->get_double("inverse.ssa_velocity_scale", "m second-1");
