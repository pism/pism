...
IPTwoBlockVec::IPTwoBlockVec(Vec a, Vec b) {
  ...
  ierr = PetscObjectGetComm((PetscObject)b, &comm_b);
  PISM_CHK(ierr, "PetscObjectGetComm");
  assert(comm == comm_b);
  ...
  PISM_CHK(ierr, "VecGetOwnershipRange");
  ...
  petsc::IS is_a, is_b;
  // a in a
  ierr = ISCreateStride(comm, m_na_local, lo_a, 1, is_a.rawptr());
  PISM_CHK(ierr, "ISCreateStride");
  // a in ab
  ierr = ISCreateStride(comm, m_na_local, lo_a + lo_b, 1, m_a_in_ab.rawptr());
  PISM_CHK(ierr, "ISCreateStride");

  // b in b
  ierr = ISCreateStride(comm, m_nb_local, lo_b, 1, is_b.rawptr());
  PISM_CHK(ierr, "ISCreateStride");
  // b in ab
  ierr = ISCreateStride(comm, m_nb_local, lo_a + lo_b + m_na_local, 1, m_b_in_ab.rawptr());
  PISM_CHK(ierr, "ISCreateStride");

  ierr = VecCreate(comm, m_ab.rawptr());
  PISM_CHK(ierr, "VecCreate");

  ierr = VecSetType(m_ab, "mpi");
  PISM_CHK(ierr, "VecSetType");
  ierr = VecSetSizes(m_ab, m_na_local + m_nb_local, m_na_global + m_nb_global);
  PISM_CHK(ierr, "VecSetSizes");

  ierr = VecScatterCreate(m_ab, m_a_in_ab, a, is_a, m_scatter_a.rawptr());
  PISM_CHK(ierr, "VecScatterCreate");
  ierr = VecScatterCreate(m_ab, m_b_in_ab, b, is_b, m_scatter_b.rawptr());
  PISM_CHK(ierr, "VecScatterCreate");
}
