...
# This file replaces an older (Jed, 2006) Ruby script 'ncgen.rb'.
...
def create_write_attributes(diag = False):
  ...
  Please make sure it is in your PATH and try again."""
    ...
    for line in cdl_input_file:
        if line.find("global attributes") > -1: # Find returns the substring
...
def process_the_code(input, output, diag = False):
    ...
    pairs = [("91", "grid.Mx"), ("92", "grid.My"), \
             ("93", "grid.Mz"), ("94", "grid.Mbz")]
    if diag:
...
def replace_all(str, pairs):
    ...
    Returns a new string, replacing the first element of each tuple with
    the second.
