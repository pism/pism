...
void PBLingleClark::init_impl() {
  ...
  bool bootstrap = input_given and do_regrid;
  bool restart = input_given and not do_regrid;
  ...
  if (restart) {
    ...
    m_viscous_bed_displacement.read(input_file, start);
  } else if (bootstrap) {
    this->init(m_topg, m_uplift, *ice_thickness);
  } else {
    // do nothing
  }
...
void PBLingleClark::add_vars_to_output_impl(const std::string &keyword,
                                            std::set<std::string> &result) {
  BedDef::add_vars_to_output_impl(keyword, result);

  result.insert("viscous_bed_displacement");
}

void PBLingleClark::define_variables_impl(const std::set<std::string> &vars,
                                          const PIO &nc,
                                          IO_Type nctype) {
  BedDef::define_variables_impl(vars, nc, nctype);

  if (set_contains(vars, "viscous_bed_displacement")) {
    m_viscous_bed_displacement.define(nc, nctype);
  }
}

void PBLingleClark::write_variables_impl(const std::set<std::string> &vars, const PIO &nc) {
  BedDef::write_variables_impl(vars, nc);

  if (set_contains(vars, "viscous_bed_displacement")) {
    m_viscous_bed_displacement.write(nc);
  }
}
