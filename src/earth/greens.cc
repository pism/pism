...
double ge_integrand(unsigned ndim, const double *args, void *data) {
  ...
  const double dx = d->dx, dy = d->dy, xi = args[0], eta = args[1], xi_shift = d->p * dx - xi,
               eta_shift = d->q * dy - eta, r = sqrt(xi_shift * xi_shift + eta_shift * eta_shift);
...
const double greens_elastic::rmkm[greens_elastic::N] = {
  ...
  22.24,  27.80,  33.36,  44.48,  55.60,  66.72,  88.96,  111.2,  133.4,  177.9,  222.4,
  278.0,  333.6,  444.8,  556.0,  667.2,  778.4,  889.6,  1001.0, 1112.0, 1334.0, 1779.0,
  2224.0, 2780.0, 3336.0, 4448.0, 5560.0, 6672.0, 7784.0, 8896.0, 10008.0
};

const double greens_elastic::GE[greens_elastic::N] = {
  -33.6488, -33.64, -33.56, -32.75, -31.86, -30.98, -30.12, -28.44, -26.87, -25.41, -21.80,
  -20.02,   -18.36, -17.18, -15.71, -14.91, -14.41, -13.69, -13.01, -12.31, -10.95, -9.757,
  -8.519,   -7.533, -6.131, -5.237, -4.660, -4.272, -3.999, -3.798, -3.640, -3.392, -2.999,
  -2.619,   -2.103, -1.530, -0.292, 0.848,  1.676,  2.083,  2.057,  1.643
};
...
double vd_integrand(double kappa, void *parameters) {
  ...
  const double t = p->t, R0 = p->R0, rk = p->rk, rho = p->rho, grav = p->grav, D = p->D,
               eta = p->eta, beta = rho * grav + D * pow(kappa, 4.0),
 ...
 */
double viscDisc(double t, double H0, double R0, double r, double rho, double rho_ice, double grav,
                double D, double eta) {
  ...
  // end
  for (int j = 0; j < lengthpts - 1; j++) {
    gsl_integration_qag(&F, pts[j + 1], pts[j], ABSTOL, RELTOL, N_gsl_workspace, GSL_INTEG_GAUSS21,
                        w, &result, &error);
    sum += result;
  }
