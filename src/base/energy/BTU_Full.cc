...
BTU_Full::BTU_Full(IceGrid::ConstPtr g, const BTUGrid &grid)
  ...
  m_k = m_config->get_double("bedrock_thermal_conductivity");

  const double
    rho = m_config->get_double("bedrock_thermal_density"),
    c   = m_config->get_double("bedrock_thermal_specific_heat_capacity");
