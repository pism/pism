 ...
 * <tt>manual.pdf</tt>, or see a recent version at http://www.pism-docs.org/).  
 * Most users should stick to the User's Manual for quite a while; only when 
 * a user needs to extend PISM is a look at this C++ class Browser really 
 * worthwhile.
 *
 * For the subset of this C++ class Browser which documents the ice dynamics core routines
 * in PDF form, see the PISM <i>Reference Manual</i> (do <tt>cd doc/</tt> and 
 * <tt>make refman</tt> and then view <tt>refman.pdf</tt>).  The <i>Reference Manual</i> contains
 * the minimum documentation of the PISM class structure in order to document 
 * the ice dynamics continuum models and numerical methods of PISM.  The Browser contains
 * all the material in the <i>Reference Manual</i>.
 *
