 ...
 * worthwhile.
 *
 * For a useful subset of this C++ class browser
 ...
 * and numerical method documentation is widely scattered in this C++ class browser.
 *
...
methods in PISM.  It is organized by C++ class and it was automatically 
generated by doxygen (http://www.doxygen.org/) from comments in the 
source code.

See  http://www.pism-docs.org/ for all documentation of PISM.
...
using subversion as follows: \code svn co http://svn.gna.org/svn/pism/trunk pism-dev \endcode
This \e Reference \e Manual can be rebuilt in any source code release
if the user has PDFLaTeX.  Just do
<tt>cd doc && make refman</tt> in the PISM root directory (e.g. \c pism-dev/)
and then view <tt>refman.pdf</tt>
...
C++ class browser.  That browser can be generated from a copy of the PISM source by 
<tt>cd doc && make browser</tt> .  Then use a web browser like Firefox to view
\c doxy/html/index.html.
 
The C++ source files which contain the comments which generated this manual
are in the <tt>src/base</tt> subdirectory.  Only those parts of the source with 
doxygen-formatted comments are documented here.  By contrast, the C++ class browser 
described above includes all the classes, class members, 
