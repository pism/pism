...
The website  http://www.pism-docs.org/  is the root for all PISM information.

See  https://gna.org/projects/pism/  to download the PISM source code.

You can get help, send comments, or ask for new features by email to
help@pism-docs.org.

For help \e installing PISM, see the PISM <i>Installation Manual</i>.
Do <tt>cd doc/</tt> and <tt>make installation</tt> and then view 
<tt>installation.pdf</tt>.  Or see a recent version at http://www.pism-docs.org/.  

For help \e using PISM, see the PISM <i>User's Manual</i>.  Do <tt>cd doc/</tt> 
and <tt>make userman</tt> and then view <tt>manual.pdf</tt>.  Or see a recent
version at http://www.pism-docs.org/.  Most users should stick to the 
\e User's \e Manual for quite a while.  Only when a user needs to extend
PISM functionality is a look at the current \e Browser worthwhile.

For the subset of this \e Browser which documents the ice dynamics core routines
in PDF form, see the PISM <i>Reference Manual</i>.  Do <tt>cd doc/</tt> and 
<tt>make refman</tt> and then view <tt>refman.pdf</tt>.  The <i>Reference Manual</i>
contains the minimum documentation of the PISM class structure in order to document 
the ice dynamics continuum models and numerical methods of PISM.  The \e Browser 
contains all the material in the <i>Reference Manual</i> and much more.
...
\e Authors:  Ed Bueler and Constantine Khroulev

<i>Based on PISM \c trunk revision:</i> \include revision.tex
 
\section purposeID Purpose

This \e Reference \e Manual documents the ice dynamics-related continuum models
and numerical methods in PISM.  It is organized by C++ class and it was automatically 
generated by doxygen (http://www.doxygen.org/) from comments in the source code.

The website  http://www.pism-docs.org/  is the root for all PISM information.

See https://gna.org/projects/pism/ to download the PISM source code.
Specifically, the development branch of the source can be checked-out
using subversion as follows:
\code svn co http://svn.gna.org/svn/pism/trunk pism-dev \endcode
This \e Reference \e Manual can be rebuilt in any source code release if PDFLaTeX is
available.  Just do <tt>cd doc && make refman</tt> in the PISM root directory 
(e.g. \c pism-dev/) and then view <tt>refman.pdf</tt>

Most users should start with the PISM <i>User's Manual</i> and
<i>Installation Manual</i>.  These manuals are also in any source code 
release.  Just do <tt>cd doc && make userman installation</tt> in the 
PISM root directory and then view <tt>manual.pdf</tt> and  
<tt>installation.pdf.</tt>  Even developers should see the <i>User's Manual</i>
...
Browser described above documents all the classes, class members, and source files in 
PISM.  Only those parts of the source with doxygen-formatted comments are documented here.

This \e Reference \e Manual could be used as a source for journal articles which
need to include the equations approximated by PISM, or which need to describe the
approximations.
...
classes to know about, the sources for each of which are in \c pism/src/base/:
  ...
  - MaterialType.  Various materials are derived from the class \c MaterialType which merely defines 
       ...
       which implements the Goldsby-Kohlstedt flow law \lo\cite{GoldsbyKohlstedt}\elo.  
       \lo\footnote{Actually, it is 
