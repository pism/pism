...
void Distributed::P_from_W_steady(IceModelVec2S &result) {
  double CC = m_config->get_double("hydrology_cavitation_opening_coefficient") /
                    (m_config->get_double("hydrology_creep_closure_coefficient") * m_config->get_double("ice_softness")),
    powglen = 1.0 / m_config->get_double("sia_Glen_exponent"), // choice is SIA; see #285
...
void Distributed::update_impl(double icet, double icedt) {
  ...
  const double
            ...
            nglen = m_config->get_double("sia_Glen_exponent"), // choice is SIA; see #285
            Aglen = m_config->get_double("ice_softness"),
