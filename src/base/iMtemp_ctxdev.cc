...
public:
  columnSystemCtx(int my_Mz, int my_Mbz, int my_k0)
      Mz(my_Mz), Mbz(my_Mbz), k0(my_k0) {} // constructing is trivial for now
  ~columnSystemCtx();
  PetscErrorCode ageCreate(PetscScalar **x);
  PetscErrorCode temperatureCreate(PetscScalar **x);

FIXME: should these be arguments of temperatureCreate()
  // these are all scalar values of IceModelVec2 at [i][j]:
  PetscScalar  Ts,          // set by PISM{Atmosphere|Ocean}Coupler, resp.
               Tshelfbase;
  PetscScalar  mask,
               Rb,
               Ghf;
  IceModelVec3 *T3;         // needed to evaluate by getPlaneStarZ()


public:                    // members setable by calling routine
  // constants: do not depend on [i][j]
  PetscInt     ks,
               n;
  PetscScalar  dx,
               dy,
               ...
               dzEQ,
               dzbEQ;
  PetscScalar  *u,         // length Mz
               *v,
               *w,
               *Sigma,
               *T;
  PetscScalar  *Tb;        // length Mbz
  bool         isMarginal;
...
private:  // manipulate inside tempColumnSystemSolve()
  PetscInt    Mz,
              Mbz,
              k0;
  PetscScalar *L,
  ...
  /*OLD:  PetscErrorCode solveTridiagonalSystem(
...
} // end class columnSystemCtx


columnSystemCtx::columnSystemCtx(int my_Mz, int my_Mbz, int my_k0)
      Mz(my_Mz), Mbz(my_Mbz), k0(my_k0) {
  Lp   = new PetscScalar[Mz+k0-1];
...
PetscErrorCode IceModel::temperatureStep(PetscScalar* vertSacrCount) {
  ...
  columnSystemCtx system(Mz,Mbz,k0); // linear system to solve in each column
  
  const PetscScalar   nuEQ = dtTempAge / dzEQ;

  system.rho_c_I = ice->rho * ice->c_p;
  system.rho_c_br = bed_thermal.rho * bed_thermal.c_p;
  system.rho_c_av = (dzEQ * rho_c_I + dzbEQ * rho_c_br) / (dzEQ + dzbEQ);
  system.iceK = ice->k / rho_c_I;
  system.iceR = iceK * dtTempAge / PetscSqr(dzEQ);
  system.brK = bed_thermal.k / rho_c_br;
  system.brR = brK * dtTempAge / PetscSqr(dzbEQ);
