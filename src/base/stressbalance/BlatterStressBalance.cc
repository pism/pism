...
PetscErrorCode BlatterStressBalance::update(bool fast) {
  ...
  ierr = verbPrintf(1,grid.com,
    "\n\nJed's ex48.c running now: ignoring PISM context! ...\n\n"); CHKERRQ(ierr);
  ...
  * coarser (as in the last step of grid sequencing), and so requires fewer Newton steps. */
  ierr = PetscOptionsGetTruth(NULL,"-ex48_repeat_fine_solve",&repeat_fine_solve,NULL);CHKERRQ(ierr);
  ...
  ierr = DMMGCreate(PETSC_COMM_WORLD,thi->nlevels,thi,&dmmg);CHKERRQ(ierr);
  {
    ...
    {
      ierr = PetscOptionsInt("-ex48_M","Number of elements in x-direction on coarse level","",M,&M,NULL);CHKERRQ(ierr);
      N = M;
      ierr = PetscOptionsInt("-ex48_N","Number of elements in y-direction on coarse level (if different from M)","",N,&N,NULL);CHKERRQ(ierr);
      if (thi->coarse2d) {
        ierr = PetscOptionsInt("-ex48_zlevels","Number of elements in z-direction on fine level","",thi->zlevels,&thi->zlevels,NULL);CHKERRQ(ierr);
      } else {
        ierr = PetscOptionsInt("-ex48_P","Number of elements in z-direction on coarse level","",P,&P,NULL);CHKERRQ(ierr);
      }
    }
    ierr = PetscOptionsEnd();CHKERRQ(ierr);
    if (thi->coarse2d) {
...

  {
    ...
    ierr = PetscOptionsGetString(PETSC_NULL,"-ex48_o",filename,sizeof(filename),&flg);CHKERRQ(ierr);
    if (flg) {
  ...
  ierr = verbPrintf(1,grid.com,
    "\nJed's ex48.c has now run.  ENDING ...\n\n"); CHKERRQ(ierr);
  PISMEnd();
