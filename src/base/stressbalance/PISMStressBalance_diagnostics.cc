...
IceModelVec::Ptr PSB_velbar_mag::compute_impl() {
  ...
  double fill_value = units::convert(m_sys, config->get_double("fill_value"), "m year-1", "m second-1");
...
PSB_flux_mag::PSB_flux_mag(StressBalance *m)
  ...
  Config::ConstPtr config = m_grid->ctx()->config();
  double fill_value = units::convert(m_sys, config->get_double("fill_value"), "m2 year-1", "m2 second-1");
...
PSB_velsurf::PSB_velsurf(StressBalance *m)
  ...
  m_vars[1].set_double("valid_min", units::convert(m_sys, -1e6, "m year-1", "m second-1"));
  m_vars[1].set_double("valid_max", units::convert(m_sys, 1e6, "m year-1", "m second-1"));
