...
GoldsbyKohlstedt::GoldsbyKohlstedt(double exponent, const Config &config,
...
double GoldsbyKohlstedt::flow_impl(double stress, double E, double pressure,
                                   double grainsize) const {
...
*/
double GoldsbyKohlstedt::flow_from_temp(double stress, double temp, double pressure,
                                        double gs) const {
  ...
  eps_diff = 42 * m_diff_V_m * (diff_D_v + M_PI * m_diff_delta * diff_D_b / gs) / (RT * (gs * gs));
  // Dislocation Creep
...
GKparts GoldsbyKohlstedt::flowParts(double stress, double temp, double pressure) const {
  ...
  gs       = m_d_grain_size;
  eps_diff = 14 * m_diff_V_m * (diff_D_v + M_PI * m_diff_delta * diff_D_b / gs) / (RT * (gs * gs));
  // Dislocation Creep
  if (T > m_disl_crit_temp) {
    eps_disl = m_disl_A_warm * pow(stress, m_disl_n - 1) * exp(-(m_disl_Q_warm + pV) / RT);
  } else {
...
GoldsbyKohlstedtStripped::GoldsbyKohlstedtStripped(double exponent, const Config &config,
                                                   std::shared_ptr<EnthalpyConverter> ec)
...
double GoldsbyKohlstedtStripped::flow_from_temp(double stress, double temp, double pressure,
                                                double) const {
