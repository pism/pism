/*
 * $Id: utlib.c,v 1.26 2008/07/23 15:26:59 steve Exp $
 *
    ...
    utUnit	*result;
{
    ...
    result->origin	= source->factor*source->origin + amount;
    result->hasorigin	= 1;
            ...
            if (unit->hasorigin) {
                if (utIsTime(unit)) {
                    ...
                    int         ndigsec;        /* precision of seconds in
                                                 * number of digits */
                    float       seconds;
		    double	origin = unit->origin*unit->factor;

                    dectime(origin, &year, &month, &day, &hours,
                            &minutes, &seconds);
 ...
 *	slope		The slope of the transformation y = ax + b.
 *	intercept	The intercept of the transformation y = ax + b.
 * RETURNS:
 *	0		success.
 *	UT_ENOINIT	the units-table hasn't been initialized
    ...
    double	*value;
{
	...
	*value	= ((utencdate(year, month, day) - 
		   unit->origin*unit->factor) + 
		   utencclock(hour, minute, second)) / unit->factor;
	status	= 0;
    }
...
{
    while (root) {
	UnitEntry	*entry = *(UnitEntry**)root;
