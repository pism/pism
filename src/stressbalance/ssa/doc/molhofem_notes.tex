...
\begin{align}
  \label{eq:10}
  F_{1}(u,v) &=  \I \left[ \diff{\psi}{x} \left( \eta (\Mux) \right) + \diff{\psi}{y} \left( \eta (\Muy) \right) + \diff{\psi}{z} \left( \eta (\Muz) \right) - \psi (\taubx + \taudx) \right] - \Phi_{1}\\
  \label{eq:11}
  F_{2}(u,v) &= \I \left[ \diff{\psi}{x} \left( \eta (\Mvx) \right) + \diff{\psi}{y} \left( \eta (\Mvy) \right) + \diff{\psi}{y} \left( \eta (\Mvz) \right) - \psi (\tauby + \taudy) \right] - \Phi_{2}.
\end{align}
...
\begin{align}
  \label{eq:13}
  F_{k,1} & = \highlight{ \sumQ |J_q| \cdot w_q\cdot \left[ \eta \left( \diff{\psi_k}{x} (\Mux) + \diff{\psi_k}{y} (\Muy) + \diff{\psi_k}{z}\Muz \right) - \psi_k (\taubx + \taudx) \right]_{\text{evaluated at } q} - \Phi_{k,1} }\\
  \label{eq:14}
  F_{k,2} & = \highlight{ \sumQ |J_q| \cdot w_q\cdot \left[ \eta \left( \diff{\psi_k}{x} (\Mvx) + \diff{\psi_k}{y} (\Mvy)+ \diff{\psi_k}{z}\Mvz \right) - \psi_k (\tauby + \taudy) \right]_{\text{evaluated at } q} - \Phi_{k,2}}
\end{align}
...
\begin{align*}
  F_{k,1} &= \highlight{
            \begin{aligned}[t]
              \sumQ |J_q| \cdot w_q\cdot \Bigg[ &\eta \Bigg(\diff{\psi_k}{x} \left(4 \UX + 2 \VY\right) 
  ...
  F_{k,2} &= \highlight{
            \begin{aligned}[t]
              \sumQ |J_q| \cdot w_q\cdot \Bigg[ &\eta \Bigg(\diff{\psi_k}{x} \left(\UY + \VX\right)
