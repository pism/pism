...
public:
  SSATestCaseExp(std::shared_ptr<SSA> ssa) : SSATestCase(ssa) {
...
void SSATestCaseExp::exactSolution(int /*i*/, int /*j*/, double x, double /*y*/, double *u,
                                   double *v) {
  double tauc_threshold_velocity =
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    bool stop = maybe_show_usage(*ctx->log(), "ssa_test_linear", usage);
