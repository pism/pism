...
Vector2 blatter_xz_halfar_lateral(double x, double z, double H_0, double R_0, double rho_i, double g, double B)
{
  ...
  return {
    (1.0/2.0)*B*((64.0/343.0)*x12 + (1024.0/2401.0)*x13 + (64.0/343.0)*x15)/cbrt(pow((32.0/343.0)*x12 + (512.0/2401.0)*x13 + (32.0/343.0)*x15, 2) + (4096.0/117649.0)*pow(H_0, 6)*pow(g, 6)*pow(rho_i, 6)*pow(x, 2)*pow(x9, 6)/(pow(B, 6)*pow(R_0, 8)*pow(x7, 24.0/7.0))),
    0.0
  };
}
