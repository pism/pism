...
/** The Q1 finite element MOLHO solver.
 *
 *
 *
 */
MOLHO::MOLHO(std::shared_ptr<const Grid> grid)
    : MOLHO(grid),
      m_bc_mask(grid, "bc_mask"),
      m_bc_values(grid, "_bc"),
      m_gc(*m_config),
      m_coefficients(grid, "molho_coefficients", array::WITH_GHOSTS, 1),
  ...
  const double n           = m_config->get_number("stress_balance.molho.Glen_exponent");
  const double int_order   = m_config->get_number("stress_balance.molho.viscosity_integration_order");
...
void MOLHO::init_impl() {

  MOLHO::init_impl();

  // Use explicit driving stress if provided.
  if (m_grid->variables().is_available("molho_driving_stress_x") and
      m_grid->variables().is_available("molho_driving_stress_y")) {
    m_driving_stress_x = m_grid->variables().get_2d_scalar("molho_driving_stress_x");
    m_driving_stress_y = m_grid->variables().get_2d_scalar("molho_driving_stress_y");
  }

  m_log->memolhoge(2, "  [using the SNES-based finite element method implementation]\n");

  // process command-line options
  {
    m_dirichletScale = 1.0e9;
    m_dirichletScale = options::Real(m_sys, "-molho_fe_dirichlet_scale",
                                     "Enforce Dirichlet conditions with this additional scaling",
                                     "1", m_dirichletScale);
  }

  // On restart, MOLHO::init() reads the MOLHO velocity from a PISM output file
...
/**  Solve the MOLHO system of equations.
 ...
 When running in an ice-model context, at each time step, MOLHO::update() is called, which calls
 MOLHO::solve(). Since coefficients have generally changed between time steps, we need to recompute
 ...
 So there are two different solve methods, MOLHO::solve() and MOLHO::solve_nocache(). The only
 difference is that MOLHO::solve() recomputes the cached values of the coefficients before calling
 MOLHO::solve_nocache().
 */
void MOLHO::solve(const Inputs &inputs) {

  auto reason = solve_with_reason(inputs);
  if (reason->failed()) {
    throw RuntimeError::formatted(PISM_ERROR_LOCATION,
                                  "MOLHO solve failed to converge (SNES reason %s)",
                                  reason->description().c_str());
  }

  if (m_log->get_threshold() > 2) {
    m_stdout_molho += "MOLHO converged (SNES reason " + reason->description() + ")";
  }
}

std::shared_ptr<TerminationReason> MOLHO::solve_with_reason(const Inputs &inputs) {
...
//! Solve the MOLHO without first recomputing the values of coefficients at quad
//! points.  See the disccusion of MOLHO::solve for more discussion.
std::shared_ptr<TerminationReason> MOLHO::solve_nocache() {
  PetscErrorCode ierr;

  m_epsilon_molho = m_config->get_number("stress_balance.molho.epsilon");

  options::String filename("-molho_view", "");
  if (filename.is_set()) {
    petsc::Viewer viewer;
    ierr = PetscViewerASCIIOpen(m_grid->com, filename->c_str(), viewer.rawptr());
    PISM_CHK(ierr, "PetscViewerASCIIOpen");

    ierr = PetscViewerASCIIPrintf(viewer, "SNES before MOLHOSolve_FE\n");
    PISM_CHK(ierr, "PetscViewerASCIIPrintf");

    ierr = SNESView(m_snes, viewer);
    PISM_CHK(ierr, "SNESView");

    ierr = PetscViewerASCIIPrintf(viewer, "solution vector before MOLHOSolve_FE\n");
    PISM_CHK(ierr, "PetscViewerASCIIPrintf");
  ...
  m_stdout_molho.clear();
  if (m_log->get_threshold() >= 2) {
    m_stdout_molho = "  MOLHO: ";
  }
  ...
  if (not reason->failed()) {
    ...
    // Extract the solution back from MOLHOX to velocity and communicate.
    m_velocity.copy_from(m_velocity_global);

    bool view_solution = options::Bool("-molho_view_solution", "view solution of the MOLHO system");
    if (view_solution) {
      petsc::Viewer viewer;
      ierr = PetscViewerASCIIOpen(m_grid->com, filename->c_str(), viewer.rawptr());
      PISM_CHK(ierr, "PetscViewerASCIIOpen");

      ierr = PetscViewerASCIIPrintf(viewer, "solution vector after MOLHOSolve\n");
      PISM_CHK(ierr, "PetscViewerASCIIPrintf");
...
//! Initialize stored data from the coefficients in the MOLHO.  Called by MOLHO::solve.
/**
   This method is should be called after MOLHO::init and whenever any geometry or temperature
...
*/
void MOLHO::cache_inputs(const Inputs &inputs) {
...
void MOLHO::quad_point_values(const fem::Element &E,
...
void MOLHO::explicit_driving_stress(const fem::Element &E,
                                     const Coefficients *x,
                                     Vector2d *result) {
...
*/
void MOLHO::driving_stress(const fem::Element &E,
                            const Coefficients *x,
...
void MOLHO::PointwiseNuHAndBeta(double thickness,
                                 double hardness,
                                 int mask,
                                 double tauc,
                                 const Vector2d &U,
  ...
  *nuH  = m_epsilon_molho + *nuH * thickness;
 ...
 */
void MOLHO::cache_residual_cfbc(const Inputs &inputs) {
 ...
 * is the weak form of the MOLHO, \f$x\f$ is the current approximate
 * solution, and the \f$\psi_{ij}\f$ are test functions.
 *
 * The weak form of the MOLHO system is
 */
void MOLHO::compute_local_function(Vector2d const *const *const velocity_global,
  ...
  Vector2d U[Nq_max], U_x[Nq_max], U_y[Nq_max] U_z[Nq_max];
...
void MOLHO::monitor_function(Vector2d const *const *const velocity_global,
                              Vector2d const *const *const residual_global) {
  PetscErrorCode ierr;
  bool monitorFunction = options::Bool("-molho_monitor_function", "monitor the MOLHO residual");
  if (not monitorFunction) {
    return;
  }

  ierr = PetscPrintf(m_grid->com,
                     "MOLHO Solution and Function values (pointwise residuals)\n");
...
/*!
  ...
  where \f$G\f$ is the weak form of the MOLHO, \f$x\f$ is the current
  approximate solution, and the \f$\psi_{ij}\f$ are test functions.

*/
void MOLHO::compute_local_jacobian(Vector2d const *const *const velocity_global, Mat Jac) {
    ...
    for (int j = ys; j < ys + ym; j++) {
      for (int i = xs; i < xs + xm; i++) {
...

        {
          ...
          E->evaluate(velocity_nodal, U, U_x, U_y, U_z);
        }
        ...
        for (unsigned int q = 0; q < Nq; q++) {
          ...
          double eta, deta;
          m_flow_law->effective_viscosity(hardness[q], gamma, m_viscosity_eps, &eta, &deta);
...
void MOLHO::monitor_jacobian(Mat Jac) {
  PetscErrorCode ierr;
  bool mon_jac = options::Bool("-molho_monitor_jacobian", "monitor the MOLHO Jacobian");
  ...
  auto file_name = pism::printf("PISM_MOLHO_J%d.m", (int)iter);

  m_log->message(2,
                 "writing Matlab-readable file for MOLHO system A xsoln = rhs to file `%s' ...\n",
                 file_name.c_str());
...
//!
PetscErrorCode MOLHO::function_callback(DMDALocalInfo *info,
                                         Vector2d const *const *const velocity,
                                         Vector2d **residual,
                                         CallbackData *fe) {
  try {
    (void) info;
    fe->molho->compute_local_function(velocity, residual);
  } catch (...) {
...
PetscErrorCode MOLHO::jacobian_callback(DMDALocalInfo *info,
                                         Vector2d const *const *const velocity,
                                         Mat A, Mat J, CallbackData *fe) {
  try {
    (void) A;
    (void) info;
    fe->molho->compute_local_jacobian(velocity, J);
  } catch (...) {
    MPI_Comm com = MPI_COMM_SELF;
    PetscErrorCode ierr = PetscObjectGetComm((PetscObject)fe->da, &com); CHKERRQ(ierr);
    handle_fatal_errors(com);
