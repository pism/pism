...
static void compute_strain_heating_errors(const array::Array3D &strain_heating,
                                          const array::Scalar &thickness, const Grid &grid,
  ...
  const double ice_rho = grid.ctx()->config()->get_number("constants.ice.density"),
  ...
  try {
    for (auto p : grid.points()) {
      ...
      double xx = grid.x(i), yy = grid.y(j), r = sqrt(pow(xx, 2) + pow(yy, 2));
...
static void computeSurfaceVelocityErrors(const Grid &grid, const array::Scalar &ice_thickness,
                                         const array::Array3D &u3, const array::Array3D &v3,
                                         const array::Array3D &w3, double &gmaxUerr,
                                         double &gavUerr, double &gmaxWerr, double &gavWerr) {
  ...
  for (auto p : grid.points()) {
    ...
    if ((r >= 1.0) && (r <= LforFG - 1.0)) { // only evaluate error if inside sheet
      ...
      const double Uerr =
          sqrt(pow(u3.interpolate(i, j, H) - uex, 2) + pow(v3.interpolate(i, j, H) - vex, 2));
...
static void enthalpy_from_temperature_cold(EnthalpyConverter &EC, const Grid &grid,
  ...
  for (auto p : grid.points()) {
    ...
    for (unsigned int k = 0; k < grid.Mz(); ++k) {
      ...
      E_ij[k]      = EC.enthalpy_permissive(T_ij[k], 0.0, EC.pressure(depth));
    }
  }
...
static void setInitStateF(Grid &grid, EnthalpyConverter &EC, array::Scalar &bed,
                          array::Scalar &mask, array::Scalar &surface, array::Scalar &thickness,
                          array::Array3D &enthalpy) {
  ...
  for (auto p : grid.points()) {
    ...
    const double r = std::max(grid::radius(grid, i, j), 1.0), // avoid singularity at origin
        Ts         = Tmin + ST * r;
  ...
  enthalpy_from_temperature_cold(EC, grid, thickness, enthalpy, enthalpy);
}

static void reportErrors(const Grid &grid, units::System::Ptr unit_system,
                         const array::Scalar &thickness, const array::Array3D &u_sia,
                         const array::Array3D &v_sia, const array::Array3D &w_sia,
  ...
  compute_strain_heating_errors(strain_heating, thickness, grid, max_strain_heating_error,
                                av_strain_heating_error);

  log->message(1, "Sigma     :      maxSig       avSig\n");
  ...
  computeSurfaceVelocityErrors(grid, thickness, u_sia, v_sia, w_sia, maxUerr, avUerr, maxWerr,
                               avWerr);

  log->message(1, "surf vels :     maxUvec      avUvec        maxW         avW\n");
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // fill the fields:
    setInitStateF(*grid, *EC, geometry.bed_elevation, geometry.cell_type,
                  geometry.ice_surface_elevation, geometry.ice_thickness, enthalpy);
    ...
    const array::Array3D &u3 = stress_balance.velocity_u(), &v3 = stress_balance.velocity_v(),
                         &w3 = stress_balance.velocity_w();

    const array::Array3D &sigma = stress_balance.volumetric_strain_heating();

    reportErrors(*grid, ctx->unit_system(), geometry.ice_thickness, u3, v3, w3, sigma);
