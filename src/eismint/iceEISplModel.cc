...
const PetscScalar IceEISplModel::DEFAULT_TILL_PHI_FJORD       =  5.0;
...
PetscErrorCode IceEISplModel::initFromOptions() {
  ...
  ierr = PetscOptionsHasName(PETSC_NULL, "-no_trough", &notroughSet);
    CHKERRQ(ierr);
  ...
  ierr = verbPrintf(2,grid.com, 
    "setting up plastic till SSA modification of EISMINT II exper %c ...\n",
  ...
  ierr = PetscOptionsGetRealArray(PETSC_NULL, "-till_phi", phi_list, 
            &input_phi_list_length, &phi_listSet); CHKERRQ(ierr);
  if (phi_listSet == PETSC_TRUE) {
    if (input_phi_list_length != phi_list_length) {
      SETERRQ2(1,
      "option -till_phi must be followed by comma-separated list (no spaces!) of\n"
  ...
  ierr = verbPrintf(2,grid.com, 
           ...
           phi_list[0],phi_list[1],phi_list[2],phi_list[3],phi_list[4]); 
           CHKERRQ(ierr);
  
  ierr = setTillProperties(); CHKERRQ(ierr);

  ierr = verbPrintf(1,grid.com, 
     "running plastic till SSA modification of EISMINT II exper %c ...\n",
     expername);  CHKERRQ(ierr);
  return 0;
}
...
PetscInt IceEISplModel::tillRegionCode(PetscInt i, PetscInt j) {
  PetscInt  code = 1;    // STRONG; applies except in weaker strip and lake
  
  // half-width of EISMINT II computational domain; may not be Lx or Ly:
  const PetscScalar    L = 750.0e3;  
  const PetscScalar    dx = grid.dx, dy = grid.dy;
  const PetscScalar    dx61 = (2.0 * L) / 60.0; // = 25.0e3
  
  // based on 61 x 61 grid because they came from EISMINT II exper I
  const PetscScalar  
     streamBegin  = (31 - 1) * dx61,
...
*/
  ...
  const PetscScalar nsd = i * dx, ewd = j * dy;  // north-south and east-west
  if ( (nsd >= streamBottom) && (nsd <= streamTop) && (ewd >= streamBegin) ) {
    // in weaker strip
    if (ewd >= groundLine) {
      code = 4;  // FJORD
