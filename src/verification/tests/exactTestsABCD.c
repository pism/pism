...
int exactD_old(const double t, const double rin, double *H, double *M) {
  ...
  double r = rin;
  double Gamma, power, s, lamhat, f, Hs, temp, C, Ms, df, ddf, chi, dchi, ddchi, c1, dHs, ddHs, dH,
      ddH, divterms, Mc;
  ...
  if (r >= L - 0.01) {
    ...
    *M = -0.1 / SperA;
    ...
    if (r < 0.01)
      r = 0.01; /* avoid r=0 singularity in formulas */
    ...
    *H = Hs + Cp * sin(2.0 * M_PI * t / Tp) * f;
    ...
    if ((r > 0.3 * L) && (r < 0.9 * L)) {
      ...
      ddHs =
          c1 * ((-5.0 / 8.0) * pow(chi, -13.0 / 8.0) * dchi * dchi + pow(chi, -5.0 / 8.0) * ddchi);
