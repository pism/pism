...
#define barB                                                                                       \
  3.7e8            /* strength of shelf; Pa s^(1/3); as in Schoof 2006;
...
double F_M(double x, double alpha, double r, double Q) {
  const double aor = alpha / r, DD = x * x + x * aor + pow(aor, 2.0);
...
int exactM_old(double r, double *alpha, double *Drr, const double EPS_ABS, const double EPS_REL,
...
struct TestMParameters exactM(double r, double EPS_ABS, double EPS_REL, int ode_method) {
  ...
  result.error_code = exactM_old(r, &result.alpha, &result.Drr, EPS_ABS, EPS_REL, ode_method);
  return result;
}
