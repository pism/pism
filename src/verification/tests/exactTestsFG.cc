...
TestFGParameters exactFG(double t, double r, const std::vector<double> &z, double Cp) {
  ...
  double &H = result.H, &M = result.M;
  std::vector<double> &T = result.T, &U = result.U, &w = result.w, &Sig = result.Sig,
                      &Sigc = result.Sigc;
  ...
  const double lamhatrr =
      ((1 + 1 / n) / (n * L * L)) * (pow(1 - s, (1 / n) - 1) - pow(s, (1 / n) - 1));
  ...
  const double Tsr = ST;
  const double nur =
      (k * Tsr / (2.0 * Ggeo)) * (1 + nusqrt) + (1 / Ts) * (Hr * Ts - H * Tsr) / nusqrt;
  const double mur =
      (-Q / (Rgas * Ts * Ts * pow(nu + H, 2.0))) * (Tsr * (nu + H) + Ts * (nur + Hr));
  const double phi =
      1 / r + n * Hrr / Hr + Q * Tsr / (Rgas * Ts * Ts) - (n + 1) * mur / mu; // division by r
