...
int funcL(double r, const double u[], double f[], void *params) {
  ...
  */
  ...
  if (params == NULL) {
  } /* quash warning "unused parameters" */

  if ((r >= 0.0) && (r <= L)) {
    const double freq   = z0 * M_PI / L;
    const double bprime = b0 * freq * sin(freq * r);
    f[0] = -(8.0 / 3.0) * bprime * pow(u[0], 5.0 / 8.0) - pow(C * r * (Lsqr - r * r), 1.0 / 3.0);
  } else {
...
int getU(const double *r, int N, double *u, const double EPS_ABS, const double EPS_REL,
  ...
  if (N < 1)
    return TESTL_NO_LIST;
  for (k = 1; k < N; k++) {
    if (r[k] > r[k - 1])
      return TESTL_NOT_DECREASING;
  }
...
#else /* old GSL (< 1.15) */
int getU(const double *r, int N, const double *u, const double EPS_ABS, const double EPS_REL,
         const int ode_method) {
  (void)r;
  (void)N;
  (void)u;
...
ExactLParameters::ExactLParameters(size_t size) : H(size), a(size), b(size) {
}
...
ExactLParameters exactL(const std::vector<double> &r) {
  ...
  case TESTL_NOT_DONE:
    throw RuntimeError::formatted(
        PISM_ERROR_LOCATION, "Test L ERROR: exactL_list() returns 'NOT_DONE' (%d) ...", error_code);
    break;
  ...
  case TESTL_NO_LIST:
    throw RuntimeError::formatted(
        PISM_ERROR_LOCATION, "Test L ERROR: exactL_list() returns 'NO_LIST' (%d) ...", error_code);
    break;
  default:
    break;
  }
