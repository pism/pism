...
static int exactK_old(const double t, const double z, double *TT, double *FF,
                      const int bedrockIsIce_p) {
  int k;
  int belowB0;
  double ZZ, alpha, lambda, beta, my_gamma, XkSQR, Xk, theta, dthetakdz, P, dPdz, Ck, I1, I2, aH,
      bB, mI, mR;
  ...
  *FF = 0.0;
  for (k = Nsum - 1; k >= 0; k--) {
    ...
    theta = (z > 0) ? sin(alpha * (H0 - z)) : my_gamma * cos(beta * (B0 + z));
    theta /= Xk;
    dthetakdz = (z > 0) ? -alpha * cos(alpha * (H0 - z)) : -beta * my_gamma * sin(beta * (B0 + z));
    dthetakdz /= Xk;
    ...
    I2 = mR * (cos(bB) - 1.0 + bB * sin(bB)) / (beta * beta) - (B0 * mR + H0 * mI) * sin(bB) / beta;
