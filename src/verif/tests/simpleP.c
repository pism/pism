...
int main() {
  ...
  ierr = exactP(r*1000.0,&h,&magvb,&W,EPS_ABS[0],EPS_REL[0],1);
  if (ierr) {
  ...
  printf("    h = %11.5f (m)  Po = %11.5f (bar)  |vb| = %11.5f (m a-1)  W = %11.5f (m)\n",
         h,910.0*9.81*h/1.0e5,magvb*secpera,W);
...
#define COMMENTARY 1
  ...
  for (method=1; method<5; method++) {
    ...
    for (j=0; j<3; j++) {
      exactP(r*1000.0,&h,&magvb,&W,EPS_ABS[j],EPS_REL[j],method);
      printf("     EPS = [%.1e %.1e]:    h = %11.5f (m)   |vb| = %11.5f (m a-1)   W = %11.5f (m)\n",
