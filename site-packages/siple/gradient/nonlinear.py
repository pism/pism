...
from siple.opt import linesearchCR, linesearchHZ
from siple.reporting import msg

from .forward import ForwardProblemLineSearchAdaptor
from .linear import BasicKrylovCGNE, KrylovSolver
    ...
    def solve(self, x0, y, *args):
        ...
        for kkkkk in range(1):
            while True:
                ...
                rho = (discrepancy - discrepancyPrev) / (
                    discrepancyLin - discrepancyPrev
                )
                ...
                if rho > self.params.rhoLow:
                    ...
                    if rho > self.params.rhoHigh:
                        if theta < self.params.thetaMax:
                            theta = min(
                                theta / self.params.kappaTrust, self.params.thetaMax
                            )
                            msg(
                                "Very good decrease (rho=%g).  New theta %g", rho, theta
                            )
                        else:
                            msg(
                        ...
                        msg(
                            "Reasonable decrease (rho=%g). Keeping theta %g", rho, theta
                        )
                else:
                    # We have a lousy fit.  Try asking for less correction.
                    theta = theta * params.kappaTrust
                    msg(
                        "Poor decrease (rho=%g) from %g to %g;",
                        rho,
                        discrepancyPrev,
                        discrepancy,
                    )
            ...
            while True:
                ...
                if line_search.error():
                    if prevLineSearchFailed:
                        raise NumericalFailure(
                            "linesearch failed twice in a row: %s" % line_search.errMsg
                        )
                    else:
                        msg(
                ...
                if self.params.steepest_descent:
                    ...
                    beta = forward_problem.domainIP(
                        TStarR, TStarR - TStarRLast
                    ) / forward_problem.domainIP(TStarRLast, TStarRLast)
