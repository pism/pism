...
class LinesearchHZ:
    ...
    def search(self, F, F0, F0p, t0):
        ...
        if params.verbose:
            msg("starting at z=%g (%g,%g)", z.t, z.F, z.Fp)
        ...
        if params.verbose:
            msg("initial guess c=%g (%g,%g)", c.t, c.F, c.Fp)
        ...
        if params.verbose:
            msg("initial bracket %g %g", aj.t, bj.t)
        ...
        while True:
            ...
            if params.verbose:
                msg("secantsq a %g b %g", a.t, b.t)
            if params.verbose:
                self.printBracket(a, b)
            if self.code >= 0:
                self.doneMsg("secant")
                return
            ...
            if (b.t - a.t) > params.gamma * (bj.t - aj.t):
                ...
                if params.verbose:
                    msg("update to a %g b %g", aj.t, bj.t)
                if params.verbose:
                    self.printBracket(a, b)
                if self.code >= 0:
                    self.doneMsg("bisect")
    ...
    def printBracket(self, a, b):
        msg(
            "a %g b %g f(a) %g fp(a) %g f(b) %g fp(b) %g fpert %g",
            a.t,
            b.t,
            a.F,
            a.Fp,
    ...
    def update(self, a, b, ct):
        ...
        if params.verbose:
            msg("update %g %g %g", a.t, b.t, ct)
        if (ct <= a.t) or (ct >= b.t):
            if params.verbose:
                msg("midpoint out of interval")
            return (abar, bbar)
    ...
    def secant(self, a, b):
        ...
        if self.params.verbose:
            msg("secant: a %g fp(a) %4.8g b %g fp(b) %4.8g", a.t, a.Fp, b.t, b.Fp)
        if a.t == b.t:
    ...
    def secantsq(self, a, b):
        ...
        if B.t == ct:
            ct2 = self.secant(b, B)
            if self.params.verbose:
                msg("second secant on left half A %g B %g with c=%g", A.t, B.t, ct2)
            (abar, bbar) = self.update(A, B, ct2)
        elif A.t == ct:
            ct2 = self.secant(a, A)
            if self.params.verbose:
                msg("second secant on right half A %g B %g with c=%g", A.t, B.t, ct2)
            (abar, bbar) = self.update(A, B, ct2)
        else:
            if self.params.verbose:
                msg("first secant gave a shrink in update. Keeping A %g B %g", A.t, B.t)
            abar = A
            bbar = B
    ...
    def bracket(self, z, c):
        ...
        while True:
            ...
            if b.F > self.fpert:
                if self.params.verbose:
                    msg("initial bracket contraction")
                return self.ushrink(a, b)

            if self.params.verbose:
                msg("initial bracket expanding")
            a = b
            rho = self.params.rho
            while True:
                if count > self.params.nshrink:
    ...
    def wolfe(self, c):
        ...
        if c.Fp >= self.wolfe_lo:
            ...
            if self.params.verbose:
                msg("failed sufficient decrease")
