...
"""
    ...
    def init_config(self, overrides=None, unit_system_path="", use_command_line_options=True):
        """Initializes the global :cpp:class:`Config`.

            :param overrides: A :cpp:class:`Config` to contain parameter overrides specified
                              via ``-config_override``.
            :param unit_system: A :cpp:class:`UnitSystem` for unit conversion (``None`` for default).
        ...
        system = PISM.UnitSystem(unit_system_path)
        self._config = DefaultConfig(self.com, "pism_config", "-config", system)
        self._config.init_with_default()

        if overrides is None:
            overrides = DefaultConfig(self.com, "pism_overrides", "-config_override", system)
            overrides.init()

        self._config.import_from(overrides)
        PISM.set_config_from_options(self._config)
