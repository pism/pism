# Copyright (C) 2011-2015, 2018, 2022 David Maxwell and Constantine Khroulev
#
...
"""Functions and objects relating to interaction with :cpp:class:`Array`'s from python."""
...
class Access(object):
    ...
    """
    Python context manager to simplify `Array` access and ghost communication.

    In PISM C++ code, one uses :cpp:member:`Array::begin_access`/:cpp:member:`Array::end_access`
    access pairs to delimit a code block allowing access to the contents of an :cpp:class`IcdeModelVec`.
    If the contents of a ghosted vector were changed in the block, :cpp:member:`Array::update_ghosts` needs to
...
def randVectorS(grid, scale, stencil_width=None):
    """Create an :cpp:class:`Scalar` of normally distributed random entries.

      :param grid:  The :cpp:class:`Grid` to use for creating the vector.
    ...
    if stencil_width is None:
        rv = PISM.Scalar(grid, "rand vec")
    elif stencil_width == 1:
        rv = PISM.Scalar1(grid, "rand vec")
    elif stencil_width == 2:
        rv = PISM.Scalar2(grid, "rand vec")
...
def randVectorV(grid, scale, stencil_width=None):
    """Create an :cpp:class:`Vector` of normally distributed random entries.
    ...
    if stencil_width == 0:
        rv = PISM.Vector(grid, "rand vec")
    elif stencil_width == 1:
        rv = PISM.Vector1(grid, "rand vec")
    elif stencil_width == 2:
        rv = PISM.Vector2(grid, "rand vec")
    else:
