...
def initGrid(grid, Lx, Ly, Lz, Mx, My, Mz, p):
    ...
    """
    grid.set_size_and_extent(0.0, 0.0, Lx, Ly, Mx, My, p)
    z = grid.compute_vertical_levels(Lz, Mz, PISM.EQUAL)
    grid.set_vertical_levels(z)
    grid.allocate()
...
def initGridFromFile(ctx, input_file, periodicity, variable='enthalpy'):
    ...
    grid = PISM.IceGrid.FromFile(ctx, pio, variable, periodicity)
    pio.close()
...
def _stencil_width(grid, ghost_type, width):
    ...
    if ghost_type == PISM.WITH_GHOSTS:
        if width is None:
            return int(grid.ctx().config().get_double("grid_max_stencil_width"))
        else:
            return width
    else:
        return 0
...
def createAveragedHardnessVec(grid, name='hardav', desc="vertical average of ice hardness",
    ...
    power = 1.0 / grid.ctx().config().get_double("ssa_Glen_exponent")
    units = "Pa s%f" % power
...
def createBasalWaterVec(grid, name='tillwat', desc="effective thickness of subglacial melt water",
    ...
    valid_max = PISM.Context().config.get_double("hydrology_tillwat_max")
