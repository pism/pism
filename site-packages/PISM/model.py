# Copyright (C) 2011, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022 David Maxwell and Constantine Khroulev
#
...
class ModelVecs(object):
    ...
    to a file.::
        ...
        yieldstress.set_attrs("diagnostic", desc, "Pa", "Pa", "", 0)

        # Construct a thickness vector with intrinsic name 'thk'
        thickness = PISM.Scalar(grid, 'thk')
        thickness.set_attrs("model_state", "land ice thickness", "m", "m", "land_ice_thickness", 0)
...
def createIceSurfaceVec(grid, name='usurf', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    surface.set_attrs("diagnostic", "ice upper surface elevation", "m", "m", "surface_altitude", 0)
    return surface
...
def createBedrockElevationVec(grid, name='topg', desc="bedrock surface elevation",
    ...
    bed.set_attrs("model_state", desc, "m", "m", "bedrock_altitude", 0)
    return bed
...
def createYieldStressVec(grid, name='tauc', desc="yield stress for basal till (plastic or pseudo-plastic model)",
    ...
    tauc.set_attrs("diagnostic", desc, "Pa", "Pa", "", 0)
    return tauc
...
def createAveragedHardnessVec(grid, name='hardav', desc="vertical average of ice hardness",
    ...
    hardav.set_attrs("diagnostic", desc, "1", "1", "", 0) # use dummy units for now

    hardav.metadata().set_units_without_validation("Pa s^(1/n)")
...
def createAgeVec(grid, name='age', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    age.set_attrs("model_state", "age of the ice", "s", "s", "", 0)
    return age
...
def create3DVelocityVecs(grid, ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    u.set_attrs("diagnostic", "x-component of the ice velocity", "m s-1", "m year-1", "", 0)

    v = PISM.Array3D(grid, "vvel", ghost_type, grid.z(), stencil_width)
    v.set_attrs("diagnostic", "y-component of the ice velocity", "m s-1", "m year-1", "", 0)
...
def createBasalWaterVec(grid, name='tillwat', desc="effective thickness of subglacial melt water",
    ...
    tillwat.set_attrs("model_state", desc, "m", "m", "", 0)

    valid_max = PISM.Context().config.get_number("hydrology.tillwat_max")
    tillwat.metadata().set_number("valid_min", 0.0)
    tillwat.metadata().set_number("valid_max", valid_max)
    return tillwat
...
def createGroundingLineMask(grid, name='gl_mask', desc="fractional floatation mask",
    ...
    gl_mask.set_attrs("model_state", desc, "1", "1", "", 0)
    gl_mask.metadata().set_number("valid_min", 0.0)
    gl_mask.metadata().set_number("valid_max", 1.0)
    return gl_mask
...
def create2dVelocityVec(grid, name="", desc="", intent="", ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    vel.set_attrs(intent, "%s%s" % ("X-component of the ", desc), "m s-1", "m year-1", "", 0)
    vel.set_attrs(intent, "%s%s" % ("Y-component of the ", desc), "m s-1", "m year-1", "", 1)
...
def createDrivingStressXVec(grid, name="ssa_driving_stress_x", desc="driving stress", intent="model_state",
    ...
    stress.set_attrs(intent, "%s%s" % ("X-component of the ", desc), "Pa", "Pa", "", 0)
    return stress
...
def createVelocityMisfitWeightVec(grid, name="vel_misfit_weight", ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    vel_misfit_weight.set_attrs("diagnostic",
                                "weight for surface velocity misfit functional", "", "", "", 0)
...
def createIceMaskVec(grid, name='mask', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    ice_mask.set_attrs("model_state", "grounded_dragging_floating integer mask", "", "", "", 0)
...
def createBCMaskVec(grid, name='vel_bc_mask', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    vel_bc_mask.set_attrs("model_state",
...
def createZetaFixedMaskVec(grid, name='zeta_fixed_mask', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    zeta_fixed_mask.set_attrs("model_state", "tauc_unchanging integer mask", "", "", "", 0)
    mask_values = [0, 1]
