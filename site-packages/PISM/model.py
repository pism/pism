...
class ModelVecs:
  ...
  a similar role as the C++ :cpp:class:`Vars` with the key difference that it keeps
  references to its contents.  By contrast, if an :cpp:class:`IceModelVec` is created in python
  and added to a :cpp:class:`Vars`, and if no other reference to the python object is kept, the
  :cpp:class:`IceModelVec` will be deleted, leaving a null pointer lurking in the :cpp:class:`Vars`.
  See method :meth:`asPISMVars` to generate a :cpp:class:`Vars` from the contents of a :class:`ModelVecs`

  The lookup name associated with a vector in a :class:`ModelVecs` has no meaning outside the context of the :class:`ModelVecs`.
  In particular, when converting to a :cpp:class:`Vars`, the lookup names are by default simply the intrinsic
  ...
  converting to a :cpp:class:`Vars`.  And see also functions :func:`createYieldStressVec` and friends
  ...
  def __repr__(self):
    ...
    for (key, value) in self._vecs.items():
      s += "    %s: %s %s\n" %(key, value, value.metadata().get_string("short_name"))
    return s
  ...
  def rename(self, old_name, new_name):
    """Rename a vec. Note that if a vec had a PISMVars name under the old
    name, it should have it under the new name too. Also, if it needed
    writing under the old name, it still needs writing after the
    rename.
    """
    var = self.get(old_name)

    # try to add first; this will fail if new_name is taken already
    self.add(var, new_name)

    if self.pvarnames.has_key(old_name):
      self.pvarnames[new_name] = self.pvarnames[old_name]

    if old_name in self.needs_writing:
      self.needs_writing.add(new_name)
  ...
  def asPISMVars(self):
    """
    :returns: a :cpp:class:`Vars` containing all the vecs in the :class:`ModelVecs`.

    The name associated with each vector in the :cpp:class:`Vars` will be its intrinsic name
    ...
    for (name, var) in self.items():
      ...
      if pv_name is not None:
        ...
        pismVars.add(var)
    pismVars.lock()             # mark this dictionary as fully initialized
    return pismVars
...
def initGrid(grid, Lx, Ly, Lz, Mx, My, Mz, p):
  ...
  grid.set_size(Mx, My)
  grid.set_periodicity(p)

  grid.set_vertical_levels(Lz, Mz, PISM.EQUAL)
  grid.allocate()
...
def initShallowGrid(grid, Lx, Ly, Mx, My, p):
  ...
  Lz = 4000.0                   # pretty arbitrary
  Mz = 3
  initGrid(grid, Lx, Ly, Lz, Mx, My, Mz, p)
...
def initGridFromFile(grid, input_file, periodicity, variable='enthalpy'):
  ...
  pio.open(input_file, PISM.PISM_READONLY)
  PISM.IceGrid.FromFile(pio, variable, periodicity, grid)
  pio.close()
...
def _stencil_width(grid, ghost_type, width):
  if ghost_type == PISM.WITH_GHOSTS:
    if width is None:
      return int(grid.config.get("grid_max_stencil_width"))
    else:
      return width
  else:
    return 0
...
def createAveragedHardnessVec(grid, name='hardav', desc="vertical average of ice hardness",
  ...
  power = 1.0 / grid.config.get("ssa_Glen_exponent")
  units = "Pa s%f" % power
...
def createStrainHeatingVec(grid, name='strain_heating',
                           ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
  """Returns an :cpp:class:`IceModelVec3` with attributes setup for strain heating.
  """
...
def create3DVelocityVecs(grid, ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
  """Returns an :cpp:class:`IceModelVec3`s with attributes setup for the
  x, y, and z-components of the 3D velocity.
  """
  stencil_width = _stencil_width(grid, ghost_type, stencil_width)
  ...
  w.set_attrs("diagnostic", "z-component of the ice velocity relative to the base directly below",
              "m s-1", "")
...
def createBasalWaterVec(grid, name='tillwat', desc="effective thickness of subglacial melt water",
  ...
  valid_max = PISM.Context().config.get("hydrology_tillwat_max")
...
def createCBarVec(grid, name="velbar_mag", ghost_type=PISM.WITHOUT_GHOSTS, stencil_width=None):
  ...
  velbar_mag.set_attrs("diagnostic",
                 ...
                 "m s-1", "")
  velbar_mag.set_glaciological_units("m year-1")
