# Copyright (C) 2011, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025 David Maxwell and Constantine Khroulev
#
...
class ModelVecs(object):
    ...
    def __getitem__(self, key):
        """Allows access to contents via dictionary-like notation, e.g. `vecs['thickness']`."""
        return self.get(key)
...
def initGrid(ctx, Lx, Ly, Lz, Mx, My, Mz, r):
    ...
    """
    P = PISM.GridParameters(ctx.config, Mx, My, Lx, Ly)
...
def createIceSurfaceVec(grid, name='usurf', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    surface.metadata(0).long_name("ice upper surface elevation").units("m").standard_name("surface_altitude")
    return surface
...
def createSeaLevelVec(grid, name='sea_level', ghost_type=PISM.WITH_GHOSTS, stencil_width=1):
    ...
    sea_level.metadata(0).long_name("sea level elevation above reference ellipsoid").units("m").standard_name("sea_surface_height_above_reference_ellipsoid")
    return sea_level

def createIceAreaSpecificVolumeVec(grid, name='ice_area_specific_volume', ghost_type=PISM.WITH_GHOSTS, stencil_width=1):
    """Returns an :cpp:class:`Scalar` with attributes setup for
    ice area specific volume data.
    ...
    sea_level.metadata(0).long_name("ice-volume-per-area in partially-filled grid cells").units("m^3/m^2")
    return sea_level
...
def createIceSurfaceStoreVec(grid, name='usurfstore', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    usurfstore.metadata(0).long_name("saved surface elevation for use to keep surface gradient constant in no_model strip").units("m")
    return usurfstore
...
def createIceThicknessStoreVec(grid, name='thkstore', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    thkstore.metadata(0).long_name("saved ice thickness for use to keep driving stress constant in no_model strip").units("m")
    return thkstore
...
def createAveragedHardnessVec(grid, name='hardav', desc="vertical average of ice hardness",
    ...
    hardav.metadata().set_string("comment",
...
def createEnthalpyVec(grid, name='enthalpy', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    enthalpy.metadata(0).long_name("ice enthalpy (includes sensible heat, latent heat, pressure)").units("J kg^-1")
    return enthalpy
...
def createStrainHeatingVec(grid, name='strain_heating',
    ...
    result.metadata(0).long_name("rate of strain heating in ice (dissipation heating)").units("W m^-3")
    return result
...
def create3DVelocityVecs(grid, ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    u.metadata(0).long_name("x-component of the ice velocity").units("m s^-1").output_units("m year^-1")

    v = PISM.Array3D(grid, "vvel", ghost_type, grid.z(), stencil_width)
    v.metadata(0).long_name("y-component of the ice velocity").units("m s^-1").output_units("m year^-1")

    w = PISM.Array3D(grid, "wvel_rel", ghost_type, grid.z(), stencil_width)
    w.metadata(0).long_name("z-component of the ice velocity relative to the base directly below").units("m s^-1").output_units("m year^-1")
...
def createBasalMeltRateVec(grid, name='bmelt', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    bmr.metadata(0).long_name("ice basal melt rate in ice thickness per time").units("m s^-1").output_units("m year^-1").standard_name("land_ice_basal_melt_rate")
...
def create2dVelocityVec(grid, name="", desc="", ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
...
def createDrivingStressXVec(grid, name="ssa_driving_stress_x", desc="driving stress",
                            ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    """Returns an :cpp:class:`Scalar` with attributes setup for
...
def createDrivingStressYVec(grid, name="ssa_driving_stress_y", desc="driving stress",
                            ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    """Returns an :cpp:class:`Scalar` with attributes setup for
...
def createVelocityMisfitWeightVec(grid, name="vel_misfit_weight", ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    vel_misfit_weight.metadata(0).long_name("weight for surface velocity misfit functional")
...
def createCBarVec(grid, name="velbar_mag", ghost_type=PISM.WITHOUT_GHOSTS, stencil_width=None):
    ...
    velbar_mag.metadata(0).long_name("magnitude of vertically-integrated horizontal velocity of ice").units("m s^-1").output_units("m year^-1")
...
def createBCMaskVec(grid, name='vel_bc_mask', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    """Returns an :cpp:class:`Scalar` with attributes setup for
    ...
    vel_bc_mask.metadata(0).long_name("mask defining locations of velocity boundary conditions")
    vel_bc_mask.metadata().set_numbers("flag_values", [0, 1])
    vel_bc_mask.metadata().set_string("flag_meanings", "no_data ssa.dirichlet_bc_location")
...
def createNoModelMaskVec(grid, name='no_model_mask', ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    no_model_mask.metadata(0).long_name("mask: zeros (modeling domain) and ones (no-model buffer near grid edges)")
...
def createLongitudeVec(grid, name="lon", ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    longitude.metadata(0).long_name("longitude").units("degree_east").output_units("degree_east").standard_name("longitude")
...
def createLatitudeVec(grid, name="lat", ghost_type=PISM.WITH_GHOSTS, stencil_width=None):
    ...
    latitude.metadata(0).long_name("latitude").units("degree_north").standard_name("latitude")
