...
class InvSSASolver_Tikhonov:
  ...
  def solveInverse(self,zeta0,u_obs):
    ...
    if self.method == 'tikhonov_lcl':
      ...
      # pl = []
      # self.ip.addListener(PISM.InvSSA_LCLTikhonovProblemListener())
    else:
...
class InvSSASolver_Siple:
  ...
  def solveInverse(self,zeta0,u_obs):
    try:
      (self.zeta_i,self.u_i) = self.siple_solver.solve(zeta0,u_obs,self.rms_error)
    except Exception:
...
class InvertSSANLCG(InvertNLCG):
  ...
  def stopConditionMet(self,count,x,Fx,y,r):
    ...
    siple.reporting.msg('(%d) J=%g goal=%g',count,J*PISM.secpera,self.Jgoal*PISM.secpera)
