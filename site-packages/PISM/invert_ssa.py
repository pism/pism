...
def constructTikhonovFunctionals(ssarun):
  ...
  if tauc_norm == "hilbert":
    ...
    if useGroundedIceOnly:
      ...
      designFunctional = PISM.GroundedIceH1NormFunctional2S(grid,cL2,cH1,ice_mask,tauc_fixed_mask)
    else:
  ...
  velocity_scale = grid.config.get("inv_ssa_velocity_scale")/secpera;
  ...
  if misfit_type == "rms":  
    ...
    stateFunctional = PISM.LogRatioFunctional(grid,vel_ssa_observed)
    stateFunctional.normalize(0.01);
  elif misfit_type == "log_relative":
    vel_ssa_observed = vecs.vel_ssa_observed
    stateFunctional = PISM.LogRelativeFunctional(grid,vel_ssa_observed)
  ...
  def __call__(self,invssasolver,it,data):
    ...
    if self.misfit_type is None:
      ...
      if self.misfit_type!="rms":
        self.stateFunctional = PISM.MeanSquareFunctional2V(grid);    
        self.stateFunctional.normalize(velScale/secpera);

    if self.misfit_type=="rms":
      misfit = math.sqrt(stateVal)*velScale
    else:
      misfit = math.sqrt(self.stateFunctional.valueAt(data.r))*velScale
