# Copyright (C) 2011, 2012, 2014, 2015, 2017, 2020, 2022 David Maxwell
#
...
class PISMLocalVector(siple.linalg.AbstractVector):
    ...
    """Implements the :class:`siple.linalg.linalg_abstract.AbstractVector` interface for few kinds of PISM :cpp:class:`Array`."""

    def __init__(self, u):
        if isinstance(u, PISM.Scalar):
            self._core = u
            self.dof = 1
        elif isinstance(u, PISM.Vector):
            self._core = u
            self.dof = 2
        else:
            raise ValueError(
                "An PISMLocalVector wraps PISM array::Scalar or Vector: found a %s" % u
            )
        self.grid = u.grid()
    ...
    def vector_like(self):
        if self.dof == 1:
            c = PISM.Scalar2(self.grid, "")
        else:
            c = PISM.Vector2(self.grid, "")
