...
class InvSSASolver_Gradient:
  """Inverse SSA solver based on `siple` iterative gradient methods."""

  def __init__(self,ssarun,method):
    """
    :param ssarun: The :class:`PISM.ssa.SSARun` defining the forward problem. See :class:`PISM.inv_ssa.SSATaucForwardRunFromInputFile`.
    ...
    self.rms_error = self.config.get("inv_root_misfit");

    if PISM.optionsFlag("-inv_old_siple",""):
      self.forward_problem = SSAForwardProblem_deprecated(ssarun)
    else:
      self.forward_problem = SSATaucForwardProblem(ssarun)
  ...
  def solveInverse(self,zeta0,u_obs,zeta_inv):
    ...
    :param zeta0: The best `a-priori` guess for the value of the parameterized :math:`\\tau_c` value :math:`\zeta`.
                  Ignored by siple iterative algorithms.
...
class SSATaucForwardProblem(NonlinearForwardProblem):
  """Subclass of a :class:`siple.NonlinearForwardProblem` defining the forward problem for ``siple``.
  The heavy lifting is forwarded to an :cpp:class:`IP_SSATaucForwardProblem` living inside
  a :class:`PISM.invert.ssa.SSATaucForwardRun`."""

  def __init__(self,ssarun):
    """

    :param ssarun:  A :class:`PISM.invert.ssa.SSATaucForwardRun`
                    or :class:`PISM.invert.ssa.SSATaucForwardRunFromInputFile` defining
                    the forward problem."""
    ...
    (self.designFunctional,self.stateFunctional) = createFunctionals(ssarun)
  ...
  def rangeVector(self):
    ...
    huge_vel = self.grid.convert(1e6, "m/year", "m/second")
...
class SSAForwardProblem_deprecated(NonlinearForwardProblem):
...
class InvertSSANLCG(InvertNLCG):
  """Subclass of :class:`siple.gradient.nonlinear.InvertNLCG` for inversion of SSA velocities
  from :math:`\\tau_c`."""
...
class InvertSSAIGN(InvertIGN):
  """Subclass of :class:`siple.gradient.nonlinear.InvertIGN` for inversion of SSA velocities
  from :math:`\\tau_c`."""
  ...
  def initialize(self,x,y,misfit_size):
    """
    Hook called at the start of :meth:`solve`.  This gives the class a chance to massage the input.
...
class SipleDesignUpdateListenerAdaptor:
