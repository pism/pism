...
class InvSSASolver_Gradient:
  ...
  def __init__(self,ssarun,method):
    ...
    self.target_misfit = self.config.get("inv_target_misfit");

    for o in PISM.OptionsGroup(ssarun.grid.com,"","InvSSASolver_Gradient"):
      monitor_adjoint = PISM.optionsFlag("-inv_monitor_adjoint","Track accuracy of the adjoint during computation",default=False)
      old_siple = PISM.optionsFlag("-inv_old_siple","")
...
class SipleIterationListenerAdaptor:
  ...
  def __call__(self,siplesolver,it,x,Fx,y,d,r,*args):    
    ...
    data = Bunch(zeta=x.core(),u=Fx.core(),u_obs=y.core(),d_zeta=d.core(),r=r.core(),target_misfit=self.owner.target_misfit)
