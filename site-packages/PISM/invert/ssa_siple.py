# Copyright (C) 2012, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023 David Maxwell
#
...
from PISM.util import Bunch, convert
...
from siple.gradient.nonlinear import InvertIGN, InvertNLCG
...
class InvSSASolver_Gradient(InvSSASolver):
    ...
    def __init__(self, ssarun, method):
        ...
        monitor_adjoint = PISM.OptionBool(
            "-inv_monitor_adjoint", "Track accuracy of the adjoint during computation"
        )
        morozov_scale_factor = PISM.OptionReal(
            unit_system,
            "-inv_morozov_scale",
        ...
        ign_theta = PISM.OptionReal(
            unit_system, "-ign_theta", "theta parameter for IGN algorithm", "1", 0.5
        )
        ...
        velocity_scale = (
            ssarun.grid.ctx()
            .config()
            .get_number("inverse.ssa.velocity_scale", "m/year")
        )
    ...
    def solveForward(self, zeta, out=None):
        ...
        if out is None:
            ...
            out = self.forward_problem.F(
                PISMLocalVector(zeta), out=PISMLocalVector(out)
            )
        return out.core()

    def addIterationListener(self, listener):
        """Add a listener to be called after each iteration.  See FIXME."""
        self.siple_solver.addIterationListener(
            SipleIterationListenerAdaptor(self, listener)
        )

    def addDesignUpdateListener(self, listener):
        self.siple_solver.addXUpdateListener(
            SipleDesignUpdateListenerAdaptor(self, listener)
        )

    def addLinearIterationListener(self, listener):
        "Add a linear iteration listener."
        self.siple_solver.addLinearIterationListener(
            SipleLinearIterationListenerAdaptor(self, listener)
        )
    ...
    def solveInverse(self, zeta0, u_obs, zeta_inv):
        ...
        :param u_obs: :cpp:class:`Vector` of observed surface velocities.
        ...
        try:
            ...
            (self.zeta_i, self.u_i) = self.siple_solver.solve(
                zeta_inv, u_obs, self.target_misfit
            )
        except Exception:
            import traceback
...
class SSAForwardProblem(NonlinearForwardProblem):
    ...
    def __init__(self, ssarun):
        ...
        (
            self.designFunctional,
            self.stateFunctional,
        ) = PISM.invert.ssa.createGradientFunctionals(ssarun)
    ...
    def TStar(self, r, out=None):
        ...
        if self.designForm is None:
            stencil_width = int(
                self.grid.ctx().config().get_number("grid.max_stencil_width")
            )
        ...
        if reason < 0:
            raise RuntimeError(
                "TStarB linear solve failed to converge (KSP reason %s)\n\n" % reason
            )
        else:
    ...
    def rangeVector(self):
        ...
        dims = ["X", "Y"]
        for d in [0, 1]:
            v.metadata(d).long_name(
                "%s-component of the SSA velocity computed by inversion" % dims[d]
            )
...
class SipleIterationListenerAdaptor(object):
    ...
    def __call__(self, siplesolver, it, x, Fx, y, d, r, *args):
        ...
        data = Bunch(
            ...
            zeta_step=d.core(),
            residual=r.core(),
            target_misfit=self.owner.target_misfit,
        )
...
class MonitorAdjoint(object):
    ...
    def __call__(self, inverse_solver, count, data):
        ...
        logMessage(
            "adjoint test: <Td,r>=%g <d,T^*r>=%g (percent error %g)",
            ip1,
            ip2,
            (abs(ip1 - ip2)) / max(abs(ip1), abs(ip2)),
        )
