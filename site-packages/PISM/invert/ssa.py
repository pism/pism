...
def createTikhonovFunctionals(ssarun):
  ...
  """
    ...
    if misfit_type == "meansquare":
        ...
        scale = grid.config().get_double("inv_log_ratio_scale")
        velocity_eps = grid.config().get_double("inv_ssa_velocity_eps", "m/year", "m/second")
        misfit_weight = None
        ...
        vel_ssa_observed = vecs.vel_ssa_observed
        velocity_scale = grid.config().get_double("inv_ssa_velocity_scale", "m/year", "m/second")
        velocity_eps = grid.config().get_double("inv_ssa_velocity_eps", "m/year", "m/second")
        misfit_weight = None
        if vecs.has('vel_misfit_weight'):
            misfit_weight = vecs.vel_misfit_weight
    ...
    if design_functional == "sobolevH1":
        ...
        velocity_scale = grid.config().get_double("inv_ssa_velocity_scale", "m/year", "m/second")
        length_scale = grid.config().get_double("inv_ssa_length_scale")
        lebesgue_exponent = grid.config().get_double("inv_ssa_tv_exponent")
        cTV = 1 / area
        ...
        if strain_rate_eps is None:
            schoofLen = grid.config().get_double("Schoof_regularizing_length", "km", "m")
...
def createHilbertDesignFunctional(grid, vecs, useGroundedIceOnly):
  ...
  """
    cL2 = grid.config().get_double("inv_design_cL2")
    cH1 = grid.config().get_double("inv_design_cH1")
...
class RMSMisfitReporter(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        rms_misfit = math.sqrt(Jmisfit) * grid.config().get_double("inv_ssa_velocity_scale")
...
class MisfitLogger(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        if self.misfit_type == "meansquare":
            velScale_m_per_year = grid.config().get_double("inv_ssa_velocity_scale")
