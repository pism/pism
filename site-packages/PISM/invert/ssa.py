...
class SSAForwardRunFromInputFile(SSAForwardRun):
    ...
    def _initGrid(self):
        ...
        # Use enthalpy to determine the vertical grid if available;
        # fall back to thk (2D) for isothermal runs without enthalpy.
        if input_file.variable_exists("enthalpy"):
            z_vars = ["enthalpy"]
        else:
            z_vars = ["thk"]
        self.grid = PISM.Grid.FromFile(ctx, input_file, z_vars, registration)
