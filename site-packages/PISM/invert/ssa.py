...
def createGradientFunctionals(ssarun):
  ...
  command-line parameters ``inv_misfit_func`` and ``inv_design_func``.  
  ...
  for o in PISM.OptionsGroup(grid.com,"","InvSSAFunctionals"):
      ...
      (misfit_type,wasSet) = PISM.optionsListWasSet(grid.com,'-inv_misfit_func',"Misfit functional type",['rms','log_relative','log_ratio'],'rms')
  if misfit_type != 'rms':
    inv_method = grid.config.get_string("inv_ssa_method")
    raise Exception("'-inv_misfit_func %s' is not supported with '-inv_method %s'.\nUse '-inv_misfit_func rms' instead" % (misfit_type,inv_method))
...
"""
  ...
  if misfit_type == "rms":  
    ...
    raise Exception("Unknown inv_misfit_func '%s'; unable to construct solver.",misfit_type)
...
class PrintTikhonovProgress:
  ...
  def __call__(self,invssasolver,it,data):
    ...
    if self.misfit_type is None:
      for o in PISM.OptionsGroup(grid.com,"","SSATikhonovFunctionals"):
        (self.misfit_type,wasSet) = PISM.optionsListWasSet(grid.com,'-inv_misfit_func',"Misfit functional type",['rms','log_relative','log_ratio'],'rms')
