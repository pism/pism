...
class SSAForwardRunFromInputFile(SSAForwardRun):
    ...
    def _initGrid(self):
        ...
        pio = PISM.File(ctx.com(), "netcdf3", self.input_filename, PISM.PISM_READONLY)
...
def createTikhonovFunctionals(ssarun):
  ...
  """
    ...
    if misfit_type == "meansquare":
        ...
        scale = grid.ctx().config().get_number("inverse.log_ratio_scale")
        velocity_eps = grid.ctx().config().get_number("inverse.ssa.velocity_eps", "m/second")
        misfit_weight = None
    ...
    if design_functional == "sobolevH1":
        ...
        lebesgue_exponent = grid.ctx().config().get_number("inverse.ssa.tv_exponent")
        cTV = 1 / area
        ...
        if not strain_rate_eps.is_set():
            schoofLen = grid.ctx().config().get_number("flow_law.Schoof_regularizing_length", "m")
...
class MisfitLogger(object):
    ...
    def write(self, output_filename):
        ...
        ds = PISM.File(PISM.Context().com, "netcdf3", output_filename, PISM.PISM_READWRITE)
        ...
        ds.put_vara_double("inv_ssa_misfit", [0], [N], self.misfit_history)
        ds.close()
