...
class SSAForwardRun(PISM.ssa.SSARun):
    ...
    def __init__(self, design_var):
        ...
        self.config = self.grid.ctx().config()
...
class SSAForwardRunFromInputFile(SSAForwardRun):
    ...
    def _initGrid(self):
        ...
        self.grid = PISM.model.initGridFromFile(PISM.Context().ctx, self.input_filename, periodicity)
...
def createGradientFunctionals(ssarun):
    ...
    if misfit_type != 'meansquare':
        inv_method = grid.ctx().config().get_string("inv_ssa_method")
...
def createTikhonovFunctionals(ssarun):
  ...
  """
    ...
    if misfit_type == "meansquare":
        ...
        velocity_eps = grid.ctx().config().get_double("inv_ssa_velocity_eps", "m/year", "m/second")
        misfit_weight = None
        ...
        vel_ssa_observed = vecs.vel_ssa_observed
        velocity_scale = grid.ctx().config().get_double("inv_ssa_velocity_scale", "m/year", "m/second")
        velocity_eps = grid.ctx().config().get_double("inv_ssa_velocity_eps", "m/year", "m/second")
        misfit_weight = None
        if vecs.has('vel_misfit_weight'):
            misfit_weight = vecs.vel_misfit_weight
    ...
    if design_functional == "sobolevH1":
        ...
        if strain_rate_eps is None:
            schoofLen = grid.ctx().config().get_double("Schoof_regularizing_length", "km", "m")
...
def createHilbertDesignFunctional(grid, vecs, useGroundedIceOnly):
  ...
  """
    cL2 = grid.ctx().config().get_double("inv_design_cL2")
    cH1 = grid.ctx().config().get_double("inv_design_cH1")
...
class RMSMisfitReporter(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        rms_misfit = math.sqrt(Jmisfit) * grid.ctx().config().get_double("inv_ssa_velocity_scale")
...
class MisfitLogger(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        if self.misfit_type is None:
            self.misfit_type = grid.ctx().config().get_string("inv_state_func")
        ...
        if self.misfit_type == "meansquare":
            velScale_m_per_year = grid.ctx().config().get_double("inv_ssa_velocity_scale")
