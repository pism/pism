...
def printTikhonovProgress(invssasolver, it, data):
    ...
    if data.has_key('grad_JTikhonov'):
      logMessage("gradient: design %.8g state %.8g sum %.8g\n" % (data.grad_JDesign.norm(PETSc.NormType.NORM_2)*dWeight,
                                                                  data.grad_JState.norm(PETSc.NormType.NORM_2)*sWeight,
                                                                  data.grad_JTikhonov.norm(PETSc.NormType.NORM_2)))
    else:
      logMessage("gradient: design %.8g state %.8g; constraints: %.8g\n" % (data.grad_JDesign.norm(PETSc.NormType.NORM_2)*dWeight,
                                                                            data.grad_JState.norm(PETSc.NormType.NORM_2)*sWeight,
                                                                            data.constraints.norm(PETSc.NormType.NORM_2)))
