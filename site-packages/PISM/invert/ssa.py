# Copyright (C) 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2021, 2022, 2023 David Maxwell and Constantine Khroulev
#
...
class SSAForwardRun(PISM.ssa.SSARun):
    ...
    needed for solving the SSA (:cpp:class:`Array`'s, :cpp:class:`EnthalpyConverter`, etc.) as well
    ...
    def _constructSSA(self):
        ...
        """
        md = self.modeldata
        return createSSAForwardProblem(
            md.grid, md.enthalpyconverter, self.design_var_param, self.design_var
        )
...
class SSAForwardRunFromInputFile(SSAForwardRun):
    ...
    def _initGrid(self):
        ...
        self.grid = PISM.Grid.FromFile(
            ctx, self.input_filename, ["enthalpy"], registration
        )
    ...
    def _initPhysics(self):
        ...
        if PISM.OptionBool("-ssa_glen", "SSA flow law Glen exponent"):
            ...
            config.scalar_from_option(
                "flow_law.isothermal_Glen.ice_softness", "ice_softness"
            )
        else:
    ...
    def _initSSACoefficients(self):
        ...
        file_has_inputs = (
            PISM.util.fileHasVariable(self.input_filename, "bmelt")
            and PISM.util.fileHasVariable(self.input_filename, "tillwat")
            and PISM.util.fileHasVariable(self.input_filename, "tillphi")
        )
        ...
        if file_has_inputs:
            ...
            if self.is_regional:
                yieldstress = PISM.RegionalDefaultYieldStress(
                    self.modeldata.grid, subglacial_hydrology
                )
            else:
                yieldstress = PISM.MohrCoulombYieldStress(
                    self.modeldata.grid, subglacial_hydrology
                )
        ...
        vecs.gl_mask.regrid(
            self.input_filename, critical=False, default_value=0.0
        ...
        if self.config.get_flag("stress_balance.ssa.dirichlet_bc"):
            vecs.add(
                PISM.model.create2dVelocityVec(
                    self.grid, name="_bc", desc="SSA velocity boundary condition"
                ),
                "vel_bc",
            )
            ...
            if (not has_u_bc) or (not has_v_bc):
                PISM.verbPrintf(
                    2,
                    self.grid.com,
                    "Input file '%s' missing Dirichlet boundary data u/v_bc; using zero default instead."
                    % self.input_filename,
                )
                vecs.vel_bc.set(0.0)
            else:
            ...
            if self.is_regional:
                ...
                if PISM.util.fileHasVariable(self.input_filename, bc_mask_name):
                    ...
                    PISM.verbPrintf(
                        2,
                        self.grid.com,
                        "Input file '%s' missing Dirichlet location mask '%s'.  Default to no Dirichlet locations."
                        % (self.input_filename, bc_mask_name),
                    )
...
class InvSSASolver(object):
    ...
    def solveForward(self, zeta, out=None):
        ...
        :param zeta: :cpp:class:`Array` containing :math:`\zeta`.
        :param out: optional :cpp:class:`Array` for storage of the computation result.
        :returns: An :cpp:class:`Array` contianing the computation result.
        """
    ...
    def solveInverse(self, zeta0, u_obs, zeta_inv):
        ...
        :param u_obs: :cpp:class:`array::Vector` of observed surface velocities.
    ...
    def inverseSolution(self):
        """Returns a tuple ``(zeta, u)`` of :cpp:class:`Array`'s corresponding to the values
...
def createInvSSASolver(ssarun, method=None):
    ...
    if method == "tikhonov_gn":
        ...
        try:
            ...
            raise RuntimeError(
                "Inversion method '%s' requires the TAO library." % method
            )
    ...
    if method == "sd" or method == "nlcg" or method == "ign":
        try:
            ...
            raise RuntimeError(
                "Inversion method '%s' requires the siple python library." % method
            )
...
design_param_types = {
    "ident": PISM.IPDesignVariableParamIdent,
...
def createTikhonovFunctionals(ssarun):
    ...
    if misfit_type == "meansquare":
        ...
        stateFunctional = PISM.IPLogRatioFunctional(
            grid, vel_ssa_observed, velocity_eps, misfit_weight
        )
        ...
        raise RuntimeError(
            "Unknown inv_state_func '%s'; unable to construct solver.", misfit_type
        )
...
def createMeanSquareMisfitFunctional(grid, vecs):
    ...
    velocity_scale = (
        grid.ctx().config().get_number("inverse.ssa.velocity_scale", "m/second")
    )
...
def printTikhonovProgress(invssasolver, it, data):
    ...
    logMessage(
        "design objective %.8g; weighted %.8g\n" % (designVal, designVal * dWeight)
    )
    if "grad_JTikhonov" in data:
        logMessage(
            "gradient: design %.8g state %.8g sum %.8g\n"
            % (
        ...
        logMessage(
            "gradient: design %.8g state %.8g; constraints: %.8g\n"
            % (
                data.grad_JDesign.norm(norm_type)[0] * dWeight,
                data.grad_JState.norm(norm_type)[0] * sWeight,
...
class RMSMisfitReporter(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        rms_misfit = math.sqrt(Jmisfit) * grid.ctx().config().get_number(
...
class MisfitLogger(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        if self.misfit_type == "meansquare":
            velScale_m_per_year = (
                grid.ctx().config().get_number("inverse.ssa.velocity_scale")
            )
    ...
    def write(self, output_filename):
        ...
        ds = PISM.File(
            PISM.Context().com, output_filename, PISM.PISM_NETCDF3, PISM.PISM_READWRITE
        )
