...
class SSAForwardRunFromInputFile(SSAForwardRun):
    ...
    def _initSSACoefficients(self):
        ...
        if self.config.get_boolean('ssa.dirichlet_bc'):
...
def createInvSSASolver(ssarun, method=None):
    """Factory function returning an inverse solver appropriate for the config variable ``inverse.ssa_method``.

    :param ssarun: an instance of :class:`SSAForwardRun:` or :class:`SSAForwardRunFromInputFile`.
    :param method: a string correpsonding to config variable ``inverse.ssa_method`` describing the inversion method to be used.
    """
    if method is None:
        method = ssarun.config.get_string('inverse.ssa_method')
...
def createDesignVariableParam(config, design_var_name, param_name=None):
    ...
    if param_name is None:
        param_name = config.get_string("inverse.design_param")
...
def createGradientFunctionals(ssarun):
    ...
    command-line parameters ``inverse.state_func`` and ``inverse.design_func``.
    ...
    if misfit_type != 'meansquare':
        inv_method = grid.ctx().config().get_string("inverse.ssa_method")
    ...
    design_functional = grid.ctx().config().get_string("inverse.design_func")
...
def createTikhonovFunctionals(ssarun):
  ...
  """
    ...
    if misfit_type == "meansquare":
        ...
        velocity_eps = grid.ctx().config().get_double("inverse.ssa_velocity_eps", "m/second")
        misfit_weight = None
        ...
        vel_ssa_observed = vecs.vel_ssa_observed
        velocity_scale = grid.ctx().config().get_double("inverse.ssa_velocity_scale", "m/second")
        velocity_eps = grid.ctx().config().get_double("inverse.ssa_velocity_eps", "m/second")
        misfit_weight = None
        if vecs.has('vel_misfit_weight'):
            misfit_weight = vecs.vel_misfit_weight
    ...
    if design_functional == "sobolevH1":
        ...
        velocity_scale = grid.ctx().config().get_double("inverse.ssa_velocity_scale", "m/second")
        length_scale = grid.ctx().config().get_double("inverse.ssa_length_scale")
        lebesgue_exponent = grid.ctx().config().get_double("inverse.ssa_tv_exponent")
        cTV = 1 / area
...
def createHilbertDesignFunctional(grid, vecs, useGroundedIceOnly):
  ...
  """
    cL2 = grid.ctx().config().get_double("inverse.design_cL2")
    cH1 = grid.ctx().config().get_double("inverse.design_cH1")
...
class RMSMisfitReporter(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        rms_misfit = math.sqrt(Jmisfit) * grid.ctx().config().get_double("inverse.ssa_velocity_scale")
...
class MisfitLogger(object):
    ...
    def __call__(self, invssa_solver, it, data):
        ...
        if self.misfit_type == "meansquare":
            velScale_m_per_year = grid.ctx().config().get_double("inverse.ssa_velocity_scale")
