...
class SSAForwardRun(PISM.ssa.SSARun):
  ...
  def _setFromOptions(self):
    ...
    for o in PISM.OptionsGroup(PISM.Context().com,"","SSAForwardRun"):
      self.using_zeta_fixed_mask = PISM.optionsFlag("-use_zeta_fixed_mask","Keep zeta constant except where grounded ice is present",default=True)
...
def createDesignVariableParam( config, design_var_name, param_name=None ):
  ...
  if param_name is None:
    param_name = PISM.optionsList(PISM.Context().com,"-inv_ssa_design_param","zeta->tauc parameterization",["ident","square","exp","trunc"],"")
    if param_name == "":
      param_name = config.get_string("inv_ssa_design_param")
  design_param = design_param_types[param_name]()
  design_param.set_scales(config,design_var_name)
  return design_param

ssa_forward_problems = {'tauc':PISM.IP_SSATaucForwardProblem,'hardness': None} #PISM.IP_SSAHardnessForwardProblem}

def createSSAForwardProblem(grid,basal,ec,tauc_param,config,design_var):
  ...
  return ForwardProblem(grid,basal,ec,tauc_param,config)
...
def createGradientFunctionals(ssarun):
  ...
  command-line parameters ``inv_ssa_misfit`` and ``inv_ssa_design_functional``.  
  ...
  if misfit_type != 'rms':
    ...
    raise Exception("'-inv_ssa_misfit %s' is not supported with '-inv_method %s'.\nUse '-inv_ssa_misfit rms' instead" % (design_functional,inv_method))

  design_functional = grid.config.get_string("inv_ssa_design_functional")
  if design_functional != "hilbert":
    inv_method = grid.config.get_string("inv_ssa_method")
    raise Exception("'-inv_ssa_design_functional %s' is not supported with '-inv_method %s'.\nUse '-inv_ssa_design_functional hilbert' instead" % (design_functional,inv_method))
  ...
  # Ugly second renormalization so that stateFunctional always has units of m/year.  This is assumed
  # in both InvSSASolver_Gradient and PrintRMSMisfit and shoudl be fixed....
  velocity_scale = grid.convert(1.0, "m/year", "m/second")
...
"""
  ...
  if design_functional == "hilbert":
    ...
    raise Exception("Unknown inv_ssa_design_functional '%s'; unable to construct solver.",design_functional)
