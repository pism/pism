# Copyright (C) 2012, 2014, 2015, 2016, 2018, 2022 David Maxwell and Constantine Khroulev
#
...
class InvSSASolver_Tikhonov(InvSSASolver):
    ...
    def solveInverse(self, zeta0, u_obs, zeta_inv):
        ...
        if design_var == "tauc":
            ...
            raise RuntimeError(
                "Unsupported design variable '%s' for InvSSASolver_Tikhonov. Expected 'tauc' or 'hardness'"
                % design_var
            )

        tao_type = self.tao_types[self.method]
        (stateFunctional, designFunctional) = PISM.invert.ssa.createTikhonovFunctionals(
            self.ssarun
        )

        self.ip = problemClass(
            self.ssarun.ssa, zeta0, u_obs, eta, stateFunctional, designFunctional
        )
