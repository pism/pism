# Copyright (C) 2011, 2012, 2015, 2018, 2022, 2024 David Maxwell
#
...
class PlotListener(object):
  ...
  Provides objects for converting :cpp:class:`Array`'s to ``numpy`` vectors
  on processor zero, as well as basic ``matplotlib`` figure management."""
    ...
    def toproczero(self, *args):
        """Returns a ``numpy`` vector on processor zero corresponding to an :cpp:class:`Array`.
        Takes as input either a single :cpp:class:`Array` or dictionary of such
