...
class InvSSASolver_TikhonovGN(InvSSASolver):
    ...
    def __init__(self, ssarun, method):
        ...
        target_misfit = PISM.OptionReal(
            "-inv_target_misfit", "m/year; desired root misfit for inversions", 0.0
        )
    ...
    def solveInverse(self, zeta0, u_obs, zeta_inv):
        ...
        self.solver = PISM.IP_SSATaucTikhonovGNSolver(
            self.ssarun.ssa, zeta0, u_obs, eta, designFunctional, stateFunctional
        )

        vel_scale = (
            self.ssarun.grid.ctx().config().get_number("inverse.ssa.velocity_scale")
        )
