# Copyright (C) 2011, 2012, 2013, 2015, 2016, 2018, 2019, 2021, 2024 David Maxwell and Constantine Khroulev
#
...
def prepare_output(filename, append_time=True, time_units=None, calendar=None):
    "Create an output file and prepare it for writing."
    ctx = PISM.Context()
    output = PISM.File(ctx.com,
                       filename,
                       PISM.string_to_backend(ctx.config.get_string("output.format")),
                       PISM.PISM_READWRITE_MOVE,
                       ctx.ctx.pio_iosys_id())

    time_name = ctx.config.get_string("time.dimension_name")

    if time_units is None:
        time_units = ctx.time.units_string()

    if calendar is None:
        calendar = ctx.config.get_string("time.calendar")

    PISM.define_time(output, time_name, calendar, time_units, ctx.unit_system)

    if append_time:
        PISM.append_time(output, time_name, ctx.time.current())
...
def writeProvenance(outfile, message=None):
    ...
    ds = PISM.File(com, outfile, PISM.PISM_NETCDF3, PISM.PISM_READWRITE)

    if message is None:
        message = PISM.timestamp(com) + ": " + PISM.args_string()
    ds.append_history(message)
    ds.write_attribute("PISM_GLOBAL", "source", "PISM " + PISM.revision)
