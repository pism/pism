...
def prepare_output(filename, append_time=True):
    ...
    PISM.define_dimension(output, time.get_name(), PISM.PISM_UNLIMITED)
    PISM.define_variable(output, time, [time.get_name()])

    if append_time:
        PISM.append_time(output, time.get_name(), ctx.time.current())
