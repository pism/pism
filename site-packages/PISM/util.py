...
def prepare_output(filename, append_time=True):
    ...
    ctx = PISM.Context()
    output = PISM.File(ctx.com, ctx.config.get_string("output.format"),
...
def writeProvenance(outfile, message=None):
    ...
    ds = PISM.File(com, "netcdf3", outfile, PISM.PISM_READWRITE)

    if message is None:
        message = PISM.timestamp(com) + ": " + PISM.args_string()
    ds.append_history(message)
    ds.put_att_text("PISM_GLOBAL", "source", "PISM " + PISM.revision)
...
def fileHasVariable(filename, varname):
    ...
    return PISM.File(PISM.Context().com, "netcdf3", filename, PISM.PISM_READONLY).inq_var(varname)
