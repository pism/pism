...
def create_scalar_forcing(file_name, variable_name, units, values, times, time_bounds=None, time_name=None):
    ...
    output.write_attribute(time_name, "units", ctx.time.units().string())
...
def create_forcing(grid, file_name, variable_name, units, values,
    ...
    time.units(time_units).set_string("bounds", time_bounds_name)
    ...
    if time_bounds is not None:
        ...
        output.define_variable(time_bounds_name, [time_name, "nv"], PISM.PISM_DOUBLE,
                               bounds.attributes())
