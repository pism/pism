...
def create_forcing(grid, file_name, variable_name, units, values,
    ...
    output = PISM.util.prepare_output(file_name, append_time=False)
    output.put_att_text(time_name, "units", time_units)

    if time_bounds is not None:
        output.put_att_text(time_name, "bounds", time_bounds_name)

        bounds = PISM.TimeBoundsMetadata(time_bounds_name, time_name, ctx.unit_system())
