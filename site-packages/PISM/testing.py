...
def create_scalar_forcing(
    file_name, variable_name, units, values, times, time_bounds=None, time_name=None
):
    ...
    if time_bounds is not None and times is None:
        ...
        for k in range(len(values)):
            times.append(0.5 * (time_bounds[2 * k + 0] + time_bounds[2 * k + 1]))

    try:
        output = PISM.File(ctx.com, file_name, PISM.PISM_NETCDF3, PISM.PISM_READWRITE)
    except:
        output = PISM.File(
            ctx.com, file_name, PISM.PISM_NETCDF3, PISM.PISM_READWRITE_CLOBBER
        )
    ...
    output.write_attribute(
        time_name, "calendar", ctx.config.get_string("time.calendar")
    )
    ...
    output.write_variable(
        time_name, [0], [len(times)], np.array(times, dtype=np.float64).data
    )

    output.write_variable(
        variable_name, [0], [len(values)], np.array(values, dtype=np.float64).data
    )
    ...
    if time_bounds is not None:
        ...
        for k in range(len(values)):
            ...
            PISM.write_time_bounds(
                output, bounds, k, (time_bounds[k], time_bounds[k + 1])
            )
