...
class InvSSASolver_Tikhonov:
  ...
  def solveInverse(self,zeta0,u_obs,zeta_inv):
    ...
    if self.method == 'tikhonov_lcl':
      ...
      self.ip = PISM.SSATaucTikhonovProblem(self.ssarun.ssa,zeta0,u_obs,eta,stateFunctional,designFunctional)
      self.solver = PISM.SSATaucTikhonovSolver(self.ssarun.grid.com,tao_type,self.ip)
...
class TikhonovIterationListenerAdaptor(PISM.SSATaucTikhonovProblemListener):
  """Adaptor for passing listening events from a TAO-based Tikhonov solver to a python object."""
  def __init__(self,owner,listener):
    PISM.SSATaucTikhonovProblemListener.__init__(self)
    self.owner = owner
    self.listener = listener
