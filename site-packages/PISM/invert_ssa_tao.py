...
class InvSSASolver_Tikhonov:
  ...
  def solveInverse(self,zeta0,u_obs,zeta_inv):
    ...
    if self.method == 'tikhonov_lcl':
      self.ip = PISM.IP_SSATaucTikhonovProblemLCL(self.ssarun.ssa,zeta0,u_obs,eta,stateFunctional,designFunctional)
      self.solver = PISM.IP_SSATaucTikhonovProblemLCLSolver(self.ssarun.grid.com,tao_type,self.ip)
      pl = [ TikhonovLCLIterationListenerAdaptor(self,l) for l in self.listeners ]
    else:
      self.ip = PISM.IP_SSATaucTikhonovProblem(self.ssarun.ssa,zeta0,u_obs,eta,stateFunctional,designFunctional)
      self.solver = PISM.IP_SSATaucTikhonovSolver(self.ssarun.grid.com,tao_type,self.ip)
...
class TikhonovLCLIterationListenerAdaptor(PISM.IP_SSATaucTikhonovProblemLCLListener):
  ...
  def __init__(self,owner,listener):
    PISM.IP_SSATaucTikhonovProblemLCLListener.__init__(self)
    self.owner = owner
...
class TikhonovIterationListenerAdaptor(PISM.IP_SSATaucTikhonovProblemListener):
  """Adaptor for passing listening events from a TAO-based Tikhonov solver to a python object."""
  def __init__(self,owner,listener):
    PISM.IP_SSATaucTikhonovProblemListener.__init__(self)
    self.owner = owner
    self.listener = listener
