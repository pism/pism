...
"""
  ...
  cpp.optionsGroupBegin(com,prefix,title,"");
...
def optionsStringArrayWasSet(option,text,default=None):
  ...
  if default is None:
    default = ""
  val = cpp._optionsStringArray(option,text,default)
  is_set = val[-1]
  result = val[:-1]
  if not is_set:
    result = origDefault
...
def optionsListWasSet(com,option,text,opt_set,default):
  ...
  :returns: Tuple ``(value,wasSet)`` where ``value`` is the value that was set (or the ``default`` value if it was not)
            and ``wasSet`` is a boolean that is ``True`` if the command line option was set explicitly.
  """
  val = cpp._optionsList(com,option,text,opt_set,default)
  return (val[0],val[1])
...
The option value is 
  ...
  neither is set, the `default` is used, and if both are set a :exc:`RuntimeException` is raised.
  """
  ...
  if true_set and false_set:
    raise RuntimeException("Command line options inconsistant: both -%s and -no_%s are set" % (option,option) )
