...
class SSAExactTestCase(SSARun):
  ...
  def report(self):
      ...
      gavuerr      = PISM.globalSum(avuerr, grid.com) / N
      gavverr      = PISM.globalSum(avverr, grid.com) / N
      gmaxvecerr   = PISM.globalMax(maxvecerr, grid.com)
      gavvecerr    = PISM.globalSum(avvecerr, grid.com) / N
...
class SSAFromInputFile(SSARun):
  ...
  def _initSSACoefficients(self):
    ...
    if self.phi_to_tauc:
      ...
      if self.is_regional:
        yieldstress = PISM.RegionalDefaultYieldStress(self.modeldata.grid, self.modeldata.config)
      else:
        yieldstress = PISM.MohrCoulombYieldStress(self.modeldata.grid, self.modeldata.config)
