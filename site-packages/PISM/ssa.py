...
class SSARun:
  ...
  def solve(self):
    ...
    return self.ssa.get_2D_advective_velocity()
  ...
  def write(self, filename):
    ...
    vel_ssa = self.ssa.get_2D_advective_velocity()

    velbar_mag = model.createCBarVec(self.grid)
    vel_ssa.magnitude(velbar_mag)
    velbar_mag.mask_by(vecs.thickness, self.grid.convert(-0.01, "m/year", "m/second"))
  ...
  def _initSSACoefficients(self):
    ...
    if self.config.get_flag('ssa_dirichlet_bc'):
      vecs.add(model.create2dVelocityVec(self.grid, name='_ssa_bc',
                                         desc='SSA velocity boundary condition', intent='intent'), "vel_ssa_bc")
      ...
      if (not has_u_ssa_bc) or (not has_v_ssa_bc):
        PISM.verbPrintf(2, grid.com,
                        "Input file '%s' missing Dirichlet boundary data u/v_ssa_bc; using zero default instead." % self.boot_file)
