...
class SSARun(object):
    ...
    def __init__(self):
        ...
        #: the :class:`SSARun` is involved in setting up this object. Tasks include setting up :cpp:class:Array
    ...
    def setup(self):
        ...
        if self.modeldata is None:
            raise RuntimeError(
                "SSARun._constructModelData failed to provide a ModelData."
            )
        ...
        if self.modeldata.enthalpyconverter is None:
            raise RuntimeError(
                "SSARun._initPhysics failed to initialize the physics of the underlying SSA solver."
            )
    ...
    def solve(self):
        ...
        melange_back_pressure.metadata(0).long_name(
    ...
    def write(self, filename, append=False):
        ...
        PISM.apply_mask(vecs.thk, util.convert(-0.01, "m/year", "m/second"), velbar_mag)
        vecs.add(velbar_mag)
    ...
    def _allocStdSSACoefficients(self):
        """Helper method that allocates the standard :cpp:class:`Array` variables used to solve the SSA and stores them
    ...
    def _allocateBCs(self, velname="_bc", maskname="vel_bc_mask"):
        """Helper method that allocates standard Dirichlet data
          :cpp:class:`Array` variable and stores them in
          :attr:`modeldata` ``.vecs``:
        ...
        * ``vel_bc_mask``
        ...
        vecs.add(
            model.create2dVelocityVec(
                self.grid, name=velname, desc="SSA velocity boundary condition"
            ),
            "vel_bc",
        )
        vecs.add(model.createBCMaskVec(self.grid, name=maskname), "vel_bc_mask")

    def _initSSACoefficients(self):
        """Override to allocate and initialize all :cpp:class:`Array` variables in :attr:`modeldata` ``.vecs``
...
class SSAExactTestCase(SSARun):
    ...
    def report(self):
        ...
        if (
            self.config.get_flag("basal_resistance.pseudo_plastic.enabled")
            and self.config.get_number("basal_resistance.pseudo_plastic.q") != 1.0
        ):
            PISM.verbPrintf(
                1,
                grid.com,
        ...
        PISM.verbPrintf(
            1, grid.com, "NUMERICAL ERRORS in velocity relative to exact solution:\n"
        )
    ...
    def write(self, filename):
        ...
        grid = self.grid
        exact = model.create2dVelocityVec(
            grid, name="_exact", desc="SSA exact solution"
        )
        exact.begin_access()
...
class SSAFromInputFile(SSARun):
    ...
    def _setFromOptions(self):
        self.phi_to_tauc = PISM.OptionBool(
            "-phi_to_tauc", "Recompute pseudo yield stresses from till friction angles."
        )
    ...
    def _initGrid(self):
        ...
        if self.is_regional and (
            self.config.get_string("stress_balance.ssa.method") == "fem"
        ):
        ...
        self.grid = PISM.Grid.FromFile(ctx, self.boot_file, ["enthalpy"], registration)
    ...
    def _constructSSA(self):
        ...
        md = self.modeldata
        if self.is_regional and (
            md.config.get_string("stress_balance.ssa.method") == "fd"
        ):
