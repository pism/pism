...
SSAAlgorithms = {"fem":PISM.SSAFEM, "fd":PISM.SSAFD, "fem_f":PISM.SSAFEM_Forward}
...
class SSASolver:
  ...
  def _constructSSA(self):
    return SSAAlgorithms[self.config.get_string("ssa_method")](self.grid,self.basal,self.ice,self.enthalpyconverter,self.config)
  ...
  def write(self,filename):
    vars = [self.surface,self.bed,self.thickness,self.tauc,self.enthalpy,self.ice_mask]
    for var in vars:
      var.write(filename)
...
class SSATestCase:
  ...
  def write(self,filename):
    ...
    pio.append_time(self.config.get_string("time_dimension_name"),0.0)
    pio.close()
  ...
  def _constructSSA(self):
    return SSASolver(self.grid,self.ice,self.basal,self.enthalpyconverter)
