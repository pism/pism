...
class SSARun:
  ...
  def setup(self):
    ...
    self._initPhysics()
    if (self.modeldata.ice == None) or (self.modeldata.basal == None) or (self.modeldata.enthalpyconverter == None):
  ...
  def solve(self):
    ...
    vars.add(self.ssa.get_advective_2d_velocity())
  ...
  def write(self,filename):
    ...
    vars.writeall(filename)
  
    if vars.has('vel_ssa'):
      cbar = util.standardCBar(self.grid);
      vars.vel_ssa.magnitude(cbar)
      cbar.mask_by(vars.thickness,-0.01/PISM.secpera);
      cbar.write(filename)
  ...
  def _allocStdSSACoefficients(self):
    vars = self.modeldata.vars; grid = self.grid
    vars.add( util.standardIceSurfaceVec( grid ), 'surface')
    vars.add( util.standardIceThicknessVec( grid ), 'thickness')
    vars.add( util.standardBedrockElevationVec(grid), 'bed')
    vars.add( util.standardYieldStressVec( grid ), 'tauc')
    vars.add( util.standardEnthalpyVec( grid ), 'enthalpy' )
    vars.add( util.standardIceMask( grid ), 'ice_mask' )

  def _allocateBCs(self,velname='_bc',maskname='bc_mask'):
    vars = self.modeldata.vars
    vars.add( util.standard2dVelocityVec( self.grid, name=velname, desc='SSA velocity boundary condition',intent='intent' ), "vel_bc" )
    vars.add( util.standardBCMask( self.grid,name=maskname ), "bc_mask" )
  ...
  def _constructSSA(self):
    md = self.modeldata
    return SSAAlgorithms[md.config.get_string("ssa_method")](md.grid,md.basal,md.ice,md.enthalpyconverter,md.config)
...
class SSAExactTestCase(SSARun):
  ...
  def report(self):
      ...
      vel_ssa = self.modeldata.vars.vel_ssa
...
class SSAFromBootFile(SSARun):
  ...
  def _initSSACoefficients(self):
    ...
    mask = vars.ice_mask; surface = vars.surface
    ...
    yieldstress.init(vars.asPISMVars()) 
    yieldstress.basal_material_yield_stress(vars.tauc)
