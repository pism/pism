...
class SSARun(object):
    ...
    def write(self, filename):
        ...
        PISM.append_time(pio, grid.ctx().config().get_string("time.dimension_name"), 0.0)
        pio.close()
    ...
    def _constructSSA(self):
        ...
        md = self.modeldata
        return SSAAlgorithms[md.config.get_string("ssa.method")](md.grid, md.enthalpyconverter)
...
class SSAFromInputFile(SSARun):
    ...
    def _initGrid(self):
        ...
        if pflag:
            ...
            if self.is_regional and (self.config.get_string("ssa.method") == "fem"):
    ...
    def _constructSSA(self):
        ...
        md = self.modeldata
        if self.is_regional and (md.config.get_string("ssa.method") == "fd"):
            algorithm = PISM.SSAFD_Regional
        else:
            algorithm = SSAAlgorithms[md.config.get_string("ssa.method")]
