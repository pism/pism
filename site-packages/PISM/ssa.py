...
class SSAFromInputFile(SSARun):
  ...
  def _initPhysics(self):
    ...
    if (config.get_flag("do_cold_ice_methods") == True):
      ...
      softness = PISM.optionsReal('-glen_ice_softness','Glen ice softness',default=config.get("ice_softness"))
      ice.setSoftness(softness)
    else:
  ...
  def _constructSSA(self):
    ...
    return algorithm(md.grid,md.basal,md.ice,md.enthalpyconverter,md.config)
