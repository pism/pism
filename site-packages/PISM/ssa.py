...
class SSARun:
  ...
  def solve(self):
    ...
    pvars = vecs.asPISMVars(translate={'bc_mask':'bcflag'})
    self.ssa.init(pvars)
    if vecs.has('vel_bc'):
  ...
  def _allocRegionalCoeffs(self):
    vecs = self.modeldata.vecs
    vecs.add( util.standardNoModelMask(grid), 'no_model_mask' )
...
class SSAFromInputFile(SSARun):
  ...
  def _initSSACoefficients(self):
    ...
    if self.phi_to_tauc:
      ...
      # FIXME: Maybe it would be nicer to move PISMRegionalDefaultYieldStress out
      # of pismo.cc and use it to do the following.

      # Now do regional correction if need be
      if self.is_regional:
        tauc = vecs.tauc; no_model_mask = vecs.no_model_mask
        with PISM.util.Access(comm=tauc,nocomm=no_model_mask):
          for (i,j) in grid.points():
